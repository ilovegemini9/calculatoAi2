...
function getDefaultDB(): AppSchema {
  const initialPassword = ADMIN_PASSWORD;
  const adminPasswordHash = bcrypt.hashSync(initialPassword, 10);

  console.log('[db] Initialising default admin user from ADMIN_USERNAME / ADMIN_PASSWORD env vars.');
...
function applySettingsDefaults(parsed: AppSchema): AppSchema {
  ...
  // An empty stored key means "never configured" — fall back to env var.
