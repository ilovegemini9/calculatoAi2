...
import { pruneExpiredSessions } from "./db/session-service.js";
...
const dbReadyPromise = whenReady()
  .then(async () => {
    ...
    // Prune stale sessions on startup, then every 6 hours
...
function getPublicOrigin(req: Request): string {
  ...
  return host ? `${protocol}://${host}` : 'https://calculatorplatform.com';
}
