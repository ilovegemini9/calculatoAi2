...
const handlers: Record<string, CalculatorHandler> = Object.assign({}, {
  ...
  ratio: i => ({ ratio: `${n(i,'a')}:${n(i,'b')}` }),
  proportion: i => ({ d:n(i,'b')*n(i,'c')/positive(i,'a') }),
  'fraction-to-decimal': i => ({ decimal:n(i,'numerator')/positive(i,'denominator') }),
  'decimal-to-fraction': i => { const x=n(i,'decimal'); if(!Number.isFinite(x)) throw new Error('Invalid decimal'); const sign=x<0?-1:1; let num=Math.abs(x), den=1; for(let k=0;k<10 && Math.abs(num-Math.round(num))>1e-10;k++){num*=10;den*=10;} num=Math.round(num)*sign; const g=gcd(Math.abs(num),den); return { numerator:num/g, denominator:den/g, fraction:`${num/g}/${den/g}`}; },
  'quadratic-equation': i => { const a0=n(i,'a'),b=n(i,'b'),cc=n(i,'c'); if(a0===0) throw new Error('a cannot be zero'); const d=b*b-4*a0*cc; if(d<0) return { discriminant:d, roots:'No real roots' }; const r=Math.sqrt(d); return { discriminant:d, root1:(-b+r)/(2*a0), root2:(-b-r)/(2*a0) }; },
  'future-value': i => ({ futureValue:n(i,'presentValue')*Math.pow(1+n(i,'rate'),n(i,'periods')) }),
  'present-value': i => ({ presentValue:n(i,'futureValue')/Math.pow(1+n(i,'rate'),n(i,'periods')) }),
  'loan-payment': i => { const p=positive(i,'principal'), r=n(i,'rate'), m=positive(i,'periods'); if(r===0) return { payment:p/m }; const q=Math.pow(1+r,m); return { payment:p*r*q/(q-1) }; },
  'compound-growth': i => ({ final:n(i,'initial')*Math.pow(1+n(i,'growthRate'),n(i,'periods')) }),
  mortgage: i => { const p=positive(i,'principal'), r=n(i,'annualRate')/12/100, m=positive(i,'years')*12; if(r===0)return {monthlyPayment:p/m}; const q=(1+r)**m; return {monthlyPayment:p*r*q/(q-1)}; },
  'fuel-cost': i => { const used=n(i,'distance')/positive(i,'efficiency'); return { fuelUsed:used,cost:used*n(i,'fuelPrice') }; },
  'real-estate-commission': i => ({ commission:n(i,'salePrice')*n(i,'commissionRate')/100 }),
  'sales-profit': i => { const revenue=n(i,'revenue'),profit=revenue-n(i,'expenses'); return {profit,margin:revenue===0?0:profit/revenue*100}; },
  'celsius-to-kelvin': i => ({ kelvin:n(i,'celsius')+273.15 }),
  'kelvin-to-celsius': i => ({ celsius:n(i,'kelvin')-273.15 }),
  'length-conversion': i => { const m=n(i,'value')*({m:1,km:1000,cm:.01,mm:.001,ft:.3048,in:.0254,mi:1609.344}[String(i.fromUnit)] ?? (()=>{throw new Error('Unsupported from unit')})()); const f={m:1,km:1000,cm:.01,mm:.001,ft:.3048,in:.0254,mi:1609.344}[String(i.toUnit)]; if(!f)throw new Error('Unsupported to unit'); return {converted:m/f}; },
  'area-conversion': i => { const f:{[k:string]:number}={m2:1,km2:1e6,cm2:1e-4,ft2:.09290304,in2:.00064516,acre:4046.8564224,ha:10000}; const a=f[String(i.fromUnit)],b=f[String(i.toUnit)]; if(!a||!b)throw new Error('Unsupported area unit'); return {converted:n(i,'value')*a/b}; },
  'volume-conversion': i => { const f:{[k:string]:number}={m3:1,l:.001,ml:1e-6,gal:.003785411784,ft3:.028316846592,in3:1.6387064e-5}; const a=f[String(i.fromUnit)],b=f[String(i.toUnit)]; if(!a||!b)throw new Error('Unsupported volume unit'); return {converted:n(i,'value')*a/b}; },
  'weight-conversion': i => { const f:{[k:string]:number}={kg:1,g:.001,mg:1e-6,lb:.45359237,oz:.028349523125,tonne:1000}; const a=f[String(i.fromUnit)],b=f[String(i.toUnit)]; if(!a||!b)throw new Error('Unsupported weight unit'); return {converted:n(i,'value')*a/b}; },
  'speed-conversion': i => { const f:{[k:string]:number}={mps:1,kmh:1/3.6,mph:.44704,knot:.514444}; const a=f[String(i.fromUnit)],b=f[String(i.toUnit)]; if(!a||!b)throw new Error('Unsupported speed unit'); return {converted:n(i,'value')*a/b}; },
  'pressure-conversion': i => { const f:{[k:string]:number}={pa:1,kpa:1000,bar:100000,psi:6894.757293168,atm:101325}; const a=f[String(i.fromUnit)],b=f[String(i.toUnit)]; if(!a||!b)throw new Error('Unsupported pressure unit'); return {converted:n(i,'value')*a/b}; },
  ...
  permutation: i => { const a=n(i,'n'),r=n(i,'r'); if(!Number.isInteger(a)||!Number.isInteger(r)||r<0||a<r)throw new Error('Invalid n or r'); let v=1; for(let k=0;k<r;k++)v*=a-k; return {permutations:v}; },
  combination: i => { const a=n(i,'n'),r=n(i,'r'); if(!Number.isInteger(a)||!Number.isInteger(r)||r<0||a<r)throw new Error('Invalid n or r'); const k=Math.min(r,a-r); let v=1; for(let j=1;j<=k;j++)v=v*(a-k+j)/j; return {combinations:v}; },
  'circle-area': i => ({ area:Math.PI*Math.pow(positive(i,'radius'),2) }),
  'circle-circumference': i => ({ circumference:2*Math.PI*positive(i,'radius') }),
  'rectangle-perimeter': i => ({ perimeter:2*(positive(i,'length')+positive(i,'width')) }),
  'triangle-perimeter': i => ({ perimeter:positive(i,'a')+positive(i,'b')+positive(i,'c') }),
  'pythagorean-theorem': i => ({ hypotenuse:Math.hypot(n(i,'a'),n(i,'b')) }),
  'cone-volume': i => ({ volume:Math.PI*Math.pow(positive(i,'radius'),2)*positive(i,'height')/3 }),
  'surface-area-sphere': i => ({ area:4*Math.PI*Math.pow(positive(i,'radius'),2) }),
  'surface-area-cylinder': i => { const r=positive(i,'radius'),h=positive(i,'height'); return {area:2*Math.PI*r*(r+h)}; },
  'angular-speed': i => ({ angularSpeed:n(i,'angle')/positive(i,'time') }),
  acceleration: i => ({ acceleration:(n(i,'finalVelocity')-n(i,'initialVelocity'))/positive(i,'time') }),
  momentum: i => ({ momentum:n(i,'mass')*n(i,'velocity') }),
  impulse: i => ({ impulse:n(i,'force')*n(i,'time') }),
  'centripetal-force': i => ({ force:n(i,'mass')*Math.pow(n(i,'velocity'),2)/positive(i,'radius') }),
  'wave-speed': i => ({ speed:n(i,'frequency')*n(i,'wavelength') }),
  frequency: i => ({ frequency:1/positive(i,'period') }),
  wavelength: i => ({ wavelength:n(i,'speed')/positive(i,'frequency') }),
  'electric-power': i => ({ power:n(i,'voltage')*n(i,'current') }),
  'electric-energy': i => ({ energy:n(i,'power')*n(i,'time') }),
  resistance: i => ({ resistance:n(i,'voltage')/positive(i,'current') }),
  current: i => ({ current:n(i,'voltage')/positive(i,'resistance') }),
  voltage: i => ({ voltage:n(i,'current')*n(i,'resistance') }),
  'bmr-mifflin-st-jeor': i => { const w=positive(i,'weightKg'),h=positive(i,'heightCm'),age=positive(i,'age'),sex=String(i.sex).toLowerCase(); const b=10*w+6.25*h-5*age; return {bmr:sex==='male'||sex==='m'?b+5:b-161}; },
  'ideal-weight': i => { const h=positive(i,'heightCm'); return {maleKg:50+2.3*Math.max(0,(h/2.54)-60),femaleKg:45.5+2.3*Math.max(0,(h/2.54)-60)}; },
  'body-fat-estimate': i => { const bmi=n(i,'bmi'),age=n(i,'age'),sex=String(i.sex).toLowerCase(); const s=(sex==='male'||sex==='m')?1:0; return {bodyFatPercent:1.2*bmi+0.23*age-10.8*s-5.4}; },
  'water-intake': i => ({ liters:positive(i,'weightKg')*0.033 }),
  'protein-intake': i => ({ grams:positive(i,'weightKg')*n(i,'gramsPerKg') }),
  'heart-rate-max': i => ({ maxHeartRate:220-positive(i,'age') }),
  'calories-burned': i => ({ calories:n(i,'met')*positive(i,'weightKg')*positive(i,'hours') }),
  'pregnancy-due-date': i => ({ dueDate:new Date(new Date(String(i.lastPeriod)).getTime()+280*86400000).toISOString().slice(0,10) }),
  'ovulation': i => ({ estimatedOvulationDay:14, cycleLength:positive(i,'cycleLength') }),
  'standard-deviation': i => { const v=values(i),m=v.reduce((s,x)=>s+x,0)/v.length; return {standardDeviation:Math.sqrt(v.reduce((s,x)=>s+(x-m)**2,0)/v.length)}; },
  'z-score': i => ({ zScore:(n(i,'value')-n(i,'mean'))/positive(i,'standardDeviation') }),
  'percentile-rank': i => { const v=values(i).sort((a,b)=>a-b),x=n(i,'value'),below=v.filter(a=>a<x).length,equal=v.filter(a=>a===x).length; return {percentile:(below+0.5*equal)/v.length*100}; },
  'linear-regression-slope': i => { const x=values({values:i.xValues}),y=values({values:i.yValues}); if(x.length!==y.length||x.length<2)throw new Error('xValues and yValues must have same length ≥ 2'); const mx=x.reduce((s,a)=>s+a,0)/x.length,my=y.reduce((s,a)=>s+a,0)/y.length; let num=0,den=0; x.forEach((a,j)=>{num+=(a-mx)*(y[j]-my);den+=(a-mx)**2}); if(den===0)throw new Error('x values cannot all be equal'); return {slope:num/den,intercept:my-(num/den)*mx}; },
  'ph-to-hydrogen-ion': i => ({ hydrogenIonConcentration:Math.pow(10,-n(i,'pH')) }),
  'gas-law': i => ({ finalVolume:n(i,'initialPressure')*n(i,'initialVolume')*n(i,'finalTemperature')/(positive(i,'finalPressure')*positive(i,'initialTemperature')) }),
  'density-mass': i => ({ mass:n(i,'density')*positive(i,'volume') }),
  'density-volume': i => ({ volume:n(i,'mass')/positive(i,'density') }),
  'molar-mass': i => ({ molarMass:n(i,'mass')/positive(i,'moles') }),
