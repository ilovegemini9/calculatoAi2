...
export const CALCULATOR_CONTENT: Record<string, CalcContent> = {
  ...
  'pace': {howToSteps:['Enter distance and elapsed minutes.','Confirm both units.','Review pace per kilometer.','Use the result for training planning.'],formula:{expression:'Pace = elapsed minutes ÷ distance in kilometers',variables:[{symbol:'Minutes',definition:'Elapsed duration'},{symbol:'Distance',definition:'Distance in kilometers'}],notes:'A zero distance returns 0 to avoid division by zero.'},examples:[{title:'Running pace',scenario:'5 km in 30 minutes.',steps:['Enter 5 km.','Enter 30 minutes.','Divide minutes by kilometers.','Review 6 min/km.'],result:'The pace is 6.00 minutes per kilometer.'}],useCases:['Training pace planning.'],commonPitfalls:['Mixing miles and kilometers.','Using moving time inconsistently.'],glossary:[{term:'Pace',definition:'Time required per unit distance.'}],faqs:[{question:'Is this speed?',answer:'No; pace is time per distance.'},{question:'Are results client-side?',answer:'Yes.'}],sources:[{title:'Running Pace Concepts',publisher:'American College of Sports Medicine',url:'https://www.acsm.org/',year:2024}],author:{name:'CalculatorFree Health Review Team',credentials:'Calculation Review',description:'Original client-side estimate.'}},
  'gfr': {howToSteps:['Enter standardized creatinine in mg/dL.','Enter adult age and sex.','Review the CKD-EPI 2021 estimate.','Discuss consequential results with a clinician.'],formula:{expression:'eGFR = 142 × min(Scr/K,1)^α × max(Scr/K,1)^−1.200 × 0.9938^Age × 1.012 if female',variables:[{symbol:'Scr',definition:'Serum creatinine in mg/dL'},{symbol:'K',definition:'0.7 female or 0.9 male'},{symbol:'Age',definition:'Adult age in years'}],notes:'Race-free CKD-EPI 2021 adult estimate; not a diagnosis.'},examples:[{title:'Adult eGFR estimate',scenario:'Use a standardized lab creatinine, age, and sex.',steps:['Enter creatinine.','Enter age.','Choose sex.','Review mL/min/1.73m².'],result:'The estimate is displayed with clinical limitations.'}],useCases:['Educational interpretation of lab inputs.'],commonPitfalls:['Using non-standardized creatinine.','Applying an adult equation to children.','Treating eGFR as a diagnosis.'],glossary:[{term:'eGFR',definition:'Estimated glomerular filtration rate normalized to 1.73m².'}],faqs:[{question:'Does it diagnose CKD?',answer:'No.'},{question:'Which equation is used?',answer:'CKD-EPI 2021 creatinine equation.'}],sources:[{title:'CKD-EPI Creatinine Equation (2021)',publisher:'National Kidney Foundation',url:'https://www.kidney.org/ckd-epi-creatinine-equation-2021',year:2021}],author:{name:'CalculatorFree Health Review Team',credentials:'Calculation Review',description:'Educational implementation of a published equation.'}},
  'bac': {howToSteps:['Enter alcohol grams, body weight, elapsed hours, and sex.','Check units and assumptions.','Review the Widmark-style estimate.','Never use the result to decide whether to drive.'],formula:{expression:'BAC estimate = max(0, alcohol grams ÷ (body weight grams × r) × 100 − 0.015 × hours)',variables:[{symbol:'A',definition:'Alcohol consumed in grams'},{symbol:'W',definition:'Body weight in grams'},{symbol:'r',definition:'Distribution factor, 0.68 male or 0.55 female'}],notes:'This is an uncertain educational estimate and not a forensic measurement.'},examples:[{title:'Widmark-style estimate',scenario:'Enter grams of ethanol, weight, elapsed hours, and sex.',steps:['Convert weight to grams.','Apply the distribution factor.','Subtract assumed elimination.','Review the estimate with caution.'],result:'Peak and elapsed-time estimates are shown.'}],useCases:['Education about assumptions in BAC models.'],commonPitfalls:['Confusing beverage volume with ethanol grams.','Ignoring absorption variability.','Using any estimate to justify driving.'],glossary:[{term:'BAC',definition:'Blood alcohol concentration expressed as a percentage.'}],faqs:[{question:'Can this determine legal impairment?',answer:'No.'},{question:'Should I drive after a low result?',answer:'Never use a calculator to make that decision.'}],sources:[{title:'Alcohol Calculations and Their Uncertainty',publisher:'NIH/PMC',url:'https://pmc.ncbi.nlm.nih.gov/articles/PMC4361698/',year:2015}],author:{name:'CalculatorFree Health Review Team',credentials:'Calculation Review',description:'Safety-oriented educational model.'}},

  'scientific': {howToSteps:['Enter one numeric value x.','Remember that sine and cosine use radians, not degrees.','Review x², sin(x), cos(x), and ln(x) in the live results.','Check the domain note before interpreting the natural-log result.'],formula:{expression:'x² = x × x; sin(x) = sine of x in radians; cos(x) = cosine of x in radians; ln(x) = natural logarithm for x > 0.',variables:[{symbol:'x',definition:'The single numeric input'},{symbol:'e',definition:'Euler’s number, the base of the natural logarithm'}],notes:'For x ≤ 0, the client-side model returns a guarded ln result of 0 rather than presenting an undefined numeric output. This is a display safeguard, not a valid natural-log value.'},examples:[{title:'Evaluate common functions at x = 2',scenario:'Enter 2 in the Value x field.',steps:['Square the input: 2² = 4.','Evaluate sin(2) and cos(2) in radians.','Evaluate ln(2) ≈ 0.6931.','Compare the displayed values with the domain and unit assumptions.'],result:'The calculator shows x² = 4.0000, sin(x) ≈ 0.9093, cos(x) ≈ -0.4161, and ln(x) ≈ 0.6931.'},{title:'Check a non-positive logarithm input',scenario:'Enter 0 or a negative value.',steps:['Read the square, sine, and cosine outputs normally.','Notice that the natural logarithm is undefined mathematically for x ≤ 0.','Use the displayed 0 as the model’s guarded output, not as ln(x).','Switch to a positive value for a meaningful ln calculation.'],result:'The page keeps the UI finite and explains the domain limitation.'}],useCases:['Quick checks of elementary scientific functions.','Classroom practice with radians, powers, and natural logarithms.','Reference values before using a symbolic or engineering system.'],commonPitfalls:['Entering degrees while interpreting the sine or cosine output as degrees.','Treating the guarded ln(x) value for x ≤ 0 as a mathematical logarithm.','Expecting symbolic simplification, complex roots, or unit conversion from this numeric tool.','Relying on rounded display values for precision-critical work.'],glossary:[{term:'Radian',definition:'The angular unit used by the JavaScript sine and cosine functions in this route.'},{term:'Natural logarithm',definition:'The logarithm with base e, defined for positive real inputs.'},{term:'Domain',definition:'The set of input values for which a mathematical operation is defined.'}],faqs:[{question:'Are sine and cosine calculated in degrees?',answer:'No. They use radians, consistent with JavaScript Math.sin and Math.cos.'},{question:'What happens when x is zero or negative?',answer:'The square, sine, and cosine remain numeric, while the model shows a guarded ln result of 0 because the real natural logarithm is undefined for x ≤ 0.'},{question:'Does the calculator solve symbolic expressions?',answer:'No. It evaluates four numeric functions for one entered value; it is not a computer algebra system.'},{question:'Are results calculated on a server?',answer:'No. The widget runs client-side in the browser.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026},{title:'Math.sin() — JavaScript reference',publisher:'MDN Web Docs',url:'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/sin',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side numeric implementation with route-specific assumptions stated openly.'}},
  'fraction': {howToSteps:['Enter the labeled values.','Confirm units and signs.','Review the live result.','Use the result for educational or planning purposes.'],formula:{expression:'a/b + c/d = (ad + cb)/(bd), then reduce by the greatest common divisor.',variables:[{symbol:'a,b,c,d',definition:'Fraction numerators and denominators'}]},examples:[{title:'Fraction addition',scenario:'Enter two fractions.',steps:['Cross multiply.','Add numerators.','Multiply denominators.','Reduce by GCD.'],result:'The reduced fraction is shown.'}],useCases:['Arithmetic education.'],commonPitfalls:['Using zero denominator.','Forgetting reduction.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Can denominator be zero?',answer:'No; the model returns a guarded zero denominator.'},{question:'Are results exact?',answer:'Yes for integer inputs.'}],sources:[{title:'NIST DLMF',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'standard-deviation': {howToSteps:['Enter the labeled values.','Confirm units and signs.','Review the live result.','Use the result for educational or planning purposes.'],formula:{expression:'Population SD = sqrt(Σ(x − mean)² / N).',variables:[{symbol:'x',definition:'Each entered observation'},{symbol:'N',definition:'Number of observations'}]},examples:[{title:'Four observations',scenario:'Enter four values.',steps:['Calculate mean.','Calculate squared deviations.','Average squared deviations.','Take the square root.'],result:'Population standard deviation and mean are shown.'}],useCases:['Describe population dispersion.'],commonPitfalls:['Confusing population and sample SD.'],glossary:[{term:'Mean',definition:'Arithmetic average.'}],faqs:[{question:'Is this sample SD?',answer:'No; this route displays population SD.'},{question:'Are non-finite values accepted?',answer:'They are sanitized.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'ratio': {howToSteps:['Enter the labeled values.','Confirm units and signs.','Review the live result.','Use the result for educational or planning purposes.'],formula:{expression:'Simplified ratio = a/gcd(a,b) : b/gcd(a,b).',variables:[{symbol:'a,b',definition:'Two non-negative ratio terms'}]},examples:[{title:'Ratio simplification',scenario:'Enter 10 and 20.',steps:['Find GCD.','Divide both terms.','Display simplified ratio.','Display decimal ratio.'],result:'The result is 1:2.'}],useCases:['Compare quantities.'],commonPitfalls:['Using incompatible units.','Interpreting a ratio without context.'],glossary:[{term:'Ratio',definition:'Relationship between two quantities.'}],faqs:[{question:'Can the second term be zero?',answer:'The decimal ratio is guarded.'},{question:'Are units converted?',answer:'No.'}],sources:[{title:'NIST DLMF',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'quadratic-formula': {howToSteps:['Enter the labeled values.','Confirm units and signs.','Review the live result.','Use the result for educational or planning purposes.'],formula:{expression:'x = (−b ± sqrt(b² − 4ac)) ÷ (2a).',variables:[{symbol:'a,b,c',definition:'Quadratic coefficients'},{symbol:'Δ',definition:'Discriminant b² − 4ac'}]},examples:[{title:'Two real roots',scenario:'Use x² − 3x + 2 = 0.',steps:['Enter 1, -3, 2.','Compute discriminant.','Take the square root.','Apply plus/minus branches.'],result:'Roots 1 and 2 are shown.'}],useCases:['Solve quadratic equations.'],commonPitfalls:['Using a=0.','Assuming negative discriminants have real roots.'],glossary:[{term:'Discriminant',definition:'Value determining the number of real roots.'}],faqs:[{question:'What if the discriminant is negative?',answer:'The route reports no real roots.'},{question:'Are complex roots shown?',answer:'No; this simple route shows real roots only.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},

  'exponent': {howToSteps:['Enter the labeled values.','Confirm domain and units.','Review the live result.','Use it as a mathematical estimate.'],formula:{expression:'x^n',variables:[{symbol:'x',definition:'Primary entered value'},{symbol:'n/b/d',definition:'Secondary entered parameter'}],notes:'Power = x^n.'},examples:[{title:'Basic example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the stated formula.','Check the domain.','Review the result.'],result:'The result updates client-side.'}],useCases:['Mathematics education.','Quick numerical checks.'],commonPitfalls:['Using invalid domains.','Confusing percentages and decimals.'],glossary:[{term:'Domain',definition:'Inputs for which a function is defined.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'root': {howToSteps:['Enter the labeled values.','Confirm domain and units.','Review the live result.','Use it as a mathematical estimate.'],formula:{expression:'x^(1/n)',variables:[{symbol:'x',definition:'Primary entered value'},{symbol:'n/b/d',definition:'Secondary entered parameter'}],notes:'Root = x^(1/n).'},examples:[{title:'Basic example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the stated formula.','Check the domain.','Review the result.'],result:'The result updates client-side.'}],useCases:['Mathematics education.','Quick numerical checks.'],commonPitfalls:['Using invalid domains.','Confusing percentages and decimals.'],glossary:[{term:'Domain',definition:'Inputs for which a function is defined.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'log': {howToSteps:['Enter the labeled values.','Confirm domain and units.','Review the live result.','Use it as a mathematical estimate.'],formula:{expression:'log_b(x) = ln(x) ÷ ln(b)',variables:[{symbol:'x',definition:'Primary entered value'},{symbol:'n/b/d',definition:'Secondary entered parameter'}],notes:'Domain requires x > 0 and b > 0, b ≠ 1.'},examples:[{title:'Basic example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the stated formula.','Check the domain.','Review the result.'],result:'The result updates client-side.'}],useCases:['Mathematics education.','Quick numerical checks.'],commonPitfalls:['Using invalid domains.','Confusing percentages and decimals.'],glossary:[{term:'Domain',definition:'Inputs for which a function is defined.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'percent-error': {howToSteps:['Enter the labeled values.','Confirm domain and units.','Review the live result.','Use it as a mathematical estimate.'],formula:{expression:'|experimental − theoretical| ÷ |theoretical| × 100',variables:[{symbol:'x',definition:'Primary entered value'},{symbol:'n/b/d',definition:'Secondary entered parameter'}],notes:'Zero theoretical value returns 0.'},examples:[{title:'Basic example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the stated formula.','Check the domain.','Review the result.'],result:'The result updates client-side.'}],useCases:['Mathematics education.','Quick numerical checks.'],commonPitfalls:['Using invalid domains.','Confusing percentages and decimals.'],glossary:[{term:'Domain',definition:'Inputs for which a function is defined.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'rounding': {howToSteps:['Enter the labeled values.','Confirm domain and units.','Review the live result.','Use it as a mathematical estimate.'],formula:{expression:'round(x, d)',variables:[{symbol:'x',definition:'Primary entered value'},{symbol:'n/b/d',definition:'Secondary entered parameter'}],notes:'Decimal places are bounded from 0 to 15.'},examples:[{title:'Basic example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the stated formula.','Check the domain.','Review the result.'],result:'The result updates client-side.'}],useCases:['Mathematics education.','Quick numerical checks.'],commonPitfalls:['Using invalid domains.','Confusing percentages and decimals.'],glossary:[{term:'Domain',definition:'Inputs for which a function is defined.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},

  'triangle': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Triangle area = base × height ÷ 2; hypotenuse = √(base² + height²).',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'volume': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Rectangular volume = length × width × height.',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'number-sequence': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Term n = first + (n−1) × difference; sum = n(2first + (n−1)d) ÷ 2.',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'binary': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Binary representation is the base-2 expansion of an integer.',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'hex': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Hexadecimal representation is the base-16 expansion of an integer.',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'half-life': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Remaining = initial × (1/2)^(elapsed ÷ half-life).',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'slope': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the live result.','Use the result as a mathematical estimate.'],formula:{expression:'Slope = (y₂−y₁) ÷ (x₂−x₁); intercept = y₁ − slope×x₁.',variables:[{symbol:'x',definition:'Entered geometric or numeric value'}],notes:'Inputs are sanitized and division-by-zero cases are guarded.'},examples:[{title:'Worked example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check units.','Review result.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick planning calculations.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'Unit',definition:'Measurement scale used for an input.'}],faqs:[{question:'Are results client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the displayed formula and may be rounded for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},

  'area': {howToSteps:['Enter the radius in your chosen unit.','Review the circle area and sphere surface area outputs.','Keep the same unit for both results.','Use the values as geometry estimates.'],formula:{expression:'Circle area = πr²; sphere surface area = 4πr².',variables:[{symbol:'r',definition:'Circle or sphere radius in the selected unit'},{symbol:'π',definition:'The circle constant, approximately 3.14159'}],notes:'Both outputs use the same radius; the surface result describes a sphere, not a flat circle.'},examples:[{title:'Radius of 10 units',scenario:'A circle or sphere has a radius of 10 units.',steps:['Square the radius: 10² = 100.','Multiply by π for circle area: approximately 314.1593 square units.','Multiply by 4π for sphere surface area: approximately 1,256.6371 square units.','Keep square units in the final interpretation.'],result:'Circle area is approximately 314.1593 square units and sphere surface area is approximately 1,256.6371 square units.'}],useCases:['Check geometry homework or design dimensions.','Estimate a circular footprint.','Compare the surface exposure of spherical objects.'],commonPitfalls:['Entering diameter when the field asks for radius.','Mixing linear and square units.','Treating sphere surface area as circle area.'],glossary:[{term:'Radius',definition:'The distance from a circle or sphere center to its boundary.'},{term:'Circle area',definition:'The flat region enclosed by a circle.'},{term:'Sphere surface area',definition:'The area covering the outside of a sphere.'}],faqs:[{question:'Does the tool use diameter?',answer:'No. Enter the radius; divide a known diameter by two first.'},{question:'What units are returned?',answer:'The area outputs are in square units derived from the radius unit.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/3.5',year:2026}],author:{name:'CalculatorFree Geometry Review Team',credentials:'Calculation Review',description:'Original client-side circle and sphere geometry model.'}},
  'distance': {howToSteps:['Enter the X and Y coordinates for the first point.','Enter the X and Y coordinates for the second point.','Review the straight-line distance.','Keep coordinate units consistent.'],formula:{expression:'Distance = √((x₂−x₁)² + (y₂−y₁)²).',variables:[{symbol:'x₁,y₁',definition:'Coordinates of the first point'},{symbol:'x₂,y₂',definition:'Coordinates of the second point'}],notes:'The result is the Euclidean distance in the same unit as the coordinates.'},examples:[{title:'Distance between two points',scenario:'Compare (0,0) with (3,4).',steps:['Find the horizontal difference: 3−0 = 3.','Find the vertical difference: 4−0 = 4.','Apply the theorem: √(3²+4²) = √25.','Take the square root to obtain 5 units.'],result:'The distance between (0,0) and (3,4) is 5 coordinate units.'}],useCases:['Measure a segment on a coordinate plane.','Check map or layout coordinates.','Verify geometry and vector examples.'],commonPitfalls:['Mixing coordinate units or scales.','Subtracting only one coordinate.','Using Manhattan distance when straight-line distance is intended.'],glossary:[{term:'Euclidean distance',definition:'The shortest straight-line distance between two points.'},{term:'Coordinate pair',definition:'An ordered X and Y position on a plane.'}],faqs:[{question:'Can coordinates be negative?',answer:'Yes. Negative coordinates are valid positions and the squared differences keep the distance non-negative.'},{question:'Does it calculate 3D distance?',answer:'No. This route uses two-dimensional X and Y coordinates.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/1.2',year:2026}],author:{name:'CalculatorFree Geometry Review Team',credentials:'Calculation Review',description:'Original client-side two-dimensional distance model.'}},
  'sample-size': {howToSteps:['Enter the confidence z-score.','Enter the expected population proportion as a percent.','Enter the desired margin of error as a percent.','Review the rounded-up sample-size estimate.'],formula:{expression:'n = z²p(1−p) ÷ e², rounded up.',variables:[{symbol:'z',definition:'Critical z-score for the chosen confidence level'},{symbol:'p',definition:'Expected population proportion as a decimal'},{symbol:'e',definition:'Margin of error as a decimal'}],notes:'The interface accepts p and e as percentages and converts them to decimals before applying the formula; a zero margin returns a guarded zero.'},examples:[{title:'95% confidence planning estimate',scenario:'Use z = 1.96, p = 50%, and a 5% margin of error.',steps:['Convert 50% to p = 0.50 and 5% to e = 0.05.','Square the z-score: 1.96² = 3.8416.','Apply n = 3.8416 × 0.50 × 0.50 ÷ 0.05².','Round up to obtain 385 observations.'],result:'The estimated minimum sample size is 385 observations before nonresponse or design adjustments.'}],useCases:['Plan a basic proportion survey.','Explain how confidence and precision affect sample size.','Compare sample requirements under different assumptions.'],commonPitfalls:['Entering 0.05 when the field already expects 5%.','Using a z-score that does not match the confidence level.','Treating the estimate as a substitute for a complex survey design.'],glossary:[{term:'Margin of error',definition:'The maximum expected difference between an estimate and the true population value under the stated assumptions.'},{term:'Proportion',definition:'The expected share of the population with the measured characteristic.'}],faqs:[{question:'Why is the result rounded up?',answer:'A sample cannot contain a fraction of an observation, so the estimate is rounded upward.'},{question:'Does this include finite-population correction?',answer:'No. The route uses the simple proportion estimate shown in its formula.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Statistics Review Team',credentials:'Calculation Review',description:'Original client-side proportion sample-size estimate.'}},
  'probability': {howToSteps:['Enter the number of successful outcomes k.','Enter the total number of trials n.','Enter the success probability as a percent.','Review the exact probability for k successes.'],formula:{expression:'P(X=k) = C(n,k)p^k(1−p)^(n−k).',variables:[{symbol:'n',definition:'Total number of independent trials'},{symbol:'k',definition:'Exact number of successes'},{symbol:'p',definition:'Success probability as a decimal'}],notes:'The interface accepts p as a percentage and guards cases where k is outside 0 through n.'},examples:[{title:'Exactly two successes',scenario:'Use k = 2, n = 4, and p = 50%.',steps:['Convert 50% to p = 0.5.','Count combinations: C(4,2) = 6.','Multiply 6 × 0.5² × 0.5².','Obtain 0.375, or 37.5%.'],result:'The probability of exactly two successes is 37.5%.'}],useCases:['Teach binomial probability.','Estimate an exact outcome count in repeated independent trials.','Compare probabilities under different success rates.'],commonPitfalls:['Confusing exactly k with at least k.','Entering a percentage as a decimal percentage twice.','Using the model when trials are dependent.'],glossary:[{term:'Binomial trial',definition:'A trial with two possible outcomes and a fixed success probability.'},{term:'Combination',definition:'A selection of items where order does not matter.'}],faqs:[{question:'Does this calculate at least k successes?',answer:'No. It calculates exactly k successes for the inputs shown.'},{question:'What if k is greater than n?',answer:'The guarded result is zero because that outcome is impossible.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Probability Review Team',credentials:'Calculation Review',description:'Original client-side binomial probability model.'}},
  'statistics': {howToSteps:['Enter up to four observations in the displayed fields.','Review the mean and median for central tendency.','Review the mode and range for distribution and spread.','Interpret the summary with the context of the dataset.'],formula:{expression:'Mean = sum of observations ÷ count; median = middle ordered value; mode = most frequent value; range = maximum − minimum.',variables:[{symbol:'xᵢ',definition:'An entered observation'},{symbol:'n',definition:'Number of valid observations'}],notes:'The current route summarizes four displayed observations and ignores non-finite values in the production logic.'},examples:[{title:'Four observations',scenario:'Use 1, 2, 2, and 4.',steps:['Add the observations: 1+2+2+4 = 9.','Divide by four for mean = 2.25.','Order the values and take the middle pair for median = 2.','The most frequent value is 2 and the range is 4−1 = 3.'],result:'Mean = 2.25, median = 2, mode = 2, and range = 3.'}],useCases:['Summarize a small set of measurements.','Check basic descriptive statistics.','Explain how central tendency differs from spread.'],commonPitfalls:['Calling the mean the median.','Ignoring duplicate values when finding the mode.','Comparing ranges from datasets with different units.'],glossary:[{term:'Mean',definition:'The arithmetic average of the observations.'},{term:'Median',definition:'The central ordered value, or the midpoint of the two central values.'},{term:'Mode',definition:'The value that appears most often.'},{term:'Range',definition:'The difference between the largest and smallest observations.'}],faqs:[{question:'Is the standard deviation shown?',answer:'No. This route reports mean, median, mode, and range.'},{question:'What if two values tie for mode?',answer:'The production summary selects the first value reaching the highest frequency.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Statistics Review Team',credentials:'Calculation Review',description:'Original client-side descriptive-statistics summary.'}},
  'permutation-and-combination': {howToSteps:['Enter the population size n.','Enter the number of selected items r.','Review the ordered-selection result.','Confirm that r is not greater than n.'],formula:{expression:'P(n,r) = n! ÷ (n−r)!.',variables:[{symbol:'n',definition:'Total items available'},{symbol:'r',definition:'Items selected in order'}],notes:'This route reports permutations, where order matters; invalid r values return a guarded zero.'},examples:[{title:'Arrange three items from five',scenario:'Use n = 5 and r = 3.',steps:['Compute 5! = 120.','Compute (5−3)! = 2! = 2.','Divide 120 by 2.','Obtain 60 ordered arrangements.'],result:'There are 60 permutations when selecting and ordering three items from five.'}],useCases:['Count ordered schedules or assignments.','Compare ordered and unordered selection problems.','Support introductory combinatorics exercises.'],commonPitfalls:['Using combinations when order matters.','Entering r greater than n.','Forgetting that factorials grow very quickly.'],glossary:[{term:'Permutation',definition:'An ordered arrangement of selected items.'},{term:'Factorial',definition:'The product of all positive integers from one through a given integer.'},{term:'Combination',definition:'An unordered selection of items.'}],faqs:[{question:'Does this calculate combinations too?',answer:'No. It calculates P(n,r), where order matters.'},{question:'What happens when r is zero?',answer:'There is one empty ordered selection mathematically; the production factorial formula returns 1.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/5.2',year:2026}],author:{name:'CalculatorFree Combinatorics Review Team',credentials:'Calculation Review',description:'Original client-side permutation model.'}},

  'factorial': {howToSteps:['Enter a non-negative whole number n.','Review n factorial, written n!.','Keep in mind that factorial values grow rapidly.','Use the result for exact small-integer counting exercises.'],formula:{expression:'n! = n × (n−1) × … × 2 × 1, with 0! = 1.',variables:[{symbol:'n',definition:'A non-negative whole number'},{symbol:'!',definition:'The factorial operation'}],notes:'The production guard returns zero for negative, non-finite, or values above the safe implementation limit.'},examples:[{title:'Five factorial',scenario:'Use n = 5.',steps:['Start with 5.','Multiply 5 × 4 × 3 × 2 × 1.','Obtain 120.','Interpret the result as the number of ordered arrangements of five distinct items.'],result:'5! = 120.'}],useCases:['Support combinatorics lessons.','Count arrangements of a small set.','Verify factorial terms in probability formulas.'],commonPitfalls:['Entering a negative value.','Confusing factorial with exponentiation.','Expecting factorial values to remain small for large n.'],glossary:[{term:'Factorial',definition:'The product of all positive integers up to a non-negative integer.'},{term:'Zero factorial',definition:'By convention, 0! equals 1.'}],faqs:[{question:'What is 0 factorial?',answer:'The defined value is 0! = 1.'},{question:'Can it handle decimal inputs?',answer:'The interface uses whole-number factorials and floors entered values.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/5.2',year:2026}],author:{name:'CalculatorFree Combinatorics Review Team',credentials:'Calculation Review',description:'Original client-side factorial model.'}},
  'combination': {howToSteps:['Enter the total population n.','Enter the selected count r.','Review the unordered selection count.','Confirm that r is between zero and n.'],formula:{expression:'C(n,r) = n! ÷ (r!(n−r)!).',variables:[{symbol:'n',definition:'Total items available'},{symbol:'r',definition:'Items selected without regard to order'}],notes:'The implementation uses a multiplicative form to reduce unnecessary intermediate factorial growth.'},examples:[{title:'Choose three from ten',scenario:'Use n = 10 and r = 3.',steps:['Identify the ten available items.','Select three positions without ordering them.','Apply C(10,3) = 10! ÷ (3!7!).','Obtain 120 combinations.'],result:'There are 120 unordered selections.'}],useCases:['Count groups where order does not matter.','Support probability and sampling exercises.','Compare combinations with permutations.'],commonPitfalls:['Using permutations when order is irrelevant.','Entering r greater than n.','Treating repeated items as distinct without checking the model.'],glossary:[{term:'Combination',definition:'An unordered selection from a larger set.'},{term:'Binomial coefficient',definition:'The value C(n,r) used in the binomial theorem and counting.'}],faqs:[{question:'What is the difference from a permutation?',answer:'A combination ignores order, while a permutation counts different orders separately.'},{question:'What happens when r is zero?',answer:'There is one empty selection, so C(n,0) = 1.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/5.2',year:2026}],author:{name:'CalculatorFree Combinatorics Review Team',credentials:'Calculation Review',description:'Original client-side combination model.'}},
  'percentile': {howToSteps:['Enter the observations in the four fields.','Review the ordered dataset used by the calculator.','Read the displayed 50th percentile, which is the median.','Keep the observations in consistent units.'],formula:{expression:'For percentile p, interpolate at position (n−1)p; this route reports p = 50%.',variables:[{symbol:'xᵢ',definition:'An entered observation'},{symbol:'p',definition:'The percentile fraction, fixed here at 0.50'}],notes:'The route uses linear interpolation between sorted observations and filters non-finite values.'},examples:[{title:'Median of four values',scenario:'Use 1, 2, 3, and 4.',steps:['Sort the values as 1, 2, 3, 4.','Place the 50th percentile halfway between the second and third positions.','Interpolate between 2 and 3.','Obtain 2.5.'],result:'The displayed 50th percentile is 2.5.'}],useCases:['Summarize the midpoint of a small sample.','Explain percentile interpolation.','Check basic descriptive-statistics work.'],commonPitfalls:['Confusing percentile with percentage.','Mixing units across observations.','Assuming the 50th percentile equals the average in every dataset.'],glossary:[{term:'Percentile',definition:'A position in an ordered dataset below which a given share of observations falls.'},{term:'Median',definition:'The 50th percentile under the interpolation convention used here.'}],faqs:[{question:'Which percentile does this route calculate?',answer:'It reports the 50th percentile, or median, for the four displayed observations.'},{question:'Can it calculate the 90th percentile?',answer:'Not in this initial route; the production UI is intentionally fixed to the 50th percentile.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Statistics Review Team',credentials:'Calculation Review',description:'Original client-side percentile interpolation model.'}},
  'average-rating': {howToSteps:['Enter the count of one-star through five-star votes.','Review the weighted average rating.','Compare the result with the total vote count.','Use the estimate as a descriptive summary, not a quality guarantee.'],formula:{expression:'Average rating = (1×v₁ + 2×v₂ + 3×v₃ + 4×v₄ + 5×v₅) ÷ (v₁+v₂+v₃+v₄+v₅).',variables:[{symbol:'v₁…v₅',definition:'Number of votes at each star level'},{symbol:'1…5',definition:'Star value assigned to each rating level'}],notes:'Zero total votes produces a guarded zero; negative vote counts are clamped out of the calculation.'},examples:[{title:'Weighted five-star average',scenario:'Use 12 one-star, 18 two-star, 25 three-star, 30 four-star, and 15 five-star votes.',steps:['Add all votes to obtain 100 total ratings.','Multiply each star value by its vote count.','Add the weighted scores.','Divide the weighted score by 100.'],result:'The result is the vote-weighted average rating for the displayed distribution.'}],useCases:['Summarize survey star ratings.','Compare rating distributions using one number.','Explain why vote counts matter to an average.'],commonPitfalls:['Ignoring the number of votes behind a rating.','Treating the average as a fraud or quality detector.','Entering a score instead of a vote count.'],glossary:[{term:'Weighted average',definition:'An average where each value contributes according to its associated weight.'},{term:'Vote count',definition:'The number of ratings assigned to a given star level.'}],faqs:[{question:'What if there are no votes?',answer:'The guarded result is zero because no weighted average can be formed.'},{question:'Does this show review confidence?',answer:'No. It summarizes the entered distribution only.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Statistics Review Team',credentials:'Calculation Review',description:'Original client-side weighted-rating model.'}},
  'least-to-greatest': {howToSteps:['Enter the observations in the displayed fields.','Review the values sorted from smallest to largest.','Keep all observations in comparable units.','Use the ordered list for further descriptive analysis.'],formula:{expression:'Sort the finite observations in ascending numerical order.',variables:[{symbol:'xᵢ',definition:'An entered observation'}],notes:'The implementation filters non-finite values before sorting and preserves duplicate observations.'},examples:[{title:'Order four observations',scenario:'Use 4, 1, 3, and 2.',steps:['Read the four entered values.','Compare each value numerically.','Place 1 first and 4 last.','Return the sequence 1, 2, 3, 4.'],result:'The observations are ordered as 1, 2, 3, 4.'}],useCases:['Prepare data before finding a median.','Check ranking and ordering work.','Spot duplicate or outlying values in a small set.'],commonPitfalls:['Sorting text alphabetically instead of numbers.','Dropping duplicate observations.','Mixing units or scales.'],glossary:[{term:'Ascending order',definition:'A sequence arranged from the smallest value to the largest.'},{term:'Observation',definition:'One measured or recorded value in a dataset.'}],faqs:[{question:'Are duplicate values retained?',answer:'Yes. The ordered output retains every finite displayed observation.'},{question:'Does this calculate a median?',answer:'No. It only orders the values; use a statistics or percentile route for summaries.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Statistics Review Team',credentials:'Calculation Review',description:'Original client-side ascending-sort model.'}},
  'percentile-rank': {howToSteps:['Enter the value whose rank you want to estimate.','Enter the comparison observations.','Review the percentage at or below the selected value.','Interpret the result within the entered sample only.'],formula:{expression:'Percentile rank = count(values ≤ selected value) ÷ count(values) × 100.',variables:[{symbol:'x',definition:'Selected value to rank'},{symbol:'S',definition:'Comparison observations'}],notes:'The route uses an inclusive empirical rank and returns zero for an empty comparison set.'},examples:[{title:'Rank a value in a sample',scenario:'Rank 25 against comparison values 10, 20, 30, and 40.',steps:['Count comparison values at or below 25: two values.','Count all comparison values: four.','Divide 2 by 4.','Multiply by 100 for a 50% rank.'],result:'The selected value has an inclusive empirical percentile rank of 50% in the comparison sample.'}],useCases:['Describe a score relative to a small sample.','Compare a measurement with reference observations.','Explain empirical percentile concepts.'],commonPitfalls:['Confusing percentile rank with a standardized z-score.','Using a sample that is not representative.','Treating the result as a population probability.'],glossary:[{term:'Percentile rank',definition:'The percentage of comparison observations at or below a selected value.'},{term:'Reference sample',definition:'The observations used to place the selected value in context.'}],faqs:[{question:'Are equal values counted?',answer:'Yes. This route uses an inclusive less-than-or-equal comparison.'},{question:'Does this use a normal distribution?',answer:'No. It uses the empirical comparison sample displayed on the page.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Statistics Review Team',credentials:'Calculation Review',description:'Original client-side empirical percentile-rank model.'}},
  'pressure-unit-conversion': {howToSteps:['Enter a pressure value.','Choose bar, psi, atm, or pascals as the input unit.','Review the converted values in the other units.','Keep unit labels with any copied result.'],formula:{expression:'Convert through a pascal base using unit-specific scale factors.',variables:[{symbol:'P',definition:'Entered pressure value'},{symbol:'Pa',definition:'Pascal base unit'}],notes:'The tool uses 1 bar = 100,000 Pa, 1 atm = 101,325 Pa, and the standard psi-to-pascal factor.'},examples:[{title:'Convert 50 bar',scenario:'Enter 50 with bar selected.',steps:['Multiply 50 by 100,000 to obtain pascals.','Divide by the target-unit factors.','Read the psi, atm, and Pa outputs.','Keep the selected input unit distinct from outputs.'],result:'The converter returns equivalent values in Pa, bar, psi, and atm.'}],useCases:['Compare engineering pressure units.','Check vehicle or weather pressure values.','Prepare consistent units for a physics calculation.'],commonPitfalls:['Confusing gauge and absolute pressure.','Mixing pressure with force or energy units.','Rounding before conversion.'],glossary:[{term:'Pascal',definition:'The SI unit of pressure, equal to one newton per square metre.'},{term:'Absolute pressure',definition:'Pressure measured relative to a perfect vacuum.'}],faqs:[{question:'Does the tool convert gauge pressure?',answer:'No. It performs unit conversion only and does not add atmospheric-pressure corrections.'},{question:'Are the values rounded?',answer:'Displayed values are rounded for readability while calculations retain numeric precision.'}],sources:[{title:'BIPM SI Brochure',publisher:'BIPM',url:'https://www.bipm.org/en/publications/si-brochure',year:2026}],author:{name:'CalculatorFree Unit Conversion Review Team',credentials:'Calculation Review',description:'Original client-side pressure conversion model.'}},
  'salary-to-hourly': {howToSteps:['Enter annual salary.','Enter hours worked each week.','Set weeks worked per year if needed.','Review hourly, weekly, and monthly equivalents.'],formula:{expression:'Hourly wage = annual salary ÷ (weeks per year × hours per week); monthly equivalent = annual salary ÷ 12.',variables:[{symbol:'A',definition:'Annual salary'},{symbol:'h',definition:'Hours worked per week'},{symbol:'w',definition:'Weeks worked per year'}],notes:'The educational estimate uses 52 weeks per year by default and does not model tax, benefits, overtime, or unpaid leave.'},examples:[{title:'Annual salary conversion',scenario:'Use $50,000, 40 hours per week, and 52 weeks.',steps:['Multiply 52 weeks by 40 hours for 2,080 annual hours.','Divide $50,000 by 2,080.','Compute weekly and monthly equivalents.','Interpret the result as a gross-pay estimate.'],result:'The hourly estimate is approximately $24.04 before taxes and other compensation adjustments.'}],useCases:['Compare salary and hourly job offers.','Build a basic gross-pay budget.','Convert pay periods using consistent assumptions.'],commonPitfalls:['Ignoring unpaid time off.','Treating gross pay as take-home pay.','Using the wrong number of paid weeks or hours.'],glossary:[{term:'Gross pay',definition:'Compensation before taxes and deductions.'},{term:'Hourly equivalent',definition:'Annual compensation divided by assumed annual work hours.'}],faqs:[{question:'Does this calculate take-home pay?',answer:'No. It converts gross compensation and does not apply taxes or deductions.'},{question:'Can the assumptions be changed?',answer:'Yes. The hours and weeks fields are editable.'}],sources:[{title:'U.S. Bureau of Labor Statistics — Occupational Employment and Wage Statistics',publisher:'BLS',url:'https://www.bls.gov/oes/',year:2026}],author:{name:'CalculatorFree Compensation Review Team',credentials:'Calculation Review',description:'Original client-side gross-pay conversion model.'}},
  'discount-rate': {howToSteps:['Enter present value and future value.','Enter the number of periods.','Choose the compounding frequency.','Review the resulting periodic rate.'],formula:{expression:'r = (FV ÷ PV)^(1/(n×m)) − 1.',variables:[{symbol:'PV',definition:'Present value'},{symbol:'FV',definition:'Future value'},{symbol:'n',definition:'Number of periods'},{symbol:'m',definition:'Compounding periods per period'}],notes:'The guard returns zero when PV, period count, or future-value inputs cannot form a valid rate.'},examples:[{title:'Growth from present to future value',scenario:'Use PV = 1,000, FV = 1,500, n = 5, and annual frequency 1.',steps:['Divide future value by present value.','Take the fifth root of the ratio.','Subtract one.','Express the result as a percentage.'],result:'The calculator returns the periodic rate implied by the two values and assumptions.'}],useCases:['Explain discounting and compounding.','Compare present and future cash values.','Support preliminary DCF education.'],commonPitfalls:['Confusing discount rate with a loan APR.','Mixing annual and periodic frequencies.','Ignoring cash flows between endpoints.'],glossary:[{term:'Present value',definition:'Value measured at the starting point of the analysis.'},{term:'Future value',definition:'Value measured at the ending point of the analysis.'}],faqs:[{question:'Can the rate be negative?',answer:'Yes, when the future value is below the present value under valid inputs.'},{question:'Does this include periodic cash flows?',answer:'This initial route uses the two-value formula and does not model additional cash flows.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/4.2',year:2026}],author:{name:'CalculatorFree Finance Review Team',credentials:'Calculation Review',description:'Original client-side two-value discount-rate model.'}},
  'linear-combination': {howToSteps:['Enter coefficients for the first equation.','Enter coefficients for the second equation.','Review x and y when the system has one solution.','Read the guard message for dependent or inconsistent systems.'],formula:{expression:'For a₁x+b₁y=c₁ and a₂x+b₂y=c₂, solve using determinant D=a₁b₂−a₂b₁.',variables:[{symbol:'a₁,b₁,c₁',definition:'Coefficients of the first equation'},{symbol:'a₂,b₂,c₂',definition:'Coefficients of the second equation'}],notes:'A zero determinant is classified as infinitely many solutions when equations are consistent, otherwise no solution.'},examples:[{title:'Two-equation system',scenario:'Use x−4y=1 and −2x+4y=2.',steps:['Enter all six coefficients.','Calculate the determinant.','Use elimination or the equivalent determinant formulas.','Check x and y in both equations.'],result:'The result card reports the unique solution or the appropriate system classification.'}],useCases:['Practice elimination of two linear equations.','Check algebra homework.','Identify dependent and inconsistent systems.'],commonPitfalls:['Swapping a coefficient with the constant.','Dividing by a zero determinant.','Failing to verify the solution in both equations.'],glossary:[{term:'Determinant',definition:'A scalar that indicates whether a two-by-two linear system has a unique solution.'},{term:'Elimination',definition:'Combining equations to remove one variable.'}],faqs:[{question:'What does zero determinant mean?',answer:'It means the system is not uniquely solvable; it may have no solution or infinitely many.'},{question:'Are intermediate elimination steps shown?',answer:'The current route reports the classification or solution; it does not reproduce another publisher’s step-by-step layout.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/1.13',year:2026}],author:{name:'CalculatorFree Algebra Review Team',credentials:'Calculation Review',description:'Original client-side two-variable linear-system model.'}},
  'circle': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'Area = πr² and circumference = 2πr.',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'surface-area': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'Rectangular surface area = 2(lw + lh + wh).',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'pythagorean-theorem': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'c = √(a² + b²).',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'right-triangle': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'Missing leg = √(hypotenuse² − known leg²).',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'least-common-multiple': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'LCM(a,b) = |ab| ÷ GCD(a,b).',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'greatest-common-factor': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'GCD is found by Euclid’s algorithm.',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'prime-factorization': {howToSteps:['Enter the labeled values.','Confirm units and domain.','Review the result.','Use it for math education or planning.'],formula:{expression:'Repeatedly divide by the smallest prime factor.',variables:[{symbol:'x',definition:'Entered geometric or integer value'}],notes:'Non-finite and invalid values are guarded.'},examples:[{title:'Geometry or number theory example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the displayed formula.','Check units.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical verification.'],commonPitfalls:['Mixing units.','Using invalid domains.'],glossary:[{term:'GCD',definition:'Greatest common divisor.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results exact?',answer:'They follow the stated formula.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},

  'z-score': {howToSteps:['Enter the labeled statistical values.','Check the population or sample assumption.','Review the interval or summary.','Interpret results in context.'],formula:{expression:'z = (x − μ) ÷ σ.',variables:[{symbol:'x',definition:'Observed value'},{symbol:'μ',definition:'Mean'},{symbol:'σ',definition:'Standard deviation'},{symbol:'n',definition:'Sample size'}],notes:'This route is educational and uses the stated normal approximation where applicable.'},examples:[{title:'Statistics example',scenario:'Enter valid observations and parameters.',steps:['Enter values.','Apply the displayed formula.','Check assumptions.','Review output.'],result:'The statistical result updates client-side.'}],useCases:['Education and exploratory analysis.'],commonPitfalls:['Confusing population and sample statistics.','Interpreting an estimate without context.'],glossary:[{term:'Standard deviation',definition:'A measure of dispersion around the mean.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results a diagnosis or guarantee?',answer:'No.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'confidence-interval': {howToSteps:['Enter the labeled statistical values.','Check the population or sample assumption.','Review the interval or summary.','Interpret results in context.'],formula:{expression:'CI = mean ± z × σ ÷ √n.',variables:[{symbol:'x',definition:'Observed value'},{symbol:'μ',definition:'Mean'},{symbol:'σ',definition:'Standard deviation'},{symbol:'n',definition:'Sample size'}],notes:'This route is educational and uses the stated normal approximation where applicable.'},examples:[{title:'Statistics example',scenario:'Enter valid observations and parameters.',steps:['Enter values.','Apply the displayed formula.','Check assumptions.','Review output.'],result:'The statistical result updates client-side.'}],useCases:['Education and exploratory analysis.'],commonPitfalls:['Confusing population and sample statistics.','Interpreting an estimate without context.'],glossary:[{term:'Standard deviation',definition:'A measure of dispersion around the mean.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results a diagnosis or guarantee?',answer:'No.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'mean-median-mode-range': {howToSteps:['Enter the labeled statistical values.','Check the population or sample assumption.','Review the interval or summary.','Interpret results in context.'],formula:{expression:'Mean is the arithmetic average; median is the middle ordered value; mode is the most frequent; range = max − min.',variables:[{symbol:'x',definition:'Observed value'},{symbol:'μ',definition:'Mean'},{symbol:'σ',definition:'Standard deviation'},{symbol:'n',definition:'Sample size'}],notes:'This route is educational and uses the stated normal approximation where applicable.'},examples:[{title:'Statistics example',scenario:'Enter valid observations and parameters.',steps:['Enter values.','Apply the displayed formula.','Check assumptions.','Review output.'],result:'The statistical result updates client-side.'}],useCases:['Education and exploratory analysis.'],commonPitfalls:['Confusing population and sample statistics.','Interpreting an estimate without context.'],glossary:[{term:'Standard deviation',definition:'A measure of dispersion around the mean.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results a diagnosis or guarantee?',answer:'No.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},

  'factor': {howToSteps:['Enter the labeled values.','Confirm domain and operation.','Review the result.','Use it for educational calculation.'],formula:{expression:'Factors are positive divisors of an integer.',variables:[{symbol:'x',definition:'Entered numeric value'}],notes:'Invalid and non-finite inputs are guarded.'},examples:[{title:'Number example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check result.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical checks.'],commonPitfalls:['Dividing by zero.','Confusing population assumptions.'],glossary:[{term:'Divisor',definition:'A number that divides another number.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST/SEMATECH e-Handbook',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'matrix': {howToSteps:['Enter the labeled values.','Confirm domain and operation.','Review the result.','Use it for educational calculation.'],formula:{expression:'For a 2×2 matrix, determinant = ad − bc.',variables:[{symbol:'x',definition:'Entered numeric value'}],notes:'Invalid and non-finite inputs are guarded.'},examples:[{title:'Number example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check result.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical checks.'],commonPitfalls:['Dividing by zero.','Confusing population assumptions.'],glossary:[{term:'Divisor',definition:'A number that divides another number.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST/SEMATECH e-Handbook',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'basic': {howToSteps:['Enter the labeled values.','Confirm domain and operation.','Review the result.','Use it for educational calculation.'],formula:{expression:'The selected arithmetic operation is applied to two numbers.',variables:[{symbol:'x',definition:'Entered numeric value'}],notes:'Invalid and non-finite inputs are guarded.'},examples:[{title:'Number example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check result.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical checks.'],commonPitfalls:['Dividing by zero.','Confusing population assumptions.'],glossary:[{term:'Divisor',definition:'A number that divides another number.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST/SEMATECH e-Handbook',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'long-division': {howToSteps:['Enter the labeled values.','Confirm domain and operation.','Review the result.','Use it for educational calculation.'],formula:{expression:'Dividend = divisor × quotient + remainder.',variables:[{symbol:'x',definition:'Entered numeric value'}],notes:'Invalid and non-finite inputs are guarded.'},examples:[{title:'Number example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check result.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical checks.'],commonPitfalls:['Dividing by zero.','Confusing population assumptions.'],glossary:[{term:'Divisor',definition:'A number that divides another number.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST/SEMATECH e-Handbook',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'average': {howToSteps:['Enter the labeled values.','Confirm domain and operation.','Review the result.','Use it for educational calculation.'],formula:{expression:'Mean = sum of observations ÷ number of observations.',variables:[{symbol:'x',definition:'Entered numeric value'}],notes:'Invalid and non-finite inputs are guarded.'},examples:[{title:'Number example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check result.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical checks.'],commonPitfalls:['Dividing by zero.','Confusing population assumptions.'],glossary:[{term:'Divisor',definition:'A number that divides another number.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST/SEMATECH e-Handbook',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},
  'p-value': {howToSteps:['Enter the labeled values.','Confirm domain and operation.','Review the result.','Use it for educational calculation.'],formula:{expression:'A two-tailed normal-approximation p-value is derived from the absolute z statistic.',variables:[{symbol:'x',definition:'Entered numeric value'}],notes:'Invalid and non-finite inputs are guarded.'},examples:[{title:'Number example',scenario:'Enter valid values.',steps:['Enter inputs.','Apply the formula.','Check result.','Review output.'],result:'The result updates client-side.'}],useCases:['Math education.','Quick numerical checks.'],commonPitfalls:['Dividing by zero.','Confusing population assumptions.'],glossary:[{term:'Divisor',definition:'A number that divides another number.'}],faqs:[{question:'Are calculations client-side?',answer:'Yes.'},{question:'Are results rounded?',answer:'Only for display.'}],sources:[{title:'NIST/SEMATECH e-Handbook',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side model.'}},

  'scientific-notation': {howToSteps:['Enter a finite number.','Review the normalized coefficient and exponent.','Check the sign and decimal placement.','Use scientific notation for compact scale comparisons.'],formula:{expression:'x = c × 10^e, where e = floor(log10(|x|)) and c = x ÷ 10^e.',variables:[{symbol:'x',definition:'Original number'},{symbol:'c',definition:'Normalized coefficient with 1 ≤ |c| < 10 for nonzero x'},{symbol:'e',definition:'Integer base-10 exponent'}],notes:'Zero is represented as 0 × 10^0; the widget uses base-10 logarithms.'},examples:[{title:'Large value',scenario:'Convert 450000.',steps:['Find the decimal magnitude.','Move the decimal five places left.','Write 4.5 × 10^5.','Check that the coefficient is normalized.'],result:'450000 = 4.5 × 10^5.'}],useCases:['Express laboratory, engineering, and financial magnitudes compactly.','Compare orders of magnitude.'],commonPitfalls:['Using a non-integer exponent.','Moving the decimal in the wrong direction.','Forgetting that negative values keep their sign in the coefficient.'],glossary:[{term:'Coefficient',definition:'The number multiplied by a power of ten.'},{term:'Exponent',definition:'The integer showing the power of ten.'}],faqs:[{question:'What happens to zero?',answer:'Zero is returned as coefficient 0 and exponent 0.'},{question:'Is this rounding?',answer:'No; the calculation keeps the source number and only formats the coefficient for display.'}],sources:[{title:'NIST Digital Library of Mathematical Functions',publisher:'NIST',url:'https://dlmf.nist.gov/4',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side mathematical implementation.'}},
  'big-number': {howToSteps:['Enter two whole numbers.','Check that digits are valid integers.','Review the exact sum.','Use the result as an integer arithmetic reference.'],formula:{expression:'sum = a + b using arbitrary-precision integer arithmetic.',variables:[{symbol:'a',definition:'First whole-number input'},{symbol:'b',definition:'Second whole-number input'}],notes:'The browser BigInt type avoids the precision loss that can occur with Number for very large integers.'},examples:[{title:'Exact large sum',scenario:'Add 9007199254740993 and 7.',steps:['Read both values as integers.','Add them without converting to floating point.','Preserve every digit.','Review the exact result.'],result:'9007199254741000.'}],useCases:['Check long account, scientific, or identifier-like integer calculations.','Teach the difference between floating-point and integer precision.'],commonPitfalls:['Entering decimals or separators.','Assuming a floating-point result is exact for every digit length.','Adding units that are not comparable.'],glossary:[{term:'BigInt',definition:'JavaScript integer type for arbitrary-size whole-number arithmetic.'},{term:'Precision',definition:'How exactly a numeric value is represented.'}],faqs:[{question:'Can it add decimals?',answer:'No; this route is intentionally limited to whole numbers.'},{question:'Does it send inputs to a server?',answer:'No; the operation runs in the browser.'}],sources:[{title:'BigInt — JavaScript Reference',publisher:'MDN Web Docs',url:'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side exact-integer implementation.'}},
  'common-factor': {howToSteps:['Enter two whole numbers.','Review the shared positive factors.','Check the smallest and largest listed factors.','Use the list to support factorization or fraction reduction.'],formula:{expression:'A common factor d satisfies a mod d = 0 and b mod d = 0.',variables:[{symbol:'a,b',definition:'The two integer inputs'},{symbol:'d',definition:'A positive integer tested as a shared divisor'}],notes:'The widget lists positive factors up to the smaller absolute input and guards non-finite values.'},examples:[{title:'Shared factors',scenario:'Find common factors of 18 and 24.',steps:['Test positive divisors through 18.','Keep divisors of both values.','Order the retained factors.','Review 1, 2, 3, and 6.'],result:'The common factors are 1, 2, 3, and 6.'}],useCases:['Reduce fractions.','Find compatible group sizes.','Support greatest-common-factor work.'],commonPitfalls:['Including a factor larger than the smaller absolute value.','Ignoring negative inputs should be treated by absolute magnitude.','Confusing common factors with prime factors.'],glossary:[{term:'Factor',definition:'An integer that divides another integer without remainder.'},{term:'Common factor',definition:'A factor shared by two or more integers.'}],faqs:[{question:'Are negative factors listed?',answer:'No; this route lists positive factors.'},{question:'What if one input is zero?',answer:'The guarded result is an empty list because the finite positive-factor range is undefined.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/',year:2024}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side integer implementation.'}},
  'date': {howToSteps:['Choose a calendar date.','Enter a whole number of days to add; negative values subtract days.','Review the resulting ISO date.','Check timezone assumptions when comparing with another system.'],formula:{expression:'resulting date = date shifted by an integer number of UTC calendar days.',variables:[{symbol:'date',definition:'Starting calendar date'},{symbol:'days',definition:'Whole days added or subtracted'}],notes:'The route uses UTC midnight to avoid daylight-saving offsets changing the calendar date.'},examples:[{title:'Add calendar days',scenario:'Add 30 days to 2026-01-01.',steps:['Select 2026-01-01.','Enter 30.','Shift the UTC calendar date.','Read the ISO result.'],result:'The resulting date is 2026-01-31.'}],useCases:['Plan due dates and review windows.','Count calendar days between simple date milestones.'],commonPitfalls:['Confusing calendar days with business days.','Using a locale-formatted date as if it were ISO.','Ignoring holidays or time-of-day requirements.'],glossary:[{term:'UTC',definition:'A stable reference time standard used here to avoid local offset drift.'},{term:'Calendar day',definition:'A date increment of one day, independent of business holidays.'}],faqs:[{question:'Does it skip weekends?',answer:'No; it shifts calendar days, including weekends.'},{question:'Does it include holidays?',answer:'No holiday calendar is applied.'}],sources:[{title:'Date and Time — JavaScript Guide',publisher:'MDN Web Docs',url:'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Representing_dates_times',year:2026}],author:{name:'CalculatorFree Date Review Team',credentials:'Calculation Review',description:'Original client-side calendar-date implementation.'}},
  'time': {howToSteps:['Enter the starting clock time.','Enter the ending clock time.','Review elapsed minutes.','For an overnight interval, interpret the result with the displayed same-day assumption.'],formula:{expression:'elapsed minutes = end time in minutes since midnight − start time in minutes since midnight.',variables:[{symbol:'start',definition:'Starting HH:MM clock time'},{symbol:'end',definition:'Ending HH:MM clock time'}],notes:'This route reports same-day elapsed minutes; if the end is earlier, the result is negative rather than silently assuming an overnight shift.'},examples:[{title:'Elapsed clock time',scenario:'Compare 09:15 with 11:45.',steps:['Convert 09:15 to 555 minutes.','Convert 11:45 to 705 minutes.','Subtract start from end.','Review 150 minutes.'],result:'The elapsed time is 150 minutes, or 2 hours 30 minutes.'}],useCases:['Check meeting and appointment durations.','Convert clock-time differences into minutes for schedules.'],commonPitfalls:['Assuming a negative result automatically means overnight.','Mixing local dates with clock-only times.','Forgetting that HH:MM has a 60-minute hour.'],glossary:[{term:'Elapsed time',definition:'The duration between two clock readings.'},{term:'Minutes since midnight',definition:'A clock reading converted to hour × 60 plus minutes.'}],faqs:[{question:'Does it calculate across midnight?',answer:'Not silently; an earlier end time produces a negative same-day difference.'},{question:'Are seconds supported?',answer:'No; inputs are HH:MM.'}],sources:[{title:'Date and Time — JavaScript Guide',publisher:'MDN Web Docs',url:'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Representing_dates_times',year:2026}],author:{name:'CalculatorFree Date Review Team',credentials:'Calculation Review',description:'Original client-side time-difference implementation.'}},


  'hours': {howToSteps:['Enter the start clock time.','Enter the end clock time.','Enter unpaid break minutes.','Review net paid hours and confirm whether the interval crosses midnight.'],formula:{expression:'Net hours = max(0, end minutes − start minutes − unpaid break minutes) ÷ 60.',variables:[{symbol:'start',definition:'Starting HH:MM time'},{symbol:'end',definition:'Ending HH:MM time'},{symbol:'break',definition:'Unpaid break duration in minutes'}],notes:'The widget uses a same-day interval and clamps negative net time to zero; overnight shifts should be entered with an appropriate date-aware workflow.'},examples:[{title:'Paid shift',scenario:'Work from 09:00 to 17:00 with a 60-minute unpaid break.',steps:['Convert the clock readings to minutes.','Subtract start from end for 480 minutes.','Subtract the 60-minute break.','Divide 420 by 60.'],result:'Net paid time is 7 hours.'}],useCases:['Estimate paid hours for a same-day shift.','Check break deductions before reviewing a timesheet.'],commonPitfalls:['Counting unpaid breaks as paid time.','Using a same-day calculator for an overnight shift.','Entering minutes as decimal hours.'],glossary:[{term:'Elapsed minutes',definition:'Clock duration expressed as a number of minutes.'},{term:'Net hours',definition:'Elapsed shift time after the entered unpaid break.'}],faqs:[{question:'Does it handle overnight shifts?',answer:'No; an earlier end time is clamped to zero rather than silently assuming the next day.'},{question:'Are breaks deducted?',answer:'Yes, the entered non-negative break minutes are deducted.'}],sources:[{title:'Fair Labor Standards Act — Hours Worked',publisher:'U.S. Department of Labor',url:'https://www.dol.gov/agencies/whd/flsa/hours-worked',year:2026}],author:{name:'CalculatorFree Time Review Team',credentials:'Calculation Review',description:'Original client-side duration model.'}},
  'grade': {howToSteps:['Enter points earned.','Enter points possible.','Review the percentage and letter band.','Confirm the grading scale before using the result for a course decision.'],formula:{expression:'Percentage = min(100, max(0, earned ÷ possible × 100)); letter bands are A ≥ 90, B ≥ 80, C ≥ 70, D ≥ 60, otherwise F.',variables:[{symbol:'earned',definition:'Points received'},{symbol:'possible',definition:'Maximum available points'}],notes:'The educational banding is a simple 90/80/70/60 scale; institutions may use different cutoffs, plus/minus grades, or weighting.'},examples:[{title:'Course score',scenario:'Earn 85 points out of 100.',steps:['Enter 85 as earned points.','Enter 100 as possible points.','Divide earned by possible.','Multiply by 100 and apply the displayed band.'],result:'The result is 85.00%, which maps to B under this simple scale.'}],useCases:['Check a raw assignment or exam percentage.','Compare a score with a stated letter-grade scale.'],commonPitfalls:['Using a weighted gradebook score as raw points.','Assuming every institution uses the same letter cutoffs.','Entering a possible score of zero.'],glossary:[{term:'Raw percentage',definition:'Earned points divided by possible points, expressed as a percent.'},{term:'Letter band',definition:'A label assigned by the threshold scale shown in the formula.'}],faqs:[{question:'Does this calculate a GPA?',answer:'No; it converts one earned/possible score into a percentage and simple letter band.'},{question:'Does it support plus or minus grades?',answer:'No; the displayed scale uses whole letter bands only.'}],sources:[{title:'Grading Practices and Policies',publisher:'National Education Association',url:'https://www.nea.org/resource-library/grades',year:2026}],author:{name:'CalculatorFree Education Review Team',credentials:'Calculation Review',description:'Original client-side grading helper.'}},


  'height': {howToSteps:['Enter whole feet.','Enter additional inches.','Review total inches and metric conversions.','Confirm the measurement units before using the result.'],formula:{expression:'Total inches = 12 × feet + inches; centimeters = total inches × 2.54; meters = total inches × 0.0254.',variables:[{symbol:'feet',definition:'Whole-foot component of height'},{symbol:'inches',definition:'Additional inch component'}],notes:'Negative and non-finite values are clamped to zero; this is a unit-conversion calculator, not a health assessment.'},examples:[{title:'Height conversion',scenario:'Enter 5 feet and 10 inches.',steps:['Convert 5 feet to 60 inches.','Add 10 inches for 70 total inches.','Multiply by 2.54 for centimeters.','Divide centimeters by 100 for meters.'],result:'70 inches = 177.80 cm = 1.778 m.'}],useCases:['Convert imperial height to metric units.','Prepare consistent units for another calculation.'],commonPitfalls:['Entering total inches in the feet field.','Mixing centimeters with inches.','Treating a unit conversion as a medical classification.'],glossary:[{term:'Inch',definition:'An imperial length unit equal to 2.54 centimeters.'},{term:'Centimeter',definition:'A metric length unit equal to one hundredth of a meter.'}],faqs:[{question:'Does this assess ideal height?',answer:'No; it only converts feet and inches to metric units.'},{question:'Can I enter extra inches above 12?',answer:'Yes; the formula converts the full input into total inches.'}],sources:[{title:'The International System of Units (SI)',publisher:'BIPM',url:'https://www.bipm.org/en/measurement-units',year:2026}],author:{name:'CalculatorFree Measurement Review Team',credentials:'Calculation Review',description:'Original client-side unit conversion.'}},
  'concrete': {howToSteps:['Enter slab length in feet.','Enter slab width in feet.','Enter average depth in inches.','Review cubic feet and cubic yards, then add a practical waste allowance separately.'],formula:{expression:'Volume in cubic feet = length(ft) × width(ft) × depth(in) ÷ 12; cubic yards = cubic feet ÷ 27.',variables:[{symbol:'L',definition:'Length in feet'},{symbol:'W',definition:'Width in feet'},{symbol:'D',definition:'Depth in inches'}],notes:'The result is geometric volume only; site waste, irregular shapes, reinforcement, and supplier minimums are not included.'},examples:[{title:'Rectangular slab',scenario:'A 20 ft × 10 ft area with 4 in depth.',steps:['Convert 4 inches to 0.3333 feet.','Multiply 20 × 10 × 0.3333.','Convert cubic feet to yards by dividing by 27.','Add project-specific waste after reviewing the base volume.'],result:'Base volume is about 66.67 ft³ or 2.47 yd³.'}],useCases:['Estimate base material volume for a rectangular slab.','Compare supplier quantities before requesting a quote.'],commonPitfalls:['Using depth in feet while the formula expects inches.','Ignoring uneven grade or openings.','Ordering without a site-specific waste allowance.'],glossary:[{term:'Cubic foot',definition:'Volume of a cube one foot on each side.'},{term:'Cubic yard',definition:'27 cubic feet, commonly used for bulk concrete ordering.'}],faqs:[{question:'Does this include waste?',answer:'No; it returns geometric base volume so you can add a project-specific allowance.'},{question:'Does it handle circular or irregular areas?',answer:'No; this route models a rectangular prism.'}],sources:[{title:'Concrete Basics',publisher:'American Concrete Institute',url:'https://www.concrete.org/topicsinconcrete/topicdetail.aspx?search=concrete%20basics',year:2026}],author:{name:'CalculatorFree Construction Review Team',credentials:'Calculation Review',description:'Original client-side volume estimate.'}},


  'ip-subnet': {howToSteps:['Enter a valid IPv4 address.','Enter a CIDR prefix from 0 through 32.','Review network, broadcast, and host range values.','Confirm the address plan with your network documentation before changing production configuration.'],formula:{expression:'network = address AND subnet mask; total addresses = 2^(32 − prefix).',variables:[{symbol:'address',definition:'32-bit IPv4 address'},{symbol:'prefix',definition:'Number of network bits in CIDR notation'}],notes:'For /0 through /30, usable hosts are total addresses minus network and broadcast addresses; /31 and /32 are reported using their full address counts.'},examples:[{title:'Private /24 network',scenario:'Analyze 192.168.1.42/24.',steps:['Apply the 255.255.255.0 mask.','Set host bits to zero for the network.','Set host bits to one for broadcast.','Count 256 addresses and 254 conventional usable hosts.'],result:'Network 192.168.1.0, broadcast 192.168.1.255, usable range 192.168.1.1–192.168.1.254.'}],useCases:['Document a small IPv4 address block.','Check host capacity before allocating devices.'],commonPitfalls:['Confusing prefix length with host-bit count.','Using IPv4 rules for IPv6.','Assuming every /31 or /32 has two conventional hosts.'],glossary:[{term:'CIDR',definition:'Classless Inter-Domain Routing notation such as /24.'},{term:'Broadcast address',definition:'The address with all host bits set to one in a subnet.'}],faqs:[{question:'Does this support IPv6?',answer:'No; this route validates dotted-decimal IPv4 only.'},{question:'Does it configure a router?',answer:'No; it only calculates address boundaries client-side.'}],sources:[{title:'RFC 4632 — Classless Inter-Domain Routing',publisher:'Internet Engineering Task Force',url:'https://www.rfc-editor.org/rfc/rfc4632',year:2006}],author:{name:'CalculatorFree Network Review Team',credentials:'Calculation Review',description:'Original client-side IPv4 subnet implementation.'}},
  'conversion': {howToSteps:['Enter a numeric length.','Choose the source unit.','Choose the target unit.','Review the converted value and keep the unit labels with it.'],formula:{expression:'target value = source value × source-to-meter factor ÷ target-to-meter factor.',variables:[{symbol:'source value',definition:'Length entered by the user'},{symbol:'factor',definition:'Exact conversion factor for meters, kilometers, feet, miles, or inches'}],notes:'This route is a length converter; it does not infer dimensions or convert unrelated physical quantities.'},examples:[{title:'Meters to feet',scenario:'Convert 1 meter to feet.',steps:['Choose meters as the source.','Choose feet as the target.','Multiply by 1 meter ÷ 0.3048 meters per foot.','Review the rounded display.'],result:'1 meter is approximately 3.280839895 feet.'}],useCases:['Normalize length units for plans and measurements.','Translate imperial and metric values for communication.'],commonPitfalls:['Selecting the wrong source unit.','Rounding before the conversion is complete.','Treating a length conversion as an area or volume conversion.'],glossary:[{term:'Conversion factor',definition:'A ratio that relates one unit to another.'},{term:'SI unit',definition:'A unit from the International System of Units; meter is the base length unit.'}],faqs:[{question:'Does this convert area?',answer:'No; the current route converts one-dimensional length only.'},{question:'Are values sent to a server?',answer:'No; the conversion runs in the browser.'}],sources:[{title:'The International System of Units (SI)',publisher:'BIPM',url:'https://www.bipm.org/en/measurement-units',year:2026}],author:{name:'CalculatorFree Measurement Review Team',credentials:'Calculation Review',description:'Original client-side length conversion.'}},


  'fuel-cost': {howToSteps:['Enter trip distance in miles.','Enter vehicle fuel economy in miles per gallon.','Enter fuel price per gallon.','Review estimated gallons and cost, then adjust for route and driving conditions.'],formula:{expression:'Gallons used = distance ÷ MPG; fuel cost = gallons used × price per gallon.',variables:[{symbol:'distance',definition:'Trip distance in miles'},{symbol:'MPG',definition:'Fuel economy in miles per gallon'},{symbol:'price',definition:'Fuel price per gallon'}],notes:'The result excludes parking, tolls, maintenance, taxes, and changes in fuel economy.'},examples:[{title:'Trip estimate',scenario:'Drive 100 miles at 25 MPG with fuel priced at $3.50 per gallon.',steps:['Divide 100 miles by 25 MPG.','Calculate 4 gallons.','Multiply 4 by $3.50.','Review the estimated fuel cost.'],result:'The estimate is 4 gallons and $14.00.'}],useCases:['Compare fuel cost across vehicles or routes.','Budget a simple road trip.'],commonPitfalls:['Using fuel economy in liters per 100 km.','Ignoring traffic, terrain, and idling.','Treating a price estimate as a receipt total.'],glossary:[{term:'MPG',definition:'Miles traveled per U.S. gallon of fuel.'},{term:'Fuel economy',definition:'Distance a vehicle travels per unit of fuel.'}],faqs:[{question:'Does this include tolls?',answer:'No; it estimates fuel only.'},{question:'Can I use diesel or gasoline?',answer:'Yes, as long as the price and economy use the same gallon convention.'}],sources:[{title:'Fuel Economy Guide',publisher:'U.S. Department of Energy and EPA',url:'https://www.fueleconomy.gov/feg/guide.shtml',year:2026}],author:{name:'CalculatorFree Automotive Review Team',credentials:'Calculation Review',description:'Original client-side fuel estimate.'}},
  'voltage-drop': {howToSteps:['Enter load current in amps.','Enter one-way cable length in feet.','Enter conductor resistance in ohms per 1,000 feet.','Enter supply voltage and review drop and remaining voltage.'],formula:{expression:'Voltage drop = current × (2 × one-way length ÷ 1000) × resistance per 1000 ft.',variables:[{symbol:'I',definition:'Load current in amps'},{symbol:'L',definition:'One-way conductor length in feet'},{symbol:'R',definition:'Conductor resistance in ohms per 1000 feet'}],notes:'The factor of two models a round-trip pair; real installations also depend on conductor temperature, connections, and code requirements.'},examples:[{title:'Two-wire run',scenario:'A 10 A load, 100 ft one-way, 1.0 Ω/1000 ft, and 120 V supply.',steps:['Double the one-way length to 200 ft.','Convert to 0.2 of a 1000-ft resistance unit.','Multiply 10 × 0.2 × 1.0.','Compare the 2 V drop with the 120 V supply.'],result:'Drop is 2 V, or about 1.67%, leaving 118 V in the simplified model.'}],useCases:['Screen a cable run during early design.','Compare conductor resistance assumptions.'],commonPitfalls:['Using total loop length and doubling it again.','Confusing resistance per foot with resistance per 1000 feet.','Treating this estimate as a code compliance decision.'],glossary:[{term:'Voltage drop',definition:'Reduction in voltage along a conductor under load.'},{term:'Round trip',definition:'The outgoing and returning conductor path included in the simplified model.'}],faqs:[{question:'Does this select wire size?',answer:'No; it only estimates voltage drop from the entered resistance.'},{question:'Is the result code compliant?',answer:'No; consult applicable electrical codes and a qualified electrician.'}],sources:[{title:'National Electrical Code',publisher:'NFPA',url:'https://www.nfpa.org/codes-and-standards/nfpa-70-standard-development/70',year:2026}],author:{name:'CalculatorFree Electrical Review Team',credentials:'Calculation Review',description:'Original client-side screening estimate.'}},
  'btu': {howToSteps:['Enter conditioned floor area.','Enter the indoor/outdoor temperature difference.','Enter a project heat-loss factor.','Review the simplified hourly BTU estimate and obtain a professional sizing calculation for installation.'],formula:{expression:'Estimated heat load = area × temperature difference × heat-loss factor.',variables:[{symbol:'A',definition:'Area in square feet'},{symbol:'ΔT',definition:'Temperature difference in °F'},{symbol:'F',definition:'User-entered heat-loss factor in BTU/hr/ft²/°F'}],notes:'This screening model omits windows, infiltration, solar gain, duct losses, occupancy, and local design temperatures.'},examples:[{title:'Screening estimate',scenario:'1000 ft², 30°F difference, factor 0.5.',steps:['Multiply 1000 by 30.','Multiply by 0.5 BTU/hr/ft²/°F.','Review the hourly estimate.','Compare with a Manual J or equivalent professional assessment.'],result:'The simplified estimate is 15,000 BTU/hr.'}],useCases:['Early comparison of heating-load assumptions.','Explain how area and temperature difference affect a screening estimate.'],commonPitfalls:['Using this to buy equipment without a load study.','Confusing BTU/hr with total seasonal energy.','Using a heat-loss factor outside its intended context.'],glossary:[{term:'BTU/hr',definition:'Rate of heat transfer expressed in British thermal units per hour.'},{term:'Heat-loss factor',definition:'Simplified user-entered coefficient relating area and temperature difference to heat load.'}],faqs:[{question:'Is this an HVAC sizing calculation?',answer:'No; it is a simplified educational estimate.'},{question:'Does it include cooling load?',answer:'No; this route models a heating-style screening load.'}],sources:[{title:'Manual J Residential Load Calculation',publisher:'ACCA',url:'https://www.acca.org/standards/technical-manuals',year:2026}],author:{name:'CalculatorFree Building Science Review Team',credentials:'Calculation Review',description:'Original client-side screening model.'}},
  'square-footage': {howToSteps:['Enter rectangular length in feet.','Enter rectangular width in feet.','Review square feet.','Measure separate irregular sections individually before adding them.'],formula:{expression:'Area = length × width.',variables:[{symbol:'L',definition:'Length in feet'},{symbol:'W',definition:'Width in feet'}],notes:'This route models one rectangle and returns square feet; it does not include waste or openings.'},examples:[{title:'Rectangular room',scenario:'A room is 20 ft by 10 ft.',steps:['Enter 20 for length.','Enter 10 for width.','Multiply the dimensions.','Review 200 square feet.'],result:'The rectangular area is 200 ft².'}],useCases:['Estimate flooring, paint, or surface coverage area.','Create a first-pass room or slab measurement.'],commonPitfalls:['Mixing feet with inches.','Adding length and width instead of multiplying.','Ignoring alcoves, openings, or non-rectangular sections.'],glossary:[{term:'Square foot',definition:'Area of a square one foot on each side.'},{term:'Rectangle',definition:'A four-sided shape with right-angle corners and opposite sides equal.'}],faqs:[{question:'Does it calculate perimeter?',answer:'No; it calculates rectangular area only.'},{question:'Does it add material waste?',answer:'No; add a project-specific allowance after measuring the base area.'}],sources:[{title:'Units and Measurement',publisher:'National Institute of Standards and Technology',url:'https://www.nist.gov/pml/owm/metric-si/si-units',year:2026}],author:{name:'CalculatorFree Measurement Review Team',credentials:'Calculation Review',description:'Original client-side area calculation.'}},


  'time-card': {howToSteps:['Enter days worked.','Enter average hours per day.','Enter hourly rate.','Review total, regular, overtime hours, and estimated gross pay.'],formula:{expression:'Total hours = days × hours per day; regular hours = min(total, days × 8); overtime hours = max(0, total − regular); gross pay = regular × rate + overtime × rate × 1.5.',variables:[{symbol:'days',definition:'Number of worked days'},{symbol:'hours',definition:'Average hours per day'},{symbol:'rate',definition:'Hourly pay rate'}],notes:'The default model uses 8 regular hours per day and a 1.5 overtime multiplier; payroll rules and exemptions vary by jurisdiction.'},examples:[{title:'Five-day week',scenario:'Work 5 days at 9 hours per day and $20 per hour.',steps:['Calculate 45 total hours.','Count 40 regular hours.','Count 5 overtime hours.','Apply $20 regular and 1.5× overtime rates.'],result:'Estimated gross pay is $950 before taxes and deductions.'}],useCases:['Estimate a simple weekly timesheet.','Separate regular and overtime hours for a first-pass pay check.'],commonPitfalls:['Using this as a payroll or legal determination.','Ignoring daily versus weekly overtime rules.','Entering a salary or net pay as an hourly rate.'],glossary:[{term:'Regular hours',definition:'Hours paid at the base rate in this simplified model.'},{term:'Overtime multiplier',definition:'Factor applied to overtime rate, defaulting to 1.5.'}],faqs:[{question:'Does it calculate taxes?',answer:'No; it estimates gross pay before deductions.'},{question:'Are overtime rules universal?',answer:'No; verify the rules that apply to the worker and jurisdiction.'}],sources:[{title:'Hours Worked Under the Fair Labor Standards Act',publisher:'U.S. Department of Labor',url:'https://www.dol.gov/agencies/whd/flsa/hours-worked',year:2026}],author:{name:'CalculatorFree Payroll Review Team',credentials:'Calculation Review',description:'Original client-side timesheet estimate.'}},
  'time-zone': {howToSteps:['Choose a UTC date and time.','Select a target IANA time zone.','Review the formatted local time.','Confirm daylight-saving and date context for real scheduling.'],formula:{expression:'Target display = format(UTC instant, target IANA time zone).',variables:[{symbol:'UTC instant',definition:'A date-time interpreted as Coordinated Universal Time'},{symbol:'IANA zone',definition:'A named time-zone rule set such as Europe/London'}],notes:'The browser Intl implementation applies the selected zone’s offset and daylight-saving rules; the route does not convert an unspecified local time.'},examples:[{title:'London to Tokyo',scenario:'Convert 2026-08-26 12:00 UTC to Asia/Tokyo.',steps:['Interpret the input as a UTC instant.','Select Asia/Tokyo.','Apply the zone offset for that date.','Review the localized calendar date and time.'],result:'The result is displayed in the target zone with its date and time.'}],useCases:['Coordinate calls across named time zones.','Check whether a meeting crosses a calendar-day boundary.'],commonPitfalls:['Entering local time while labeling it UTC.','Ignoring daylight-saving changes.','Assuming a fixed offset for a named zone year-round.'],glossary:[{term:'UTC',definition:'Coordinated Universal Time, used as the input reference.'},{term:'IANA time zone',definition:'A database-backed named zone containing historical and seasonal offset rules.'}],faqs:[{question:'Does it schedule a meeting?',answer:'No; it formats one UTC instant for a selected zone.'},{question:'Does it account for daylight saving?',answer:'Yes, where the browser’s time-zone database provides the applicable rule.'}],sources:[{title:'ECMA-402 Internationalization API Specification',publisher:'Ecma International',url:'https://tc39.es/ecma402/',year:2026}],author:{name:'CalculatorFree Time Review Team',credentials:'Calculation Review',description:'Original client-side timezone formatting helper.'}},


  'bra-size': {howToSteps:['Enter underbust measurement in inches.','Enter bust measurement in inches.','Review the approximate band and cup label.','Check the brand’s own size chart and fit because sizing systems vary.'],formula:{expression:'Band = max(28, nearest even underbust); cup index = round(max(0, bust − band)).',variables:[{symbol:'underbust',definition:'Measurement around the rib cage in inches'},{symbol:'bust',definition:'Measurement around the fullest bust point in inches'}],notes:'This is a simplified starting estimate using one band/cup convention; it is not a medical or guaranteed garment-size determination.'},examples:[{title:'Approximate fit starting point',scenario:'Enter 34 inches underbust and 38 inches bust.',steps:['Round the band convention to 34.','Subtract band from bust for a 4-inch difference.','Map the difference to the simplified cup sequence.','Confirm against the brand and garment fit.'],result:'The starting estimate is 34D under this simplified model.'}],useCases:['Create a first-pass sizing starting point.','Compare measurements before checking a retailer chart.'],commonPitfalls:['Measuring over thick clothing.','Assuming brands use the same cup progression.','Treating a starting estimate as a guaranteed fit.'],glossary:[{term:'Underbust',definition:'Circumference measured around the rib cage below the bust.'},{term:'Cup difference',definition:'Bust measurement minus the selected band measurement.'}],faqs:[{question:'Is the result universal?',answer:'No; sizing systems and fit vary by brand, style, and region.'},{question:'Is this a health assessment?',answer:'No; it is an approximate clothing-size helper only.'}],sources:[{title:'Bra Fit and Measurement Guidance',publisher:'The Underfashion Club',url:'https://underfashionclub.org/',year:2026}],author:{name:'CalculatorFree Measurement Review Team',credentials:'Calculation Review',description:'Original client-side garment sizing estimate.'}},
  'love': {howToSteps:['Enter two names or labels.','Review the deterministic entertainment score.','Read the playful interpretation.','Treat the output as a game, not relationship evidence.'],formula:{expression:'score = deterministic hash of the two lower-case input strings, mapped to 0–100.',variables:[{symbol:'first',definition:'First text input'},{symbol:'second',definition:'Second text input'}],notes:'The score is intentionally recreational and has no psychological, scientific, or predictive validity.'},examples:[{title:'For-fun result',scenario:'Enter Alex and Sam.',steps:['Normalize the two strings.','Apply the repeatable character hash.','Map the hash to a percentage.','Read the playful label.'],result:'A repeatable entertainment score appears; it does not measure compatibility.'}],useCases:['Use as a light conversation game.','Create a repeatable novelty result for two names.'],commonPitfalls:['Treating a novelty score as a prediction.','Using the result for a relationship decision.','Assuming different spellings have the same output.'],glossary:[{term:'Deterministic',definition:'The same inputs produce the same output.'},{term:'Entertainment score',definition:'A playful number with no validated predictive meaning.'}],faqs:[{question:'Is the score scientific?',answer:'No; it is entertainment only.'},{question:'Does a high score prove compatibility?',answer:'No; names cannot establish relationship compatibility.'}],sources:[{title:'APA Dictionary of Psychology — Validity',publisher:'American Psychological Association',url:'https://dictionary.apa.org/validity',year:2026}],author:{name:'CalculatorFree General Tools Team',credentials:'Calculation Review',description:'Transparent recreational client-side tool.'}},
  'gdp': {howToSteps:['Enter consumer spending.','Enter investment.','Enter government spending.','Enter exports and imports, then review the expenditure estimate.'],formula:{expression:'GDP = consumer spending + investment + government spending + exports − imports.',variables:[{symbol:'C',definition:'Consumer spending'},{symbol:'I',definition:'Investment'},{symbol:'G',definition:'Government spending'},{symbol:'X',definition:'Exports'},{symbol:'M',definition:'Imports'}],notes:'This is the expenditure identity for a chosen period and currency; it is not seasonally adjusted or inflation adjusted by the widget.'},examples:[{title:'Expenditure estimate',scenario:'C=20,000; I=5,000; G=4,000; X=2,000; M=1,000.',steps:['Add consumption, investment, and government spending.','Add exports.','Subtract imports.','Review the period-specific total.'],result:'Estimated GDP is 30,000 currency units.'}],useCases:['Explain the expenditure approach to GDP.','Run simple scenario comparisons for macroeconomic components.'],commonPitfalls:['Mixing periods or currencies.','Confusing nominal GDP with real GDP.','Forgetting that imports are subtracted in this identity.'],glossary:[{term:'GDP',definition:'Gross domestic product, the market value of final goods and services produced in a period.'},{term:'Net exports',definition:'Exports minus imports.'}],faqs:[{question:'Does this calculate real GDP?',answer:'No; no deflator or chained-price adjustment is applied.'},{question:'Does it use official national accounts?',answer:'No; it calculates from the values you enter.'}],sources:[{title:'National Income and Product Accounts',publisher:'U.S. Bureau of Economic Analysis',url:'https://www.bea.gov/data/gdp/gross-domestic-product',year:2026}],author:{name:'CalculatorFree Economics Review Team',credentials:'Calculation Review',description:'Original client-side expenditure-identity helper.'}},
  'gas-mileage': {howToSteps:['Enter distance driven in miles.','Enter fuel consumed in gallons.','Review miles per gallon.','Keep the same distance and fuel units when comparing trips.'],formula:{expression:'MPG = distance in miles ÷ fuel used in gallons.',variables:[{symbol:'distance',definition:'Travel distance in miles'},{symbol:'fuel',definition:'Fuel consumed in U.S. gallons'}],notes:'This route calculates observed fuel economy for a trip; it does not estimate manufacturer ratings or account for driving conditions.'},examples:[{title:'Trip fuel economy',scenario:'Travel 300 miles using 10 gallons.',steps:['Enter 300 miles.','Enter 10 gallons.','Divide distance by fuel.','Review the observed MPG.'],result:'Fuel economy is 30 MPG.'}],useCases:['Compare observed economy across trips.','Estimate a vehicle’s simple miles-per-gallon value.'],commonPitfalls:['Using fuel purchased instead of fuel consumed when the tank was not full.','Mixing liters with gallons.','Comparing trips with very different loads or conditions.'],glossary:[{term:'MPG',definition:'Miles traveled per gallon of fuel.'},{term:'Observed fuel economy',definition:'Fuel economy calculated from a specific trip’s distance and fuel use.'}],faqs:[{question:'Does it calculate L/100 km?',answer:'No; the current route reports miles per U.S. gallon.'},{question:'What if fuel used is zero?',answer:'The guarded result is 0 rather than an infinite value.'}],sources:[{title:'Fuel Economy Guide',publisher:'U.S. Department of Energy and EPA',url:'https://www.fueleconomy.gov/feg/guide.shtml',year:2026}],author:{name:'CalculatorFree Automotive Review Team',credentials:'Calculation Review',description:'Original client-side observed-MPG calculation.'}},


  'horsepower': {howToSteps:['Enter power in watts.','Review mechanical and electrical horsepower conversions.','Keep the horsepower type with the result.','Use the conversion as a unit reference rather than an equipment rating.'],formula:{expression:'Mechanical hp = watts ÷ 745.699872; electrical hp = watts ÷ 746.',variables:[{symbol:'W',definition:'Power in watts'},{symbol:'745.699872',definition:'Exact metric mechanical horsepower watt equivalent used here'},{symbol:'746',definition:'Rounded electrical horsepower watt equivalent used here'}],notes:'Horsepower conventions vary; the widget reports both named conversions.'},examples:[{title:'One kilowatt',scenario:'Convert 1000 watts.',steps:['Enter 1000 W.','Divide by 745.699872 for mechanical horsepower.','Divide by 746 for electrical horsepower.','Keep the convention in the comparison.'],result:'The result is about 1.341 mechanical hp and 1.340 electrical hp.'}],useCases:['Convert power specifications between watts and horsepower.','Compare equipment data sheets with explicit conventions.'],commonPitfalls:['Calling every horsepower value identical.','Confusing power with torque.','Mixing metric and electrical conventions without labeling them.'],glossary:[{term:'Watt',definition:'SI unit of power equal to one joule per second.'},{term:'Horsepower',definition:'A family of power units with convention-dependent watt equivalents.'}],faqs:[{question:'Which horsepower does it return?',answer:'Both a mechanical conversion and a rounded electrical conversion are shown.'},{question:'Does it rate an engine?',answer:'No; it only converts entered power.'}],sources:[{title:'NIST Guide to the SI',publisher:'National Institute of Standards and Technology',url:'https://www.nist.gov/pml/owm/metric-si/si-units',year:2026}],author:{name:'CalculatorFree Engineering Review Team',credentials:'Calculation Review',description:'Original client-side power conversion.'}},
  'engine-horsepower': {howToSteps:['Enter engine torque in pound-feet.','Enter engine speed in RPM.','Review estimated horsepower.','Treat the result as idealized power at that torque and speed, not a dyno measurement.'],formula:{expression:'Horsepower = torque(lb-ft) × RPM ÷ 5252.',variables:[{symbol:'T',definition:'Engine torque in pound-feet'},{symbol:'RPM',definition:'Rotational speed in revolutions per minute'}],notes:'The formula assumes torque and RPM are measured at the same operating point and uses the conventional 5252 constant.'},examples:[{title:'Torque and speed',scenario:'300 lb-ft at 5252 RPM.',steps:['Multiply torque by RPM.','Divide by 5252.','Check that units are lb-ft and RPM.','Review the estimated output.'],result:'Estimated power is 300 horsepower.'}],useCases:['Explain the relationship between torque, speed, and power.','Compare idealized operating points from a specification sheet.'],commonPitfalls:['Using newton-metres with the lb-ft formula.','Comparing torque at one RPM with power at another.','Treating peak values as simultaneous.'],glossary:[{term:'Torque',definition:'Rotational force, here expressed in pound-feet.'},{term:'RPM',definition:'Revolutions per minute, a measure of rotational speed.'}],faqs:[{question:'Is this a dyno result?',answer:'No; it is a formula-based estimate from entered torque and RPM.'},{question:'Can I enter metric torque?',answer:'Not in this route; convert to pound-feet first.'}],sources:[{title:'Engine Power and Torque Relationship',publisher:'Society of Automotive Engineers',url:'https://www.sae.org/',year:2026}],author:{name:'CalculatorFree Automotive Review Team',credentials:'Calculation Review',description:'Original client-side power estimate.'}},
  'stair': {howToSteps:['Enter total vertical rise in inches.','Enter target riser height.','Enter tread depth.','Review riser count, actual riser, total run, and angle, then verify local building requirements.'],formula:{expression:'Risers = ceil(total rise ÷ target riser); actual riser = total rise ÷ risers; treads = risers − 1; run = treads × tread depth; angle = atan(actual riser ÷ tread depth).',variables:[{symbol:'rise',definition:'Total vertical rise in inches'},{symbol:'riser',definition:'Target vertical height per step'},{symbol:'tread',definition:'Horizontal depth per tread'}],notes:'This is a layout estimate; landings, nosings, headroom, handrails, and jurisdictional code requirements are not modeled.'},examples:[{title:'Straight stair layout',scenario:'120 inches total rise, 7-inch target riser, 10-inch tread.',steps:['Divide 120 by 7 and round up to 18 risers.','Calculate actual riser as 120 ÷ 18.','Use 17 treads for a straight stair.','Multiply 17 by 10 inches for the run.'],result:'The estimate is 18 risers, 17 treads, 116.67 inches run, and approximately 36.53°.'}],useCases:['Create an early stair layout estimate.','Compare rise/run assumptions before detailed design.'],commonPitfalls:['Using total rise as the riser height.','Forgetting that a straight stair often has one fewer tread than risers.','Skipping code and structural review.'],glossary:[{term:'Riser',definition:'The vertical height between adjacent treads.'},{term:'Tread',definition:'The horizontal walking surface of a step.'}],faqs:[{question:'Is this a permit-ready design?',answer:'No; verify local code, structure, headroom, landings, and safety requirements.'},{question:'Does it include a landing?',answer:'No; it models a simple straight run.'}],sources:[{title:'Residential Code Stairway Provisions',publisher:'International Code Council',url:'https://codes.iccsafe.org/',year:2026}],author:{name:'CalculatorFree Construction Review Team',credentials:'Calculation Review',description:'Original client-side layout estimate.'}},


  'resistor': {howToSteps:['Enter the first two resistor digits.','Enter the multiplier power of ten.','Enter tolerance percent.','Review nominal resistance and the tolerance range, then verify the physical band code before use.'],formula:{expression:'Nominal resistance = (10 × first digit + second digit) × 10^multiplier; range = nominal × (1 ± tolerance ÷ 100).',variables:[{symbol:'d1,d2',definition:'First and second significant digits from 0 to 9'},{symbol:'n',definition:'Multiplier exponent'},{symbol:'t',definition:'Tolerance percentage'}],notes:'This route models a four-band style numeric code and clamps digit/exponent inputs; real color codes and component tolerances should be checked against the datasheet.'},examples:[{title:'2.7 kΩ resistor',scenario:'Digits 2 and 7, multiplier 2, tolerance 5%.',steps:['Form 27 from the significant digits.','Multiply by 10².','Calculate a nominal 2700 Ω.','Apply ±5% for the range.'],result:'Nominal resistance is 2700 Ω, with a 2565–2835 Ω range.'}],useCases:['Decode a simple resistor band value.','Compare nominal value with a tolerance range.'],commonPitfalls:['Reading bands in the wrong direction.','Confusing multiplier with a significant digit.','Ignoring power rating and temperature coefficient.'],glossary:[{term:'Nominal resistance',definition:'The labeled target resistance before tolerance variation.'},{term:'Tolerance',definition:'Allowed percentage deviation from nominal value.'}],faqs:[{question:'Does this identify every resistor package?',answer:'No; it models a simple four-band numeric pattern and is not a datasheet replacement.'},{question:'Does it check resistor power rating?',answer:'No; power rating requires circuit and component information.'}],sources:[{title:'Resistor Color Code',publisher:'IEC',url:'https://www.iec.ch/',year:2026}],author:{name:'CalculatorFree Electrical Review Team',credentials:'Calculation Review',description:'Original client-side resistor estimate.'}},
  'ohms-law': {howToSteps:['Enter voltage in volts.','Enter current in amps.','Review resistance and electrical power.','Verify circuit limits and isolation before working on real equipment.'],formula:{expression:'Resistance = voltage ÷ current; power = voltage × current.',variables:[{symbol:'V',definition:'Voltage in volts'},{symbol:'I',definition:'Current in amps'},{symbol:'R',definition:'Resistance in ohms'},{symbol:'P',definition:'Power in watts'}],notes:'A zero current is guarded to zero resistance rather than returning infinity; real circuits may not be purely resistive.'},examples:[{title:'Simple DC load',scenario:'120 V and 2 A.',steps:['Divide voltage by current.','Calculate 60 Ω resistance.','Multiply voltage and current.','Calculate 240 W power.'],result:'The simplified result is 60 Ω and 240 W.'}],useCases:['Teach the relationship between voltage, current, resistance, and power.','Screen a simple resistive-load calculation.'],commonPitfalls:['Using AC RMS values without context.','Assuming a nonlinear device follows a fixed resistance.','Working on live circuits based only on a calculator output.'],glossary:[{term:'Ohm',definition:'SI unit of electrical resistance.'},{term:'RMS',definition:'Root-mean-square measure commonly used for AC voltage/current.'}],faqs:[{question:'Does this replace electrical safety procedures?',answer:'No; de-energize and use qualified procedures for real electrical work.'},{question:'What happens at zero current?',answer:'The widget returns guarded zero resistance and zero power.'}],sources:[{title:'Ohm’s Law',publisher:'NIST',url:'https://www.nist.gov/pml/owm/metric-si/si-units',year:2026}],author:{name:'CalculatorFree Electrical Review Team',credentials:'Calculation Review',description:'Original client-side educational model.'}},
  'electricity': {howToSteps:['Enter appliance power in watts.','Enter usage time in hours.','Enter electricity price per kWh.','Review energy use and estimated cost, then compare with the utility bill’s tariff structure.'],formula:{expression:'Energy = watts × hours ÷ 1000; cost = energy in kWh × price per kWh.',variables:[{symbol:'W',definition:'Appliance power in watts'},{symbol:'h',definition:'Usage duration in hours'},{symbol:'p',definition:'Price per kilowatt-hour'}],notes:'The result assumes constant power and one energy price; demand charges, taxes, tiers, and standby behavior are not modeled.'},examples:[{title:'Eight-hour appliance use',scenario:'A 1000 W device runs for 8 hours at $0.25/kWh.',steps:['Multiply 1000 W by 8 hours.','Divide by 1000 for 8 kWh.','Multiply by $0.25.','Review the estimated cost.'],result:'Energy use is 8 kWh and estimated cost is $2.00.'}],useCases:['Estimate appliance operating cost.','Compare usage scenarios before reviewing a utility bill.'],commonPitfalls:['Confusing watts with kilowatts.','Using a monthly price that is not per kWh.','Assuming nameplate power is constant in every operating mode.'],glossary:[{term:'Kilowatt-hour',definition:'A unit of energy equal to one kilowatt used for one hour.'},{term:'Demand charge',definition:'A bill component based on peak power rather than energy alone.'}],faqs:[{question:'Does this reproduce my full bill?',answer:'No; it calculates energy-only cost from a single price assumption.'},{question:'Does it include standby power?',answer:'Only if you include the applicable power and usage time.'}],sources:[{title:'Energy Saver — Estimating Appliance Energy Use',publisher:'U.S. Department of Energy',url:'https://www.energy.gov/energysaver/estimating-appliance-and-home-electronic-energy-use',year:2026}],author:{name:'CalculatorFree Energy Review Team',credentials:'Calculation Review',description:'Original client-side energy-cost estimate.'}},


  'mileage': {howToSteps:['Enter starting odometer miles.','Enter ending odometer miles.','Review trip mileage.','Check that both readings belong to the same vehicle and period.'],formula:{expression:'Trip mileage = max(0, ending odometer − starting odometer).',variables:[{symbol:'start',definition:'Beginning odometer reading in miles'},{symbol:'end',definition:'Ending odometer reading in miles'}],notes:'The guarded route prevents a negative distance; odometer rollovers, corrections, and unit changes need separate handling.'},examples:[{title:'Trip distance',scenario:'Starting reading 12,000 miles and ending reading 12,300 miles.',steps:['Enter 12,000.','Enter 12,300.','Subtract the starting reading.','Review 300 miles.'],result:'Trip mileage is 300 miles.'}],useCases:['Record simple trip distance.','Prepare distance input for a fuel-cost estimate.'],commonPitfalls:['Entering a lower reading after an odometer reset.','Mixing kilometers and miles.','Using trip distance as a measure of vehicle condition.'],glossary:[{term:'Odometer',definition:'Instrument that records vehicle distance traveled.'},{term:'Trip mileage',definition:'Difference between ending and starting readings in this model.'}],faqs:[{question:'Does it detect an odometer rollover?',answer:'No; a lower ending reading is guarded to zero.'},{question:'Does it calculate fuel economy?',answer:'No; use the Gas Mileage route with fuel consumed.'}],sources:[{title:'Fuel Economy Guide',publisher:'U.S. Department of Energy and EPA',url:'https://www.fueleconomy.gov/feg/guide.shtml',year:2026}],author:{name:'CalculatorFree Automotive Review Team',credentials:'Calculation Review',description:'Original client-side mileage difference.'}},
  'density': {howToSteps:['Enter mass in grams.','Enter volume in cubic centimeters.','Review density in grams per cubic centimeter.','Keep mass and volume measurements tied to the same sample.'],formula:{expression:'Density = mass ÷ volume.',variables:[{symbol:'m',definition:'Mass in grams'},{symbol:'V',definition:'Volume in cubic centimeters'}],notes:'A zero volume is guarded to zero rather than returning an infinite or undefined value.'},examples:[{title:'Sample density',scenario:'100 g sample occupying 20 cm³.',steps:['Enter 100 g.','Enter 20 cm³.','Divide mass by volume.','Review 5 g/cm³.'],result:'Density is 5 g/cm³.'}],useCases:['Compare material density measurements.','Teach the relationship between mass and volume.'],commonPitfalls:['Confusing mass with weight.','Mixing cm³ with m³ without conversion.','Using an empty or zero volume.'],glossary:[{term:'Density',definition:'Mass per unit volume.'},{term:'Cubic centimeter',definition:'A volume unit equal to one milliliter.'}],faqs:[{question:'Does it calculate weight?',answer:'No; density uses mass and volume, while weight also depends on gravitational acceleration.'},{question:'What if volume is zero?',answer:'The guarded result is zero.'}],sources:[{title:'SI Units and Measurement',publisher:'NIST',url:'https://www.nist.gov/pml/owm/metric-si/si-units',year:2026}],author:{name:'CalculatorFree Science Review Team',credentials:'Calculation Review',description:'Original client-side density calculation.'}},
  'mass': {howToSteps:['Enter mass in kilograms.','Review kilograms, grams, and pounds.','Keep the unit with each result.','Use a suitable scale and conversion convention for real measurements.'],formula:{expression:'grams = kilograms × 1000; pounds = kilograms × 2.20462262185.',variables:[{symbol:'kg',definition:'Input mass in kilograms'},{symbol:'2.20462262185',definition:'Kilograms-to-pound conversion factor used here'}],notes:'This is a mass conversion; it does not calculate force in newtons.'},examples:[{title:'Metric to imperial mass',scenario:'Convert 70 kg.',steps:['Enter 70 kg.','Multiply by 1000 for grams.','Multiply by 2.20462262185 for pounds.','Review the labeled values.'],result:'70 kg = 70,000 g and approximately 154.324 lb.'}],useCases:['Convert mass units for recipes, shipping, and technical notes.','Prepare consistent inputs for a calculation.'],commonPitfalls:['Calling pounds a force in every context.','Mixing avoirdupois pounds with another convention.','Rounding before conversion.'],glossary:[{term:'Mass',definition:'Amount of matter, independent of local gravitational acceleration.'},{term:'Kilogram',definition:'SI base unit of mass.'}],faqs:[{question:'Is this a weight calculator?',answer:'No; this route converts mass units.'},{question:'Does it use a server?',answer:'No; conversion runs client-side.'}],sources:[{title:'The International System of Units (SI)',publisher:'BIPM',url:'https://www.bipm.org/en/measurement-units',year:2026}],author:{name:'CalculatorFree Measurement Review Team',credentials:'Calculation Review',description:'Original client-side mass conversion.'}},
  'weight': {howToSteps:['Enter mass in kilograms.','Review gravitational force in newtons.','Keep the local gravity assumption in mind.','Do not substitute this force result for a scale reading in pounds.'],formula:{expression:'Weight force = mass × standard gravitational acceleration, W = m × 9.80665.',variables:[{symbol:'m',definition:'Mass in kilograms'},{symbol:'g',definition:'Standard gravity, 9.80665 m/s²'},{symbol:'W',definition:'Force in newtons'}],notes:'The route uses standard gravity; local effective gravity varies slightly and pounds-force require a separate conversion.'},examples:[{title:'Weight force',scenario:'A 70 kg mass under standard gravity.',steps:['Enter 70 kg.','Multiply by 9.80665 m/s².','Keep the result in newtons.','Distinguish force from mass.'],result:'Weight force is approximately 686.466 N.'}],useCases:['Teach the distinction between mass and force.','Estimate gravitational force in a simple physics problem.'],commonPitfalls:['Using weight and mass as interchangeable terms.','Assuming standard gravity is identical everywhere.','Comparing newtons directly with pounds without conversion.'],glossary:[{term:'Newton',definition:'SI unit of force, equal to kg·m/s².'},{term:'Standard gravity',definition:'Reference acceleration of 9.80665 m/s².'}],faqs:[{question:'Does this calculate body weight for health?',answer:'No; it is a physics force conversion and not a health assessment.'},{question:'Does it use local gravity?',answer:'No; it uses standard gravity.'}],sources:[{title:'Standard Acceleration of Gravity',publisher:'BIPM',url:'https://www.bipm.org/en/measurement-units',year:2026}],author:{name:'CalculatorFree Physics Review Team',credentials:'Calculation Review',description:'Original client-side force estimate.'}},
  'speed': {howToSteps:['Enter distance in miles.','Enter elapsed time in hours.','Review average speed in miles per hour.','Use consistent distance and time units.'],formula:{expression:'Average speed = distance ÷ elapsed time.',variables:[{symbol:'d',definition:'Distance in miles'},{symbol:'t',definition:'Elapsed time in hours'}],notes:'Zero elapsed time is guarded to zero; the result is an average, not an instantaneous speed.'},examples:[{title:'Average travel speed',scenario:'Travel 60 miles in 2 hours.',steps:['Enter 60 miles.','Enter 2 hours.','Divide distance by time.','Review 30 mph.'],result:'Average speed is 30 mph.'}],useCases:['Estimate average travel speed.','Teach distance-time relationships.'],commonPitfalls:['Confusing average speed with speed limit or instantaneous speed.','Mixing minutes with hours.','Using a zero or stopped-time interval.'],glossary:[{term:'Average speed',definition:'Total distance divided by total elapsed time.'},{term:'Mile per hour',definition:'Distance of one mile traveled per hour.'}],faqs:[{question:'Does it calculate pace?',answer:'No; it reports miles per hour, while pace reports time per distance.'},{question:'What if time is zero?',answer:'The guarded result is zero.'}],sources:[{title:'SI Units and Measurement',publisher:'NIST',url:'https://www.nist.gov/pml/owm/metric-si/si-units',year:2026}],author:{name:'CalculatorFree Physics Review Team',credentials:'Calculation Review',description:'Original client-side average-speed calculation.'}},


  'molarity': {howToSteps:['Enter solute amount in moles.','Enter solution volume in liters.','Review molarity in moles per liter.','Keep amount and volume measured for the same solution.'],formula:{expression:'Molarity = moles of solute ÷ liters of solution.',variables:[{symbol:'n',definition:'Amount of solute in moles'},{symbol:'V',definition:'Final solution volume in liters'}],notes:'A zero volume is guarded to zero; dilution, temperature, and measurement uncertainty are not modeled.'},examples:[{title:'Simple concentration',scenario:'0.5 mol dissolved to a final volume of 2 L.',steps:['Enter 0.5 mol.','Enter 2 L.','Divide amount by volume.','Review 0.25 mol/L.'],result:'Molarity is 0.25 mol/L.'}],useCases:['Check introductory solution-concentration exercises.','Compare concentration scenarios with explicit units.'],commonPitfalls:['Using solvent volume instead of final solution volume.','Mixing milliliters and liters.','Treating a calculated concentration as a safety or dosing instruction.'],glossary:[{term:'Mole',definition:'SI amount-of-substance unit.'},{term:'Molarity',definition:'Moles of solute per liter of solution.'}],faqs:[{question:'Does it convert milliliters automatically?',answer:'No; enter liters as labeled.'},{question:'Does it provide laboratory safety guidance?',answer:'No; follow the applicable lab procedures and safety data.'}],sources:[{title:'SI Units and the Mole',publisher:'BIPM',url:'https://www.bipm.org/en/measurement-units',year:2026}],author:{name:'CalculatorFree Chemistry Review Team',credentials:'Calculation Review',description:'Original client-side concentration calculation.'}},
  'molecular-weight': {howToSteps:['Enter a supported chemical formula such as H2O or NaCl.','Review the parsed molar mass.','Check capitalization and subscripts.','Verify the element composition against a trusted reference for laboratory work.'],formula:{expression:'Molar mass = sum of (element atomic weight × atom count) across the parsed formula.',variables:[{symbol:'element',definition:'Supported chemical element symbol'},{symbol:'count',definition:'Positive integer subscript, defaulting to one'},{symbol:'atomic weight',definition:'Element weight used by this calculator’s lookup table'}],notes:'This compact route supports a curated element set and simple formulas without parentheses; unsupported or malformed formulas are rejected.'},examples:[{title:'Water molar mass',scenario:'Parse H2O.',steps:['Read two hydrogen atoms.','Read one oxygen atom.','Multiply counts by the lookup weights.','Add the contributions.'],result:'Molar mass is approximately 18.015 g/mol.'}],useCases:['Support basic stoichiometry exercises.','Check simple formula mass with explicit element counts.'],commonPitfalls:['Using lowercase element symbols.','Expecting parentheses or hydrates to be parsed.','Using rounded atomic weights for high-precision laboratory work.'],glossary:[{term:'Molar mass',definition:'Mass per mole of a substance, commonly expressed in g/mol.'},{term:'Chemical formula',definition:'Notation that identifies elements and atom counts in a substance.'}],faqs:[{question:'Does it support every element?',answer:'No; the current lookup table is deliberately limited to common symbols.'},{question:'Does it parse parentheses?',answer:'No; use a simple sequence of element symbols and positive integer subscripts.'}],sources:[{title:'Atomic Weights and Isotopic Compositions',publisher:'NIST',url:'https://physics.nist.gov/cgi-bin/Compositions/stand_alone.pl',year:2026}],author:{name:'CalculatorFree Chemistry Review Team',credentials:'Calculation Review',description:'Original client-side formula parser and mass sum.'}},
  'roman-numeral-converter': {howToSteps:['Enter an integer from 1 through 3999.','Review its canonical Roman numeral form.','Optionally enter a Roman numeral to decode.','Use the result as a notation conversion, not as a numeric parser for arbitrary text.'],formula:{expression:'Greedy Roman representation uses descending values with subtractive pairs such as IV, IX, XL, XC, CD, and CM.',variables:[{symbol:'n',definition:'Integer in the supported 1–3999 range'},{symbol:'I,V,X,L,C,D,M',definition:'Roman numeral symbols'}],notes:'The encoder returns canonical notation for the supported range; the decoder sums valid symbols but does not enforce every canonical ordering rule.'},examples:[{title:'Year conversion',scenario:'Convert 2026.',steps:['Take two thousands as MM.','Take two tens as XX.','Take six as VI.','Concatenate the symbols.'],result:'2026 is MMXXVI.'}],useCases:['Convert historical-style integers to Roman notation.','Practice place-value and subtractive notation.'],commonPitfalls:['Entering zero, which has no standard Roman numeral.','Using lowercase and assuming every malformed sequence is canonical.','Expecting values above 3999 without an extension convention.'],glossary:[{term:'Subtractive notation',definition:'A smaller symbol before a larger one, such as IV for four.'},{term:'Canonical form',definition:'The standard compact spelling returned by the encoder.'}],faqs:[{question:'Can it encode zero?',answer:'No; the supported range is 1 through 3999.'},{question:'Does decoding validate canonical order?',answer:'It validates symbols and computes a value, but the route is not a full canonical-string validator.'}],sources:[{title:'Roman Numerals',publisher:'Encyclopaedia Britannica',url:'https://www.britannica.com/topic/Roman-numeral',year:2026}],author:{name:'CalculatorFree Math Review Team',credentials:'Calculation Review',description:'Original client-side numeral conversion.'}},


  'golf-handicap': {howToSteps:['Enter the gross score.','Enter the course rating.','Enter the slope rating.','Review the one-round handicap differential and do not confuse it with an official index.'],formula:{expression:'Handicap differential = (score − course rating) × 113 ÷ slope rating.',variables:[{symbol:'score',definition:'Gross score for the round'},{symbol:'course rating',definition:'Expected score for a scratch golfer on the course and tees'},{symbol:'slope rating',definition:'Relative difficulty rating used by the handicap system'}],notes:'An official handicap index uses an authorized scoring history and applicable rules; this route calculates one differential only.'},examples:[{title:'One-round differential',scenario:'Score 90, course rating 72, slope 113.',steps:['Subtract course rating from score.','Multiply the difference by 113.','Divide by slope rating 113.','Review an 18.00 differential.'],result:'The one-round differential is 18.00.'}],useCases:['Explain the relationship among score, course rating, and slope.','Compare round differentials for practice.'],commonPitfalls:['Calling one differential an official handicap index.','Entering a net score instead of gross score.','Using a slope rating of zero or the wrong tees.'],glossary:[{term:'Course rating',definition:'Expected score for a scratch player under the rating system.'},{term:'Slope rating',definition:'Relative difficulty measure compared with a standard course.'}],faqs:[{question:'Is this an official handicap?',answer:'No; it is one differential and does not replace an authorized handicap calculation.'},{question:'Does it account for playing conditions?',answer:'No; only the three entered values are used.'}],sources:[{title:'World Handicap System',publisher:'USGA and R&A',url:'https://www.usga.org/handicapping/roh/2024-rules-of-handicapping.html',year:2024}],author:{name:'CalculatorFree Golf Review Team',credentials:'Calculation Review',description:'Original client-side educational differential.'}},
  'tire-size': {howToSteps:['Enter tire section width in millimeters.','Enter aspect ratio as a percent.','Enter wheel diameter in inches.','Review sidewall, overall diameter, circumference, and revolutions per mile.'],formula:{expression:'Sidewall = width × aspect ratio ÷ 100; diameter = 2 × sidewall + wheel diameter × 25.4.',variables:[{symbol:'width',definition:'Tire section width in millimeters'},{symbol:'aspect',definition:'Sidewall height as a percentage of width'},{symbol:'rim',definition:'Wheel diameter in inches'}],notes:'The geometric result ignores tread wear, load, inflation, and manufacturer measurement variation.'},examples:[{title:'205/55R16',scenario:'205 mm width, 55% aspect ratio, 16-inch wheel.',steps:['Calculate sidewall as 205 × 0.55.','Convert the 16-inch wheel to millimeters.','Add twice the sidewall to the wheel diameter.','Calculate circumference and revolutions per mile.'],result:'Overall diameter is about 631.9 mm.'}],useCases:['Compare tire sizes for geometric diameter.','Estimate circumference and speedometer-related differences before professional fitment review.'],commonPitfalls:['Confusing aspect ratio with sidewall millimeters.','Ignoring wheel width and clearance.','Using the result as a guarantee of fit or legal compliance.'],glossary:[{term:'Section width',definition:'Nominal tire width in millimeters.'},{term:'Aspect ratio',definition:'Sidewall height expressed as a percentage of section width.'}],faqs:[{question:'Does it guarantee fitment?',answer:'No; clearance, load rating, wheel width, and vehicle specifications also matter.'},{question:'Does it include tread wear?',answer:'No; it uses nominal geometric dimensions.'}],sources:[{title:'Tire and Rim Association Standards',publisher:'Tire and Rim Association',url:'https://www.us-tra.org/',year:2026}],author:{name:'CalculatorFree Automotive Review Team',credentials:'Calculation Review',description:'Original client-side tire geometry estimate.'}},


  'sleep': {howToSteps:['Enter bedtime using the 24-hour clock.','Enter wake time using the 24-hour clock.','Review the overnight or same-day duration.','Use the estimate as a planning aid, not as a diagnosis or treatment recommendation.'],formula:{expression:'Sleep duration = (wake minutes − bedtime minutes + 1440) mod 1440; a zero remainder is treated as 1440 minutes.',variables:[{symbol:'bedtime',definition:'Clock time when sleep begins'},{symbol:'wake time',definition:'Clock time when waking occurs'},{symbol:'1440',definition:'Minutes in one day'}],notes:'The calculator estimates elapsed clock time only; it does not assess sleep quality, sleep disorders, or individual clinical needs.'},examples:[{title:'Overnight duration',scenario:'Bedtime 22:30 and wake time 06:30.',steps:['Convert 22:30 to 1350 minutes.','Convert 06:30 to 390 minutes.','Add 1440 before taking the difference.','Review 480 minutes, or 8 hours.'],result:'Estimated sleep duration is 8 hours.'}],useCases:['Plan a simple sleep window.','Check elapsed time across midnight.'],commonPitfalls:['Forgetting that wake time may be on the following day.','Assuming time in bed equals time asleep.','Using the estimate to self-diagnose a sleep condition.'],glossary:[{term:'Sleep duration',definition:'Elapsed clock time between the entered bedtime and wake time.'},{term:'Sleep quality',definition:'A broader concept not measured by this calculator.'}],faqs:[{question:'Does this diagnose insomnia or another disorder?',answer:'No; it only calculates elapsed clock time.'},{question:'What should I do about persistent sleep problems?',answer:'Discuss persistent or concerning symptoms with a qualified healthcare professional.'}],sources:[{title:'Sleep and Sleep Disorders',publisher:'Centers for Disease Control and Prevention',url:'https://www.cdc.gov/sleep/',year:2026}],author:{name:'CalculatorFree Health Review Team',credentials:'Calculation Review',description:'Original client-side duration estimate; not medical advice.'}},
  'roofing': {howToSteps:['Enter the rectangular footprint length and width.','Enter a roof pitch factor.','Enter waste allowance.','Review base, pitched, and order areas, then confirm measurements with a qualified contractor.'],formula:{expression:'Base area = length × width; pitched area = base area × pitch factor; order area = pitched area × (1 + waste percent ÷ 100).',variables:[{symbol:'L,W',definition:'Footprint length and width in feet'},{symbol:'p',definition:'Dimensionless pitch factor'},{symbol:'w',definition:'Waste allowance percentage'}],notes:'This is a simplified rectangular estimate; valleys, hips, overhangs, dormers, multiple planes, and local material coverage are not modeled.'},examples:[{title:'Simple pitched roof',scenario:'20 ft by 10 ft footprint, pitch factor 1.1, waste 10%.',steps:['Calculate 200 ft² footprint.','Apply 1.1 for 220 ft² pitched area.','Apply 10% waste for 242 ft² order area.','Verify the takeoff in a professional survey.'],result:'Estimated order area is 242 ft².'}],useCases:['Create an early material-area estimate.','Compare pitch and waste assumptions.'],commonPitfalls:['Using floor area as the complete roof area.','Ignoring roof features and overhangs.','Treating the result as a final material takeoff.'],glossary:[{term:'Pitch factor',definition:'A multiplier used here to approximate sloped surface area.'},{term:'Waste allowance',definition:'Additional percentage for cuts, breakage, and installation waste.'}],faqs:[{question:'Does it calculate bundles or sheets?',answer:'No; coverage depends on the chosen product and packaging.'},{question:'Is it permit-ready?',answer:'No; verify structure, code, flashing, and material takeoff requirements.'}],sources:[{title:'Residential Roofing Materials',publisher:'International Code Council',url:'https://codes.iccsafe.org/',year:2026}],author:{name:'CalculatorFree Construction Review Team',credentials:'Calculation Review',description:'Original client-side area estimate.'}},
  'tile': {howToSteps:['Enter floor length and width in feet.','Enter tile length and width in inches.','Review floor area and whole-tile quantity.','Include layout cuts, breakage, and manufacturer guidance in the final order.'],formula:{expression:'Tile area in ft² = tile length × tile width ÷ 144; tiles = ceil(floor area ÷ tile area × (1 + waste percent ÷ 100)).',variables:[{symbol:'L,W',definition:'Floor dimensions in feet'},{symbol:'l,w',definition:'Tile dimensions in inches'}],notes:'The widget uses a fixed 10% waste allowance for the displayed estimate; complex layouts may need more.'},examples:[{title:'One-hundred-square-foot floor',scenario:'10 ft by 10 ft floor with 12 in by 12 in tiles.',steps:['Calculate 100 ft² floor area.','Convert a 12 by 12 inch tile to 1 ft².','Apply 10% waste.','Round up to a whole tile.'],result:'Estimated quantity is 110 tiles.'}],useCases:['Estimate a first-pass tile count.','Compare tile sizes for a rectangular room.'],commonPitfalls:['Forgetting grout joints and cuts.','Ordering the exact mathematical minimum.','Mixing inches and feet.'],glossary:[{term:'Tile area',definition:'Area covered by one tile after unit conversion.'},{term:'Waste allowance',definition:'Extra tiles reserved for cuts, breakage, and future repairs.'}],faqs:[{question:'Does it include pattern matching?',answer:'No; patterned layouts may require additional waste.'},{question:'Should I order exactly this number?',answer:'Use it as an estimate and confirm with the product and installer guidance.'}],sources:[{title:'Tile Installation Guidelines',publisher:'Tile Council of North America',url:'https://www.tcnatile.com/',year:2026}],author:{name:'CalculatorFree Construction Review Team',credentials:'Calculation Review',description:'Original client-side tile quantity estimate.'}},
  'mulch': {howToSteps:['Enter bed length in feet.','Enter bed width in feet.','Enter mulch depth in inches.','Review cubic yards and compare with supplier coverage.'],formula:{expression:'Mulch volume in cubic yards = length × width × depth in inches ÷ 324.',variables:[{symbol:'L,W',definition:'Bed dimensions in feet'},{symbol:'d',definition:'Depth in inches'},{symbol:'324',definition:'Conversion constant from ft²-inches to cubic yards'}],notes:'The estimate assumes a rectangular, level bed and does not account for settling, irregular edges, or delivery minimums.'},examples:[{title:'Three-inch bed',scenario:'20 ft by 10 ft area at 3 inches depth.',steps:['Multiply 20 by 10 for 200 ft².','Multiply by 3 inches.','Divide by 324.','Review approximately 1.85 yd³.'],result:'Estimated volume is about 1.85 cubic yards.'}],useCases:['Estimate landscape mulch volume.','Compare depth assumptions before ordering.'],commonPitfalls:['Using inches as if they were feet.','Ignoring settling and irregular boundaries.','Confusing cubic yards with bags.'],glossary:[{term:'Cubic yard',definition:'A volume unit equal to 27 cubic feet.'},{term:'Depth',definition:'Planned material thickness, entered in inches here.'}],faqs:[{question:'Does it include settling?',answer:'No; add a practical allowance based on material and supplier advice.'},{question:'Can it count bags?',answer:'No; bag conversion needs each bag’s stated volume.'}],sources:[{title:'Landscape Material Estimating',publisher:'University Extension Services',url:'https://extension.umn.edu/yard-and-garden',year:2026}],author:{name:'CalculatorFree Landscaping Review Team',credentials:'Calculation Review',description:'Original client-side volume estimate.'}},
  'gravel': {howToSteps:['Enter project length in feet.','Enter project width in feet.','Enter gravel depth in inches.','Review cubic yards and confirm density/tonnage with the supplier.'],formula:{expression:'Gravel volume in cubic yards = length × width × depth in inches ÷ 324.',variables:[{symbol:'L,W',definition:'Project dimensions in feet'},{symbol:'d',definition:'Planned depth in inches'},{symbol:'324',definition:'Conversion constant to cubic yards'}],notes:'This route estimates volume only; tons depend on aggregate density, moisture, compaction, and supplier specifications.'},examples:[{title:'Gravel path base',scenario:'20 ft by 10 ft area at 3 inches depth.',steps:['Calculate 200 ft².','Multiply by 3 inches.','Divide by 324.','Ask the supplier to convert volume to delivered weight.'],result:'Estimated volume is about 1.85 cubic yards.'}],useCases:['Estimate aggregate volume for a path or bed.','Compare depth scenarios before requesting a delivery quote.'],commonPitfalls:['Ordering tons from volume without density.','Ignoring compaction and subgrade.','Using a depth that is not uniform.'],glossary:[{term:'Aggregate',definition:'Granular material such as gravel used for construction or landscaping.'},{term:'Compaction',definition:'Reduction in volume as material is mechanically or naturally compressed.'}],faqs:[{question:'Does it calculate tons?',answer:'No; supplier-specific density is needed to convert volume to weight.'},{question:'Does it design a road base?',answer:'No; drainage, subgrade, load, and engineering requirements are outside scope.'}],sources:[{title:'Aggregates and Granular Materials',publisher:'U.S. Geological Survey',url:'https://www.usgs.gov/centers/national-minerals-information-center',year:2026}],author:{name:'CalculatorFree Construction Review Team',credentials:'Calculation Review',description:'Original client-side aggregate volume estimate.'}},


  'wind-chill': {howToSteps:['Enter air temperature in °F.','Enter wind speed in mph.','Review the calculated apparent temperature.','Use official local weather guidance for exposure decisions.'],formula:{expression:'Wind chill = 35.74 + 0.6215T − 35.75V^0.16 + 0.4275TV^0.16.',variables:[{symbol:'T',definition:'Air temperature in °F'},{symbol:'V',definition:'Wind speed in mph'}],notes:'The commonly used formula applies for cold air and sufficient wind; outside that range the widget reports air temperature and marks the formula not applicable.'},examples:[{title:'Cold windy conditions',scenario:'20°F air temperature and 15 mph wind.',steps:['Check that temperature and wind are in the common formula range.','Raise wind speed to the 0.16 power.','Substitute T and V.','Review an apparent temperature near 6.2°F.'],result:'Wind chill is approximately 6.2°F.'}],useCases:['Explain apparent temperature in cold wind.','Compare exposure scenarios using consistent units.'],commonPitfalls:['Using the formula in warm or nearly calm conditions.','Confusing wind chill with actual air temperature.','Treating a calculator estimate as a substitute for weather alerts.'],glossary:[{term:'Wind chill',definition:'Apparent cooling effect of wind on exposed skin under a defined formula.'},{term:'Apparent temperature',definition:'A perceived-temperature estimate rather than a direct thermometer reading.'}],faqs:[{question:'Does wind chill lower the actual temperature?',answer:'No; it describes the effect of wind on heat loss and apparent cold.'},{question:'Does it replace official warnings?',answer:'No; consult current local forecasts and safety guidance.'}],sources:[{title:'Wind Chill Temperature Index',publisher:'National Weather Service',url:'https://www.weather.gov/safety/cold-wind-chill-chart',year:2026}],author:{name:'CalculatorFree Weather Review Team',credentials:'Calculation Review',description:'Original client-side apparent-temperature estimate.'}},
  'heat-index': {howToSteps:['Enter air temperature in °F.','Enter relative humidity.','Review heat index when the formula range applies.','Follow current heat advisories and hydration guidance from local authorities.'],formula:{expression:'The Rothfusz regression estimates heat index from temperature and relative humidity using its published polynomial coefficients.',variables:[{symbol:'T',definition:'Air temperature in °F'},{symbol:'RH',definition:'Relative humidity in percent'}],notes:'The widget applies the polynomial in a common hot/humid range and otherwise reports air temperature as not applicable.'},examples:[{title:'Hot and humid conditions',scenario:'95°F and 60% relative humidity.',steps:['Confirm hot/humid applicability.','Substitute T=95 and RH=60.','Evaluate the regression terms.','Review approximately 113.1°F.'],result:'Heat index is approximately 113.1°F.'}],useCases:['Explain how humidity changes perceived heat.','Compare hot-weather scenarios with consistent units.'],commonPitfalls:['Confusing heat index with actual air temperature.','Using relative humidity as a fraction instead of percent.','Ignoring official heat warnings and individual vulnerability.'],glossary:[{term:'Heat index',definition:'Apparent temperature estimate combining heat and humidity.'},{term:'Relative humidity',definition:'Amount of water vapor relative to the air’s capacity at that temperature.'}],faqs:[{question:'Does it predict health outcomes?',answer:'No; it is an apparent-temperature estimate, not a medical prediction.'},{question:'What should I do during extreme heat?',answer:'Follow current local authority guidance and seek help for concerning symptoms.'}],sources:[{title:'Heat Index',publisher:'National Weather Service',url:'https://www.weather.gov/safety/heat-index',year:2026}],author:{name:'CalculatorFree Weather Review Team',credentials:'Calculation Review',description:'Original client-side apparent-temperature estimate.'}},
  'dew-point': {howToSteps:['Enter air temperature in °C.','Enter relative humidity.','Review the dew-point approximation.','Use a calibrated instrument for critical humidity or condensation work.'],formula:{expression:'Magnus approximation: γ = ln(RH/100) + (17.625T)/(243.04+T); dew point = 243.04γ/(17.625−γ).',variables:[{symbol:'T',definition:'Air temperature in °C'},{symbol:'RH',definition:'Relative humidity in percent'}],notes:'The route clamps humidity to a small positive value to avoid logarithm errors and reports an approximation.'},examples:[{title:'Indoor humidity estimate',scenario:'20°C and 60% relative humidity.',steps:['Calculate the Magnus intermediate γ.','Substitute γ into the dew-point expression.','Keep temperature in Celsius.','Review approximately 12°C.'],result:'Dew point is approximately 12.0°C.'}],useCases:['Estimate condensation-related dew point.','Explain the connection between temperature and humidity.'],commonPitfalls:['Entering Fahrenheit while the route expects Celsius.','Treating relative humidity percent as a decimal.','Using dew point as a direct mold or building diagnosis.'],glossary:[{term:'Dew point',definition:'Temperature at which air reaches saturation under the approximation used.'},{term:'Saturation',definition:'Condition where air holds as much water vapor as represented by the model.'}],faqs:[{question:'Does a high dew point always mean condensation?',answer:'No; surface temperature and local conditions also matter.'},{question:'Is this a weather-station replacement?',answer:'No; it is a formula estimate from entered values.'}],sources:[{title:'Dew Point and Relative Humidity',publisher:'National Weather Service',url:'https://www.weather.gov/lmk/humidity',year:2026}],author:{name:'CalculatorFree Weather Review Team',credentials:'Calculation Review',description:'Original client-side Magnus dew-point approximation.'}},


  'bandwidth': {howToSteps:['Enter transferred data in megabytes.','Enter transfer duration in seconds.','Review megabits per second and megabytes per second.','Keep decimal/binary unit conventions consistent when comparing providers.'],formula:{expression:'MB/s = data MB ÷ seconds; Mbps = MB/s × 8.',variables:[{symbol:'D',definition:'Transferred data in megabytes'},{symbol:'t',definition:'Elapsed transfer time in seconds'}],notes:'This is a simple average throughput model and does not include protocol overhead, bursts, latency, or binary mebibyte conventions.'},examples:[{title:'100 MB in 10 seconds',scenario:'Transfer 100 MB over 10 seconds.',steps:['Divide 100 MB by 10 seconds.','Get 10 MB/s.','Multiply by 8 bits per byte.','Get 80 Mbps.'],result:'Average throughput is 80 Mbps, or 10 MB/s.'}],useCases:['Compare simple transfer rates.','Convert between MB/s and Mbps.'],commonPitfalls:['Confusing megabits with megabytes.','Ignoring network overhead.','Comparing peak and sustained rates as if identical.'],glossary:[{term:'Mbps',definition:'Megabits per second.'},{term:'MB/s',definition:'Megabytes per second.'}],faqs:[{question:'Does it measure my live connection?',answer:'No; it calculates from values you enter.'},{question:'Does it include overhead?',answer:'No; it is an idealized average.'}],sources:[{title:'Units of Information',publisher:'NIST',url:'https://www.nist.gov/pml/owm/metric-si/si-units',year:2026}],author:{name:'CalculatorFree Web Tools Review Team',credentials:'Calculation Review',description:'Original client-side throughput conversion.'}},
  'base64-encode-decode': {howToSteps:['Enter text or a Base64 string.','Review the deterministic encoded and decoded fields.','Use UTF-8 text for predictable results.','Do not treat Base64 as encryption or secrecy.'],formula:{expression:'Base64 maps groups of bytes to a 64-character alphabet and uses padding when needed.',variables:[{symbol:'input',definition:'Text passed to the encoder or decoder'},{symbol:'alphabet',definition:'Base64 symbols A–Z, a–z, 0–9, +, and /'}],notes:'Base64 is an encoding for representation and transport; it does not provide confidentiality.'},examples:[{title:'Encode Hello',scenario:'Enter Hello.',steps:['Convert the UTF-8 bytes to six-bit groups.','Map groups to the Base64 alphabet.','Add padding if needed.','Review SGVsbG8=.'],result:'Hello encodes as SGVsbG8=.'}],useCases:['Inspect API or data transport representations.','Decode a known Base64 payload for debugging.'],commonPitfalls:['Calling encoding encryption.','Sharing sensitive data in an online tool.','Mixing URL-safe and standard Base64 alphabets.'],glossary:[{term:'Encoding',definition:'A reversible representation transform, not protection.'},{term:'Padding',definition:'Equals signs used to complete a Base64 block.'}],faqs:[{question:'Is Base64 secure?',answer:'No; anyone with the encoded text can decode it.'},{question:'Does the tool send text to a server?',answer:'No; this calculator runs client-side.'}],sources:[{title:'The Base16, Base32, and Base64 Data Encodings',publisher:'IETF RFC 4648',url:'https://www.rfc-editor.org/rfc/rfc4648',year:2006}],author:{name:'CalculatorFree Web Tools Review Team',credentials:'Calculation Review',description:'Original client-side encoding helper.'}},
  'url-encode-decode': {howToSteps:['Enter text or a percent-encoded component.','Review the encoded and decoded fields.','Use the output as a URI component, not a full URL policy validator.','Check reserved characters in the destination context.'],formula:{expression:'URL component encoding replaces characters with percent-encoded UTF-8 bytes; decoding reverses that mapping.',variables:[{symbol:'input',definition:'Text or encoded URI component'},{symbol:'%',definition:'Prefix for a hexadecimal byte escape'}],notes:'The route uses component semantics and does not validate an entire URL or its security.'},examples:[{title:'Encode a query fragment',scenario:'Enter a b?.',steps:['Encode the space.','Encode the question mark as a reserved character.','Keep the result as one URI component.','Review a%20b%3F.'],result:'The encoded component is a%20b%3F.'}],useCases:['Prepare query parameters.','Inspect percent-encoded text during web debugging.'],commonPitfalls:['Encoding an entire URL when only a component should be encoded.','Double-encoding existing escapes.','Assuming encoding sanitizes untrusted HTML or script input.'],glossary:[{term:'URI component',definition:'A portion of a URI encoded independently from the full URL.'},{term:'Percent encoding',definition:'Representation of a byte as % followed by two hexadecimal digits.'}],faqs:[{question:'Does it validate URLs?',answer:'No; it encodes or decodes component text only.'},{question:'Is encoding a security filter?',answer:'No; use context-appropriate validation and output encoding.'}],sources:[{title:'Uniform Resource Identifier: Generic Syntax',publisher:'IETF RFC 3986',url:'https://www.rfc-editor.org/rfc/rfc3986',year:2005}],author:{name:'CalculatorFree Web Tools Review Team',credentials:'Calculation Review',description:'Original client-side URI component helper.'}},
  'time-duration': {howToSteps:['Enter hours.','Enter minutes.','Enter seconds.','Review normalized duration and total seconds.'],formula:{expression:'Total seconds = hours × 3600 + minutes × 60 + seconds; normalized fields are obtained by division and remainder.',variables:[{symbol:'h',definition:'Input hours'},{symbol:'m',definition:'Input minutes'},{symbol:'s',definition:'Input seconds'}],notes:'The route accepts non-negative values and normalizes minutes/seconds above 59 into the total duration.'},examples:[{title:'Normalize mixed units',scenario:'1 hour, 61 minutes, and 62 seconds.',steps:['Convert hours to 3600 seconds.','Convert minutes and seconds.','Add to 7322 seconds.','Normalize to 2 hours, 2 minutes, 2 seconds.'],result:'Normalized duration is 2:02:02.'}],useCases:['Normalize elapsed time.','Prepare durations for schedules or media metadata.'],commonPitfalls:['Treating minutes as decimal hours.','Forgetting that 60 seconds carry into a minute.','Mixing duration with a clock timestamp.'],glossary:[{term:'Elapsed time',definition:'Amount of time between two events.'},{term:'Remainder',definition:'Part left after extracting whole hours or minutes.'}],faqs:[{question:'Does it calculate a clock time?',answer:'No; it calculates a duration.'},{question:'Can minutes exceed 59?',answer:'Yes; the result normalizes them.'}],sources:[{title:'SI Units and Time',publisher:'BIPM',url:'https://www.bipm.org/en/measurement-units',year:2026}],author:{name:'CalculatorFree Time Tools Review Team',credentials:'Calculation Review',description:'Original client-side duration normalization.'}},
  'day-of-the-week': {howToSteps:['Enter a Gregorian date in ISO YYYY-MM-DD format.','Review the weekday name.','Check the date before relying on it for a schedule.','Remember that historical calendar conventions may differ.'],formula:{expression:'The date is parsed at UTC midnight and formatted with the weekday name using the Gregorian calendar implementation.',variables:[{symbol:'date',definition:'Valid ISO calendar date'},{symbol:'UTC',definition:'Coordinated Universal Time used to avoid local midnight shifts'}],notes:'The route validates the ISO date string and returns a weekday in English; it does not model historical calendar reforms.'},examples:[{title:'ISO date lookup',scenario:'Enter 2026-08-26.',steps:['Validate year-month-day shape.','Create UTC midnight for the date.','Format the weekday.','Review Wednesday.'],result:'2026-08-26 is Wednesday.'}],useCases:['Check a weekday for a known date.','Support simple calendar exercises.'],commonPitfalls:['Using locale-specific date order.','Entering an impossible date.','Ignoring time zones when combining a date with a timestamp.'],glossary:[{term:'ISO 8601 date',definition:'Year-month-day date notation such as 2026-08-26.'},{term:'Gregorian calendar',definition:'Calendar system used for the modern date calculation.'}],faqs:[{question:'Does it include time zones?',answer:'It treats the date at UTC midnight for stable weekday output.'},{question:'Does it support ancient calendar reforms?',answer:'No; it uses the standard modern date implementation.'}],sources:[{title:'Date and Time on the Internet: Timestamps',publisher:'IETF RFC 3339',url:'https://www.rfc-editor.org/rfc/rfc3339',year:2002}],author:{name:'CalculatorFree Time Tools Review Team',credentials:'Calculation Review',description:'Original client-side weekday lookup.'}},

  'random-number': {howToSteps:['Set the inclusive minimum and maximum integers.','Choose how many values to generate and whether repeats are allowed.','Add a numeric seed for a repeatable sequence, or leave it blank for a fresh browser sequence.','Review the generated list, range, count, and ordering.'],formula:{expression:'x = min + floor(u × (max − min + 1)), where u is a value from 0 up to but not including 1.',variables:[{symbol:'x',definition:'Generated integer'},{symbol:'min/max',definition:'Inclusive range endpoints'},{symbol:'u',definition:'Uniform pseudorandom unit value'}],notes:'A seeded mode is repeatable but pseudorandom. It is not suitable for cryptographic secrets.'},examples:[{title:'Generate a repeatable integer list',scenario:'Use 1 to 100, five values, and seed 42.',steps:['Set the inclusive range.','Set the count to five.','Enter seed 42.','Run the generator and keep the resulting sequence for a reproducible test.'],result:'The same settings produce the same sequence in this calculator; the exact values are intentionally not part of the public contract.'}],useCases:['Create sample values for a classroom exercise.','Reproduce a test case with a known seed.','Draw unique integers when the selected range is large enough.'],commonPitfalls:['Treating a pseudorandom sequence as cryptographically secure.','Forgetting that the endpoints are included.','Requesting more unique values than the range can contain.'],glossary:[{term:'Seed',definition:'An initial value that makes a pseudorandom sequence reproducible.'},{term:'Uniform mapping',definition:'A mapping intended to give each integer in the selected range an equal chance.'}],faqs:[{question:'Are the minimum and maximum included?',answer:'Yes. Integer generation uses an inclusive range.'},{question:'Does a seed guarantee secure randomness?',answer:'No. It makes results repeatable and therefore predictable.'},{question:'Can I generate unique values?',answer:'Yes, when the requested count does not exceed the number of integers in the range.'}],sources:[{title:'Random Number Generation',publisher:'NIST Engineering Statistics Handbook',url:'https://www.itl.nist.gov/div898/handbook/eda/section3/eda366j.htm'}],author:{name:'CalculatorFree Math Tools Review Team',credentials:'Calculation Review',description:'Original browser-side integer sequence implementation.'}},
  'password': {howToSteps:['Choose the character groups that may appear.','Set a length between 4 and 128 characters.','Optionally exclude ambiguous characters, brackets, or repeated characters.','Regenerate and copy only when you are using an appropriate secure context.'],formula:{expression:'Estimated entropy E = L × log₂(R).',variables:[{symbol:'E',definition:'Estimated search-space entropy in bits'},{symbol:'L',definition:'Password length'},{symbol:'R',definition:'Number of available characters'}],notes:'Entropy is a search-space estimate, not a guarantee against phishing, malware, reuse, or password-manager compromise. This route uses browser randomness and does not transmit the generated value.'},examples:[{title:'Compare character pools',scenario:'Compare two passwords with the same length but different character groups.',steps:['Keep length constant.','Enable an additional character group.','Observe the larger pool size.','Compare the estimated entropy without treating it as a complete security audit.'],result:'A larger pool increases the estimated search space, while adding length increases it linearly.'}],useCases:['Create a one-time random password for a password manager.','Teach how length and character-pool size affect search space.','Generate test strings locally without sending them to an API.'],commonPitfalls:['Reusing a generated password across accounts.','Saving secrets in screenshots, logs, or shared links.','Assuming entropy alone prevents phishing or credential theft.'],glossary:[{term:'Character pool',definition:'The set of characters permitted by the selected options.'},{term:'Entropy estimate',definition:'A logarithmic estimate of the number of possible strings under a simplified model.'}],faqs:[{question:'Is the generated password sent to a server?',answer:'No. The interface generates it in the browser.'},{question:'Does entropy prove that a password is safe?',answer:'No. Use a password manager, unique credentials, and multi-factor authentication.'},{question:'Why can no-repeat mode fail at long lengths?',answer:'There may not be enough distinct characters in the selected pool.'}],sources:[{title:'Digital Identity Guidelines: Authentication and Lifecycle Management',publisher:'NIST SP 800-63B',url:'https://pages.nist.gov/800-63-4/sp800-63b.html'}],author:{name:'CalculatorFree Web Tools Review Team',credentials:'Calculation Review',description:'Original client-side password generation and entropy estimate.'}},
  'dice-roller': {howToSteps:['Enter how many dice to simulate.','Set the number of sides on each die.','Roll the dice to generate one result per die.','Review the individual rolls, total, and expected average.'],formula:{expression:'Each roll = 1 + floor(u × s); expected total = n × (s + 1) ÷ 2.',variables:[{symbol:'n',definition:'Number of dice'},{symbol:'s',definition:'Sides per die'},{symbol:'u',definition:'Uniform pseudorandom unit value'}],notes:'This is a digital simulation. It does not reproduce the physical bias or randomness of a real die.'},examples:[{title:'Simulate three six-sided dice',scenario:'Use three dice with six sides each.',steps:['Set number of dice to three.','Set sides to six.','Roll the simulation.','Add the three displayed face values.'],result:'The total is an integer from 3 through 18, with an expected average of 10.5 over many independent rolls.'}],useCases:['Run a tabletop-game dice simulation.','Demonstrate discrete uniform outcomes.','Compare totals from different side counts.'],commonPitfalls:['Confusing the expected average with a guaranteed result.','Using too few rolls to infer fairness.','Treating a software simulation as cryptographic randomness.'],glossary:[{term:'Die',definition:'One simulated object with a fixed number of possible faces.'},{term:'Expected average',definition:'The long-run mean for one or more fair discrete dice.'}],faqs:[{question:'Can I simulate a d20?',answer:'Yes. Set the side count to 20.'},{question:'Is every total equally likely?',answer:'No. Individual faces are uniform, but sums of multiple dice have different probabilities.'},{question:'Is this suitable for security decisions?',answer:'No. It is a game and teaching simulation, not a cryptographic source.'}],sources:[{title:'NIST/SEMATECH e-Handbook of Statistical Methods: Random Numbers',publisher:'NIST',url:'https://www.itl.nist.gov/div898/handbook/eda/section3/eda366j.htm'}],author:{name:'CalculatorFree Everyday Tools Review Team',credentials:'Calculation Review',description:'Original browser-side dice simulation.'}},
  'shoe-size': {howToSteps:['Measure the longer foot from heel to toe.','Enter the length and choose centimeters, millimeters, or inches.','Select adults, kids, or infants and toddlers.','Use the converted sizes as a starting reference and check the manufacturer chart before buying.'],formula:{expression:'Approximate systems use foot length: US women ≈ 3L−21, US men ≈ 3L−22, UK/India adults ≈ 3L−23, EU ≈ 1.5Lcm+2, Japan/Mexico ≈ Lcm, and China ≈ 2Lcm−10.',variables:[{symbol:'L',definition:'Foot length in inches for US/UK formulas or centimeters for EU/Japan/China formulas'},{symbol:'age group',definition:'Adult, child, or toddler formula selection'}],notes:'Shoe sizing is not globally standardized. Brand, last shape, width, and measuring method can change the fit; the output is an estimate rather than a guaranteed size.'},examples:[{title:'Convert a measured adult foot',scenario:'Enter a 25 cm foot length in the adult mode.',steps:['Choose centimeters.','Enter 25.','Review the approximate EU and regional sizes.','Compare the result with the target brand’s size chart.'],result:'The calculator returns a cross-system estimate based on foot length; fit should be confirmed with the manufacturer.'}],useCases:['Create a first-pass international size comparison.','Estimate a child shoe size from measured foot length.','Convert measurements when shopping across regions.'],commonPitfalls:['Measuring only one foot.','Ignoring width and sock thickness.','Assuming every brand uses the same last or half-size convention.'],glossary:[{term:'Mondopoint',definition:'A footwear sizing approach based on foot length and sometimes width in millimeters.'},{term:'Foot length',definition:'The heel-to-toe measurement used as the main input in this estimate.'}],faqs:[{question:'Is the output exact for every brand?',answer:'No. Treat it as a reference and check the brand-specific chart.'},{question:'Why are EU sizes usually whole numbers?',answer:'Many EU size conventions use a larger step than half-size systems, and manufacturers can vary.'},{question:'Can children and adults use the same formula?',answer:'No. The route selects age-aware approximations for child and toddler ranges.'}],sources:[{title:'Shoe sizing — Mondopoint system',publisher:'International Organization for Standardization',url:'https://www.iso.org/obp/ui/#iso:std:iso:9407:ed-1:v1:en'},{title:'Metric SI units and length measurement',publisher:'NIST',url:'https://www.nist.gov/pml/owm/metric-si/si-units'}],author:{name:'CalculatorFree Everyday Tools Review Team',credentials:'Calculation Review',description:'Original cross-system footwear estimate based on measured length.'}},
  'day-counter': {howToSteps:['Enter the start date and end date for the interval.','Choose whether the end date should be included.','Optionally list holiday dates in ISO format when interpreting business days.','Review calendar days, weekdays, weekends, business days, and the offset-date result.'],formula:{expression:'Calendar days = included date count; business days = weekdays − listed holidays that fall on weekdays.',variables:[{symbol:'start/end',definition:'The interval endpoints at UTC midnight'},{symbol:'includeEnd',definition:'Whether the final date is part of the interval'},{symbol:'holiday',definition:'A listed date excluded from the business-day count'}],notes:'The calculator treats Saturday and Sunday as weekends, counts the start date, and includes the end date only when selected. It does not automatically know country-specific public holidays.'},examples:[{title:'Count a project interval',scenario:'Compare January 1 through January 31 with the end date excluded.',steps:['Enter both ISO dates.','Leave Include the end date unchecked.','Review total calendar days and weekday/weekend split.','Add holiday dates only when they should be excluded from business days.'],result:'The interval count follows the selected endpoint rule and shows the business-day estimate separately.'}],useCases:['Estimate working days remaining on a project.','Count days between events or deadlines.','Add or subtract calendar or business days from a known date.'],commonPitfalls:['Forgetting that excluding the end date changes the total by one day.','Assuming all public holidays are automatically included.','Mixing a calendar-day offset with a business-day offset.'],glossary:[{term:'Inclusive count',definition:'A count that includes the endpoint being measured.'},{term:'Business day',definition:'A weekday that is not one of the entered holiday dates.'}],faqs:[{question:'Are weekends removed from calendar days?',answer:'No. Calendar days and business days are shown separately.'},{question:'Does the tool know my country’s holidays?',answer:'No. Enter the holiday dates relevant to your calendar.'},{question:'How does business-day offset work?',answer:'It moves forward or backward one date at a time and decrements the remaining count only on weekdays not listed as holidays.'}],sources:[{title:'ISO 8601 Date and Time Format',publisher:'ISO',url:'https://www.iso.org/iso-8601-date-and-time-format.html'},{title:'Date and Time on the Internet: Timestamps',publisher:'IETF RFC 3339',url:'https://www.rfc-editor.org/rfc/rfc3339',year:2002}],author:{name:'CalculatorFree Time Tools Review Team',credentials:'Calculation Review',description:'Original client-side calendar and business-day counter.'}},
  'emergency-fund': {howToSteps:['Enter essential monthly expenses rather than all discretionary spending.','Choose the number of months you want the reserve to cover.','Enter current liquid savings and the amount you can add each month.','Review the target, gap, time-to-goal estimate, and current coverage.'],formula:{expression:'Target = monthly expenses × target months; gap = max(0, target − current savings); months to goal = ceiling(gap ÷ monthly contribution).',variables:[{symbol:'E',definition:'Essential monthly expenses'},{symbol:'M',definition:'Target coverage in months'},{symbol:'S',definition:'Current liquid savings'},{symbol:'C',definition:'Monthly contribution'}],notes:'If the gap is positive and monthly contribution is zero, the calculator reports no scheduled payoff time instead of inventing a date. This is a planning estimate, not financial advice.'},examples:[{title:'Estimate a six-month reserve',scenario:'Use $3,500 of essential monthly expenses, six target months, $7,000 saved, and $1,000 monthly contributions.',steps:['Multiply $3,500 by six for the target.','Subtract current savings from the target.','Divide the remaining gap by the monthly contribution and round up.','Compare current savings with one month of essential expenses.'],result:'The result shows a target, remaining gap, approximate months to goal, and current coverage in months.'}],useCases:['Set a liquid savings target before taking on additional risk.','Compare the effect of a larger monthly contribution.','Separate essential expenses from discretionary spending when planning a reserve.'],commonPitfalls:['Counting investments or unavailable credit as cash reserves.','Using total spending instead of essential expenses without labeling the assumption.','Treating a three-to-six-month guideline as a rule that fits every household.'],glossary:[{term:'Emergency fund',definition:'Liquid savings reserved for unexpected necessary expenses or income interruption.'},{term:'Savings gap',definition:'The amount still needed to reach the selected target.'}],faqs:[{question:'Should the target always be six months?',answer:'No. Household income stability, dependents, insurance, debt, and access to cash can change the appropriate target.'},{question:'What happens if monthly contribution is zero?',answer:'The calculator reports no scheduled payoff time when a positive gap remains; it does not assume a future lump sum.'},{question:'Is this a financial recommendation?',answer:'No. It is an educational estimate. Review your circumstances and current account terms before acting.'}],sources:[{title:'Emergency Savings',publisher:'Consumer Financial Protection Bureau',url:'https://www.consumerfinance.gov/an-essential-guide-to-building-an-emergency-fund/'},{title:'Financial Readiness and Emergency Savings',publisher:'Federal Emergency Management Agency',url:'https://www.ready.gov/financial-preparedness'}],author:{name:'CalculatorFree Finance Tools Review Team',credentials:'Calculation Review',description:'Original client-side emergency reserve estimate with explicit assumptions.'}},
  'cagr': {howToSteps:['Choose Calculate CAGR, Find Future Value, or Find Initial Value.','Enter the known values and the number of years.','When solving for a value, enter the annual rate as a percentage.','Review the annualized result and total growth, then consider volatility and interim cash flows.'],formula:{expression:'CAGR = (FV ÷ PV)^(1 ÷ t) − 1; FV = PV × (1+r)^t; PV = FV ÷ (1+r)^t.',variables:[{symbol:'PV',definition:'Initial or present value'},{symbol:'FV',definition:'Final or future value'},{symbol:'r',definition:'Annual growth rate as a decimal'},{symbol:'t',definition:'Number of years or periods'}],notes:'CAGR is a smoothed annualized rate. It does not show the path between the starting and ending values and is not a substitute for a risk or cash-flow analysis.'},examples:[{title:'Calculate annualized growth',scenario:'A value rises from $1,000 to $1,300 over three years.',steps:['Enter 1,000 as the initial value.','Enter 1,300 as the final value.','Enter three years.','Read CAGR and total growth.'],result:'The annualized rate is about 9.14%, while total growth is 30%. The exact intermediate path is unknown.'}],useCases:['Compare growth over different time horizons.','Solve for a future value under a constant annual rate assumption.','Explain annualized returns in a business or classroom model.'],commonPitfalls:['Calling CAGR an average of yearly returns.','Ignoring deposits, withdrawals, fees, taxes, or volatility.','Using zero or negative values in the basic positive-value formula.'],glossary:[{term:'CAGR',definition:'Compound annual growth rate over a specified period.'},{term:'Total growth',definition:'The percentage change from initial value to final value without annualizing.'}],faqs:[{question:'Is CAGR the same as average return?',answer:'No. CAGR is a geometric annualized rate between two endpoints and does not describe the arithmetic average or volatility.'},{question:'Can CAGR be negative?',answer:'Yes, when the final value is below the initial value and both are positive.'},{question:'Does CAGR include cash contributions?',answer:'Not in this basic endpoint model. Use a cash-flow-aware return method for irregular contributions or withdrawals.'}],sources:[{title:'Compound Annual Growth Rate',publisher:'Corporate Finance Institute',url:'https://corporatefinanceinstitute.com/resources/valuation/compound-annual-growth-rate-cagr/'},{title:'Time Value of Money',publisher:'U.S. Securities and Exchange Commission',url:'https://www.investor.gov/financial-tools-calculators/calculators/compound-interest-calculator'}],author:{name:'CalculatorFree Finance Tools Review Team',credentials:'Calculation Review',description:'Original endpoint-based annualized growth calculator.'}},
  'loan-to-value': {howToSteps:['Enter the loan balance or amount.','Enter the appraised or estimated property value.','Optionally enter additional liens for a combined LTV estimate.','Review LTV, equity amount, equity percentage, and CLTV.'],formula:{expression:'LTV% = loan ÷ property value × 100; equity = property value − loan; CLTV% = (loan + additional liens) ÷ property value × 100.',variables:[{symbol:'loan',definition:'The first loan amount or balance'},{symbol:'property value',definition:'The appraised or estimated property value'},{symbol:'additional liens',definition:'Optional second-lien balances included only in CLTV'}],notes:'This is an educational ratio. PMI, underwriting, rates, and maximum LTV rules differ by product, lender, jurisdiction, and borrower profile.'},examples:[{title:'Estimate equity from LTV',scenario:'A $320,000 loan is secured by a $400,000 property.',steps:['Divide 320,000 by 400,000.','Multiply by 100 for LTV.','Subtract the loan from the property value for equity.','Add any second lien only when reviewing CLTV.'],result:'The estimated LTV is 80%, equity is $80,000, and equity percentage is 20% before considering fees or market changes.'}],useCases:['Understand the relationship between a mortgage and property value.','Estimate equity before discussing refinance or home-equity options.','Compare first-lien LTV with combined LTV when other liens exist.'],commonPitfalls:['Using purchase price when the lender uses a different appraisal value.','Assuming an 80% threshold guarantees PMI removal or approval.','Forgetting second liens when considering total leverage.'],glossary:[{term:'LTV',definition:'Loan-to-value ratio for one loan compared with property value.'},{term:'CLTV',definition:'Combined loan-to-value ratio including multiple liens.'}],faqs:[{question:'What does a lower LTV mean?',answer:'It generally means the loan is smaller relative to the stated property value, but lender decisions also use credit, income, documentation, and product rules.'},{question:'Does this calculate PMI?',answer:'No. It reports ratios and equity; PMI terms depend on the loan program and lender.'},{question:'Can property value change the ratio?',answer:'Yes. Holding the loan constant, a higher value lowers the ratio and a lower value raises it.'}],sources:[{title:'Home Equity Loans and Lines of Credit',publisher:'Consumer Financial Protection Bureau',url:'https://www.consumerfinance.gov/ask-cfpb/what-is-a-home-equity-loan-en-105/'},{title:'Mortgage Consumer Resources',publisher:'Federal Housing Finance Agency',url:'https://www.fhfa.gov/consumers'}],author:{name:'CalculatorFree Property Tools Review Team',credentials:'Calculation Review',description:'Original LTV and educational CLTV ratio calculator.'}},
  'bill-split': {howToSteps:['Enter the base bill before tip.','Choose a tip percentage and number of people.','Enter a fixed service charge if it appears separately on the receipt.','Review the adjusted bill, tip, grand total, and equal per-person amount.'],formula:{expression:'Adjusted bill = base bill + service charge; tip = adjusted bill × tip rate ÷ 100; grand total = adjusted bill + tip; per person = grand total ÷ people.',variables:[{symbol:'bill',definition:'Base bill amount'},{symbol:'tip rate',definition:'Tip percentage applied after the service charge in this model'},{symbol:'people',definition:'Number of equal shares'},{symbol:'service charge',definition:'Fixed amount added before tip'}],notes:'Restaurants and regions can sequence taxes, service charges, and gratuity differently. Verify the receipt and use this as an equal-split estimate.'},examples:[{title:'Split a $250 bill four ways',scenario:'Use a $250 bill, 10% tip, four people, and no service charge.',steps:['Calculate a $25 tip.','Add it to the $250 base for a $275 total.','Divide $275 by four.','Check rounding against the payment method.'],result:'The equal share is $68.75 per person.'}],useCases:['Estimate an equal restaurant bill split.','Show how a service charge changes the tip-adjusted total.','Create a transparent baseline before handling unequal orders.'],commonPitfalls:['Tipping on a service charge that already includes gratuity.','Assuming an equal split is fair when orders differ substantially.','Ignoring receipt rounding or tax treatment.'],glossary:[{term:'Gratuity',definition:'A voluntary tip or service payment added to a bill.'},{term:'Equal split',definition:'Dividing the final total into the same number for each person.'}],faqs:[{question:'Is tax included?',answer:'The route treats the entered total as the bill base; include tax there if it is already on the receipt.'},{question:'Does the tool support unequal orders?',answer:'No. It establishes an equal-split estimate; unequal shares need item-level inputs.'},{question:'Why can the last payment differ by a cent?',answer:'Currency systems may round each share separately. Compare the rounded shares with the exact grand total.'}],sources:[{title:'Tipping and Service Charges',publisher:'Consumer Financial Protection Bureau',url:'https://www.consumerfinance.gov/consumer-tools/educator-tools/youth-financial-education/teach-activities/tipping/'},{title:'Rounding Rules for Currency Calculations',publisher:'National Institute of Standards and Technology',url:'https://www.nist.gov/pml/owm'}],author:{name:'CalculatorFree Everyday Tools Review Team',credentials:'Calculation Review',description:'Original equal-share bill and gratuity estimate.'}},
  'days-until': {howToSteps:['Enter the start date used as the reference point.','Enter a future or past target date.','Choose whether the start date itself should count as day 1.','Review the absolute day count, weeks plus days, calendar breakdown, and weekday/weekend split.'],formula:{expression:'Days = round((target date − start date) ÷ 86,400,000 ms); inclusive mode adds one day when the dates differ.',variables:[{symbol:'start',definition:'Reference calendar date'},{symbol:'target',definition:'Date being counted toward or from'},{symbol:'includeStart',definition:'Whether the reference date is treated as day 1'}],notes:'The route counts whole calendar dates and does not remove public holidays. It uses UTC-normalized date values in the browser so date-only inputs are not shifted by local daylight-saving changes.'},examples:[{title:'Count down to a date',scenario:'Compare August 26, 2026 with November 24, 2026 and leave start-day counting off.',steps:['Enter the two dates.','Leave the inclusive toggle off for elapsed days.','Review days, weeks plus days, and weekday/weekend totals.','Turn the toggle on only when the rule defines the start as day 1.'],result:'The calculator reports a future countdown and presents the same span in multiple calendar units.'}],useCases:['Count down to a deadline, event, or renewal.','Count days since a past date.','Translate a date span into weeks and weekday planning time.'],commonPitfalls:['Confusing elapsed-time counting with inclusive day-one counting.','Expecting public holidays to be removed from weekdays.','Using this date-only tool when hour-and-minute precision is required.'],glossary:[{term:'Elapsed days',definition:'The whole-date difference between two calendar dates.'},{term:'Inclusive counting',definition:'A convention that treats the start date as day 1 and adds one day to a nonzero span.'}],faqs:[{question:'Does the target date need to be in the future?',answer:'No. Earlier targets are labeled as days since the target date.'},{question:'Does it count holidays?',answer:'No. It separates weekdays and weekends but does not apply country-specific holidays.'},{question:'Why is the start-day toggle important?',answer:'A deadline or challenge may define the first date as day 1, while elapsed-time calculations normally count the gap from the start.'}],sources:[{title:'ISO 8601 Date and Time Format',publisher:'ISO',url:'https://www.iso.org/iso-8601-date-and-time-format.html'},{title:'Date and Time on the Internet: Timestamps',publisher:'IETF RFC 3339',url:'https://www.rfc-editor.org/rfc/rfc3339',year:2002}],author:{name:'CalculatorFree Time Tools Review Team',credentials:'Calculation Review',description:'Original date-only countdown and elapsed-span calculator.'}},
  'discounted-cash-flow-dcf': {howToSteps:['Enter current annual free cash flow.','Set the forecast growth, discount rate, terminal growth rate, and forecast years.','Keep terminal growth below the discount rate so the Gordon growth value remains finite.','Review the forecast value, terminal value, and combined enterprise value estimate.'],formula:{expression:'Enterprise value = Σ[FCFₜ ÷ (1 + r)ᵗ] + [FCFₙ(1 + gₜ) ÷ (r − gₜ)] ÷ (1 + r)ⁿ',variables:[{symbol:'FCFₜ',definition:'Free cash flow in forecast year t'},{symbol:'r',definition:'Discount rate or WACC as a decimal'},{symbol:'gₜ',definition:'Perpetual terminal growth rate as a decimal'},{symbol:'n',definition:'Number of explicit forecast years'}],notes:'This simplified DCF uses a constant growth rate during the explicit forecast and a Gordon growth terminal value. It is an educational estimate, not an investment recommendation or a complete valuation model.'},examples:[{title:'Five-year DCF illustration',scenario:'Start with $100,000 of annual free cash flow, 5% forecast growth, a 10% discount rate, 2% terminal growth, and five forecast years.',steps:['Project each year’s cash flow by multiplying the previous year by 1.05.','Discount each projected cash flow by the corresponding power of 1.10.','Calculate terminal value from the final forecast cash flow and the 2% perpetual rate.','Discount terminal value back five years and add it to the forecast present value.'],result:'The calculator reports an enterprise-value estimate whose sensitivity is driven strongly by the discount and terminal growth assumptions.'}],useCases:['Build a transparent first-pass valuation for a business or project.','Compare how discount-rate assumptions change present value.','Explain why terminal value should be stress-tested rather than treated as a precise price.'],commonPitfalls:['Using a terminal growth rate equal to or above the discount rate.','Mixing levered equity cash flow with an enterprise discount rate.','Treating a single DCF output as a market price or certainty.'],glossary:[{term:'Free cash flow',definition:'Cash generated after operating needs and capital investment that is available to capital providers.'},{term:'Terminal value',definition:'The estimated value of cash flows beyond the explicit forecast period.'}],faqs:[{question:'What does DCF estimate?',answer:'It estimates present value from projected cash flows and a terminal value under the assumptions entered.'},{question:'Why does the discount rate matter so much?',answer:'Future cash flows are divided by larger powers of the discount factor, so higher rates reduce present value, especially for distant cash flows.'},{question:'Can I use revenue instead of free cash flow?',answer:'Not without changing the model. Revenue is not cash available to investors; this route is designed around free cash flow.'}],sources:[{title:'Valuation: Measuring and Managing the Value of Companies',publisher:'Aswath Damodaran, NYU Stern',url:'https://pages.stern.nyu.edu/~adamodar/'} ,{title:'Investor Bulletin: Discounting and Present Value',publisher:'U.S. Securities and Exchange Commission',url:'https://www.investor.gov/introduction-investing/investing-basics/glossary/present-value'}],author:{name:'CalculatorFree Valuation Review Team',credentials:'Calculation Review',description:'Original discounted-cash-flow estimate with explicit assumptions.'}},
  'break-even': {howToSteps:['Enter fixed costs for the period.','Enter the selling price and variable cost for one unit.','Check that the selling price exceeds the variable cost.','Review the units and revenue needed to reach the break-even point.'],formula:{expression:'Break-even units = Fixed costs ÷ (Selling price per unit − Variable cost per unit)',variables:[{symbol:'F',definition:'Fixed costs for the selected period'},{symbol:'P',definition:'Selling price per unit'},{symbol:'V',definition:'Variable cost per unit'},{symbol:'P − V',definition:'Contribution margin per unit'}],notes:'The calculation assumes one product or a stable blended margin, constant unit economics, and no step changes in fixed costs.'},examples:[{title:'Covering operating costs',scenario:'A product has $50,000 of fixed costs, sells for $100, and has $40 of variable cost per unit.',steps:['Contribution margin is $100 − $40 = $60.','Divide $50,000 by $60 to obtain 833.33 units.','Round up to 834 whole units when planning physical sales.','Multiply the unit target by $100 to estimate break-even revenue.'],result:'The business needs about 834 units to cover the modeled costs.'}],useCases:['Set a minimum sales target for a product launch.','Compare pricing or supplier-cost scenarios.','Explain the effect of contribution margin to a business team.'],commonPitfalls:['Forgetting variable commissions, shipping, or payment fees.','Using a blended margin when the product mix changes materially.','Rounding units down when a whole-unit target is required.'],glossary:[{term:'Fixed cost',definition:'A cost treated as unchanged by the modeled sales volume during the period.'},{term:'Contribution margin',definition:'The amount from each sale left after variable cost to cover fixed costs and profit.'}],faqs:[{question:'What if price equals variable cost?',answer:'There is no positive contribution margin, so the modeled business cannot recover fixed costs through unit sales.'},{question:'Does break-even mean profitable?',answer:'No. It is the point where modeled revenue covers modeled costs; profit begins above that point.'},{question:'Can this handle several products?',answer:'Use a weighted average contribution margin for a stable product mix, and interpret the result as an estimate.'}],sources:[{title:'Managerial Accounting: Cost-Volume-Profit Analysis',publisher:'OpenStax',url:'https://openstax.org/books/managerial-accounting/pages/3-1-describe-how-companies-use-cost-volume-profit-analysis'} ,{title:'Break-even Analysis',publisher:'U.S. Small Business Administration',url:'https://www.sba.gov/business-guide/manage-your-business/calculate-your-startup-costs'}],author:{name:'CalculatorFree Business Math Review Team',credentials:'Calculation Review',description:'Original contribution-margin and break-even planning tool.'}},
  'capital-gains': {howToSteps:['Enter the original purchase price.','Add eligible capital improvements and selling costs when applicable.','Enter the sale price and an illustrative tax rate.','Review adjusted basis, realized gain or loss, estimated tax, and after-tax proceeds.'],formula:{expression:'Adjusted basis = Purchase price + Improvements; Net proceeds = Sale price − Selling costs; Gain = Net proceeds − Adjusted basis',variables:[{symbol:'basis',definition:'Original cost adjusted by modeled capital improvements'},{symbol:'proceeds',definition:'Sale price after entered selling costs'},{symbol:'gain',definition:'Net proceeds minus adjusted basis'},{symbol:'tax rate',definition:'Illustrative percentage applied only to a positive modeled gain'}],notes:'This simplified estimate does not apply holding periods, exclusions, depreciation recapture, jurisdiction rules, filing status, or loss limitations. Confirm tax treatment with current official guidance.'},examples:[{title:'Illustrative asset sale',scenario:'Purchase for $200,000, add $25,000 of improvements, sell for $300,000, pay $5,000 selling costs, and use a 20% illustrative rate.',steps:['Adjusted basis is $200,000 + $25,000 = $225,000.','Net sale proceeds are $300,000 − $5,000 = $295,000.','Realized gain is $295,000 − $225,000 = $70,000.','An illustrative 20% rate produces $14,000 of estimated tax in this simplified model.'],result:'Estimated after-tax proceeds are $281,000 before any other taxes, debts, or transaction adjustments.'}],useCases:['Create a first-pass scenario before speaking with a tax professional.','Compare sale prices after transaction costs.','Show how documented improvements affect modeled basis.'],commonPitfalls:['Treating the illustrative rate as the final tax bill.','Leaving out selling expenses or eligible basis adjustments.','Assuming every gain receives the same rate regardless of asset and holding period.'],glossary:[{term:'Adjusted basis',definition:'A tax basis modified by qualifying costs or adjustments under applicable rules.'},{term:'Realized gain',definition:'The gain calculated when an asset is sold, before considering the complete tax treatment.'}],faqs:[{question:'Does this calculate my actual tax liability?',answer:'No. It is an educational estimate and omits many jurisdiction-specific rules and taxpayer circumstances.'},{question:'What happens when the sale price is below basis?',answer:'The route shows a negative realized gain, while estimated tax is floored at zero in this simplified positive-gain model.'},{question:'Are improvements always added to basis?',answer:'Only qualifying costs under the applicable tax rules should be included. Keep records and verify with current official guidance.'}],sources:[{title:'Topic no. 409, Capital Gains and Losses',publisher:'Internal Revenue Service',url:'https://www.irs.gov/taxtopics/tc409'} ,{title:'Publication 551, Basis of Assets',publisher:'Internal Revenue Service',url:'https://www.irs.gov/publications/p551'}],author:{name:'CalculatorFree Tax Estimate Review Team',credentials:'Calculation Review',description:'Original capital-gain scenario estimator with explicit limitations.'}},
  'dividend': {howToSteps:['Enter the number of shares held.','Enter the annual dividend per share.','Enter the current share price to estimate portfolio value and indicated yield.','Review annual income plus monthly and quarterly equivalents.'],formula:{expression:'Annual dividend income = Shares × Annual dividend per share; Indicated yield = Annual income ÷ (Shares × Share price)',variables:[{symbol:'shares',definition:'Number of shares held'},{symbol:'DPS',definition:'Annual cash dividend per share'},{symbol:'price',definition:'Current share price used for the yield denominator'}],notes:'The result is based on the entered annual dividend and price. It does not forecast dividend changes, taxes, withholding, fees, or reinvestment.'},examples:[{title:'Income and yield estimate',scenario:'Hold 100 shares, receive $2.40 per share annually, and use a $60 share price.',steps:['Annual income is 100 × $2.40 = $240.','Portfolio value is 100 × $60 = $6,000.','Indicated yield is $240 ÷ $6,000 × 100 = 4%.','Divide annual income by 12 or 4 for monthly or quarterly equivalents.'],result:'The modeled annual income is $240 and the indicated dividend yield is 4%.'}],useCases:['Estimate cash income from a share position.','Compare indicated yields at different share prices.','Build a simple income scenario before accounting for taxes and reinvestment.'],commonPitfalls:['Assuming a declared dividend is guaranteed forever.','Using quarterly dividend per share as if it were annual.','Ignoring taxes, withholding, fees, or currency conversion.'],glossary:[{term:'Dividend per share',definition:'The cash distribution attributed to one share over a stated period.'},{term:'Indicated yield',definition:'Annualized dividend divided by the current share price, expressed as a percentage.'}],faqs:[{question:'Does the calculator include dividend growth?',answer:'No. It uses the annual dividend per share that you enter today.'},{question:'Why can yield change without a dividend change?',answer:'Because the share price changes the value used in the yield denominator.'},{question:'Are monthly payments guaranteed?',answer:'No. Monthly and quarterly figures are annual-income equivalents; the issuer may pay on a different schedule.'}],sources:[{title:'Investor.gov: Dividends',publisher:'U.S. Securities and Exchange Commission',url:'https://www.investor.gov/introduction-investing/investing-basics/glossary/dividend'} ,{title:'Investor Bulletin: Investing in Stocks',publisher:'U.S. Securities and Exchange Commission',url:'https://www.investor.gov/introduction-investing/investing-basics/investment-products/stocks'}],author:{name:'CalculatorFree Income Tools Review Team',credentials:'Calculation Review',description:'Original dividend income and indicated-yield estimator.'}},
  'business-valuation': {howToSteps:['Enter annual revenue and the modeled EBITDA margin.','Choose an illustrative EBITDA valuation multiple.','Enter net debt to bridge from enterprise value to equity value.','Review EBITDA, enterprise value, equity value, and implied revenue multiple.'],formula:{expression:'EBITDA = Revenue × EBITDA margin; Enterprise value = EBITDA × multiple; Equity value = Enterprise value − net debt',variables:[{symbol:'Revenue',definition:'Annual operating revenue'},{symbol:'EBITDA margin',definition:'EBITDA as a percentage of revenue'},{symbol:'Multiple',definition:'Illustrative value assigned per unit of EBITDA'},{symbol:'Net debt',definition:'Debt less cash included in the bridge to equity value'}],notes:'This is a scenario model using one operating metric and one multiple. It is not a formal appraisal and does not account for working capital, taxes, growth, risk, or transaction structure.'},examples:[{title:'Illustrative operating-multiple valuation',scenario:'A company reports $1,000,000 revenue, a 20% EBITDA margin, a 6× multiple, and $100,000 net debt.',steps:['Calculate EBITDA as $1,000,000 × 20% = $200,000.','Multiply $200,000 by 6 for a $1,200,000 enterprise value.','Subtract $100,000 net debt for a $1,100,000 equity-value estimate.','Compare the result with other methods and company-specific evidence.'],result:'The scenario produces $200,000 EBITDA, $1,200,000 enterprise value, and $1,100,000 estimated equity value.'}],useCases:['Create a transparent first-pass valuation scenario.','Stress-test how margin or multiple assumptions affect value.','Explain the distinction between enterprise value and equity value.'],commonPitfalls:['Treating one market multiple as universally appropriate.','Subtracting gross debt without considering cash when using net debt.','Using adjusted EBITDA without clearly documenting adjustments.'],glossary:[{term:'EBITDA',definition:'Earnings before interest, taxes, depreciation, and amortization.'},{term:'Enterprise value',definition:'A value measure for the operating business before the equity/debt bridge.'}],faqs:[{question:'Is this a sale price prediction?',answer:'No. It is an illustrative scenario based on the inputs and does not predict a transaction price.'},{question:'Why subtract net debt?',answer:'A simplified enterprise-to-equity bridge subtracts debt-like obligations and adds cash; this route accepts net debt directly.'},{question:'Can I value a company with negative EBITDA?',answer:'This version floors the margin at zero, so it is not designed for loss-making businesses or revenue multiples.'}],sources:[{title:'Valuation Approaches and Metrics',publisher:'U.S. Securities and Exchange Commission',url:'https://www.investor.gov/introduction-investing/investing-basics/glossary/enterprise-value'},{title:'Investment Valuation Resources',publisher:'Aswath Damodaran, NYU Stern',url:'https://pages.stern.nyu.edu/~adamodar/'}],author:{name:'CalculatorFree Business Valuation Review Team',credentials:'Calculation Review',description:'Original EBITDA-multiple scenario calculator.'}},
  'debt-service-coverage-ratio-dscr': {howToSteps:['Enter annual net operating income.','Enter annual principal and interest debt service.','Review the coverage ratio and annual surplus or shortfall.','Use the result as a screening ratio, not as a lender approval decision.'],formula:{expression:'DSCR = Net operating income ÷ Annual debt service',variables:[{symbol:'NOI',definition:'Net operating income after modeled operating expenses'},{symbol:'Debt service',definition:'Annual principal and interest payments used in the scenario'},{symbol:'DSCR',definition:'Income coverage expressed as a ratio'}],notes:'Lenders may define NOI and debt service differently, include reserves, or require a target coverage level. Confirm the underwriting definition for the specific loan.'},examples:[{title:'Property coverage example',scenario:'Annual NOI is $120,000 and annual debt service is $80,000.',steps:['Divide $120,000 by $80,000.','Read the 1.50× coverage ratio.','Subtract debt service from NOI to find the $40,000 annual surplus.','Stress-test income and expenses before relying on the ratio.'],result:'The modeled DSCR is 1.50× with a $40,000 annual operating surplus before other items.'}],useCases:['Screen a rental or commercial property financing scenario.','Compare income coverage across debt structures.','Show how vacancy or expense assumptions change coverage.'],commonPitfalls:['Using gross rent instead of NOI.','Leaving replacement reserves or management costs out of expenses.','Assuming a ratio alone guarantees approval or a specific interest rate.'],glossary:[{term:'Net operating income',definition:'Property income after operating expenses but before financing and income taxes.'},{term:'Debt service',definition:'Scheduled principal and interest payments for the modeled debt.'}],faqs:[{question:'What does 1.25× mean?',answer:'The modeled NOI is 1.25 times annual debt service, leaving a 25% cushion relative to the debt payment.'},{question:'Does DSCR include taxes?',answer:'Definitions vary. This route uses the NOI and debt service values you enter, so label your assumptions consistently.'},{question:'Is a higher DSCR always better?',answer:'Higher coverage generally indicates more modeled income cushion, but lenders also assess leverage, property risk, borrower strength, and documentation.'}],sources:[{title:'Commercial Real Estate Lending',publisher:'Federal Reserve Board',url:'https://www.federalreserve.gov/boarddocs/caletters/2007/0715/07-15-attachment.pdf'},{title:'Commercial Real Estate Finance',publisher:'Office of the Comptroller of the Currency',url:'https://www.occ.treas.gov/topics/supervision-and-examination/credit/commercial-real-estate-lending/index-commercial-real-estate-lending.html'}],author:{name:'CalculatorFree Property Finance Review Team',credentials:'Calculation Review',description:'Original NOI-to-debt-service coverage screening tool.'}},
  'car-affordability': {howToSteps:['Enter monthly gross income and other monthly debt payments.','Choose a down payment and total debt-budget percentage.','Enter the annual loan rate used for a 60-month payment scenario.','Review the modeled payment, supported loan, vehicle price, and interest.'],formula:{expression:'Maximum auto payment = Monthly income × debt-budget rate − other monthly debts; Loan = Payment × [1 − (1+r)^−n] ÷ r',variables:[{symbol:'income',definition:'Monthly gross income'},{symbol:'debts',definition:'Existing monthly debt payments'},{symbol:'r',definition:'Monthly interest rate'},{symbol:'n',definition:'Fixed 60-month term in this route'}],notes:'This budgeting model uses a total-debt budget rather than a lender’s approval formula. It excludes taxes, insurance, fuel, maintenance, registration, fees, and trade-in value.'},examples:[{title:'Set a payment ceiling',scenario:'Use $7,000 monthly income, $500 existing debt, a 10% down payment input, a 20% total debt budget, and a 7% annual rate.',steps:['Calculate the total debt budget as $7,000 × 20% = $1,400.','Subtract $500 existing debt for a $900 modeled car payment.','Convert the $900 payment into a 60-month loan at 7%.','Add the entered down-payment amount to estimate vehicle price.'],result:'The output is a budgeting ceiling, not a recommendation to borrow the full amount.'}],useCases:['Set a car-shopping budget before visiting a dealer.','Compare payment capacity at different rates and terms.','Make recurring ownership costs visible before focusing on purchase price.'],commonPitfalls:['Treating the maximum model output as a comfortable budget.','Ignoring insurance, fuel, repairs, taxes, and registration.','Using gross income when a household budget should use dependable take-home cash flow.'],glossary:[{term:'Debt-to-income budget',definition:'A planning share of income reserved for recurring debt obligations.'},{term:'Loan principal',definition:'The amount financed before interest and fees.'}],faqs:[{question:'Why is the term fixed at 60 months?',answer:'The route uses a transparent five-year scenario so results are comparable; real loans can use different terms.'},{question:'Does the down payment input use a percentage?',answer:'No. It is a currency amount that is added to the modeled loan amount for an estimated vehicle price.'},{question:'Is this a lender qualification tool?',answer:'No. It is an educational budgeting estimate and does not account for underwriting, credit, taxes, or total ownership cost.'}],sources:[{title:'Consumer Vehicle Financing',publisher:'Consumer Financial Protection Bureau',url:'https://www.consumerfinance.gov/consumer-tools/auto-loans/'},{title:'Buying a Car',publisher:'Federal Trade Commission',url:'https://consumer.ftc.gov/articles/buying-car'}],author:{name:'CalculatorFree Auto Finance Review Team',credentials:'Calculation Review',description:'Original vehicle-payment affordability scenario.'}},
  'bond-yield': {howToSteps:['Enter bond face value, coupon rate, market price, and years to maturity.','Review annual coupon income and current yield.','Review the approximate yield to maturity based on price movement toward face value.','Compare the result with the bond prospectus and actual compounding conventions.'],formula:{expression:'Current yield = Annual coupon ÷ Market price; Approximate YTM = [Coupon + (Face value − Price) ÷ Years] ÷ [(Face value + Price) ÷ 2]',variables:[{symbol:'Face value',definition:'Amount repaid at maturity under the scenario'},{symbol:'Coupon',definition:'Annual cash coupon amount'},{symbol:'Price',definition:'Current market price'},{symbol:'Years',definition:'Years remaining to maturity'}],notes:'The YTM output is an approximation. It ignores callability, taxes, payment-frequency compounding, reinvestment assumptions, and default risk.'},examples:[{title:'Premium or discount bond',scenario:'Use $1,000 face value, a 4% coupon, a $980 price, and four years to maturity.',steps:['Annual coupon is $1,000 × 4% = $40.','Current yield is $40 ÷ $980, or about 4.08%.','The price discount adds an approximate annual pull toward face value.','Interpret the approximate YTM alongside the bond’s actual terms.'],result:'The tool shows current yield, approximate YTM, annual coupon, and the $20 discount.'}],useCases:['Compare coupon income with the price paid.','Explain why a bond’s market yield can differ from its coupon rate.','Create a first-pass estimate before reading a full bond quote.'],commonPitfalls:['Confusing coupon rate with yield.','Ignoring call provisions, taxes, fees, or default risk.','Using the approximation as an exact settlement or yield quote.'],glossary:[{term:'Coupon rate',definition:'The stated annual interest percentage applied to face value.'},{term:'Yield to maturity',definition:'A return measure that considers price, coupons, maturity, and reinvestment assumptions.'}],faqs:[{question:'Why can current yield differ from the coupon rate?',answer:'Current yield divides the annual coupon by market price, while coupon rate divides it by face value.'},{question:'What does a discount mean?',answer:'A market price below face value creates a potential price pull toward face value at maturity, subject to issuer performance and terms.'},{question:'Is the YTM exact here?',answer:'No. This route deliberately labels it approximate and omits several security-specific conventions.'}],sources:[{title:'Investor Bulletin: Bonds',publisher:'U.S. Securities and Exchange Commission',url:'https://www.investor.gov/introduction-investing/investing-basics/investment-products/bonds-or-fixed-income-products'},{title:'Treasury Bonds and Notes',publisher:'U.S. Department of the Treasury',url:'https://treasurydirect.gov/marketable-securities/treasury-bonds/'}],author:{name:'CalculatorFree Fixed Income Review Team',credentials:'Calculation Review',description:'Original current-yield and approximate-YTM scenario tool.'}},
};
