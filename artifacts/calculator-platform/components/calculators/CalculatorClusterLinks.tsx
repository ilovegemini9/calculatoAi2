...
import { getKeywordCluster, getKeywordClusterId } from '@/config/keyword-clusters';
...
export function CalculatorClusterLinks({ slug }: Props) {
  const cluster = getKeywordCluster(getKeywordClusterId(slug));
  ...
  const links = cluster.routes
    ...
    .map((route) => ({
      slug: route,
      name: CALCULATOR_BY_SLUG[route].name,
    }));
