            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition shrink-0 border ${
                isActive
                  ...
                  : 'hover:bg-[var(--bg-card-hover)] text-[var(--text-secondary)] border-[var(--border)]'
              }`}
              style={!isActive ? { backgroundColor: 'var(--bg-card)' } : undefined}
            >
              <span>{tab.icon}</span>
              <span>{tab.label}</span>
              <span
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {paginatedList.map((calc) => (
            <Link
              ...
              className="group relative flex flex-col justify-between rounded-2xl border p-5 transition-all duration-200 hover:shadow-md hover:border-blue-500/50"
              style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}
            >
              <div>
