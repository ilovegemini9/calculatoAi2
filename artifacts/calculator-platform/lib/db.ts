...
function getPostgresConfig(connectionString: string): { connectionString: string; ssl?: { rejectUnauthorized: false } } {
  try {
    const url = new URL(connectionString);
    if (url.searchParams.get('sslmode') === 'require') {
      // Preserve pg's current sslmode=require behavior explicitly and avoid its warning.
...
function getPool(): Pool {
  ...
  const pool = new Pool({
    ...getPostgresConfig(connectionString),
    max: 5,
...
function generateInitialAnalytics(): AppSchema['analytics'] {
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    analytics.push({ id: `an-${i}`, date: date.toISOString().slice(0, 10), views, uniqueVisitors: Math.round(views * 0.72) });
  }
  return analytics;
}
...
function createInitialDb(): AppSchema {
  ...
  return {
    adminUsers: password ? [{ id: 'admin-id', username, passwordHash: bcrypt.hashSync(password, 10), createdAt: new Date().toISOString() }] : [],
    calculators: [], articles: [], articleVersions: [], redirects: [], analytics: generateInitialAnalytics(),
    settings: DEFAULT_SETTINGS, logs: generateInitialLogs(), backups: [], sessions: [],
  };
}
...
function applyDefaults(value: AppSchema): AppSchema {
  ...
  return {
    ...value,
    adminUsers: value.adminUsers || [], calculators: value.calculators || [], articles: value.articles || [], articleVersions: value.articleVersions || [], redirects: value.redirects || [],
    analytics: Array.isArray(value.analytics) && value.analytics.length > 0 ? value.analytics : generateInitialAnalytics(),
    settings: { ...DEFAULT_SETTINGS, ...value.settings, openrouterApiKey: '', seo: getSeoSettings(value.settings?.seo), ads: getAdsSettings(value.settings?.ads), verification: getVerificationSettings(value.settings?.verification), ai: getAiSettings(value.settings?.ai, legacyOpenRouterKey), featureFlags: { ...DEFAULT_SETTINGS.featureFlags, ...value.settings?.featureFlags } },
    logs: Array.isArray(value.logs) && value.logs.length > 0 ? value.logs : generateInitialLogs(), backups: value.backups || [], sessions: value.sessions || [],
  };
}

async function ensureState(): Promise<AppSchema> {
  const initial = createInitialDb();
  const result = await getPool().query<{ data: AppSchema }>(`INSERT INTO app_state (id, data, updated_at) VALUES (1, $1::jsonb, NOW()) ON CONFLICT (id) DO UPDATE SET id = app_state.id RETURNING data`, [JSON.stringify(initial)]);
  return applyDefaults(result.rows[0].data);
}

export async function getDb(): Promise<AppSchema> {
  const hasDatabaseUrl = Boolean(process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_PRISMA_URL || process.env.DATABASE_URL_UNPOOLED || process.env.POSTGRES_URL_NON_POOLING);
  if (!hasDatabaseUrl) {
    if (!globalForDb.inMemoryState) globalForDb.inMemoryState = createInitialDb();
...
export async function saveDb(data: AppSchema): Promise<void> {
  ...
  try {
    await getPool().query(`INSERT INTO app_state (id, data, updated_at) VALUES (1, $1::jsonb, NOW()) ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, updated_at = NOW()`, [JSON.stringify(sanitized)]);
  } catch (error) {
...
export async function logEvent(level: 'INFO' | 'WARN' | 'ERROR', message: string, route = '/admin', details?: unknown): Promise<void> {
  ...
  data.logs = [{ id: `log-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, timestamp: new Date().toISOString(), level, message, route, details }, ...(data.logs || [])].slice(0, 500);
  await saveDb(data);
}
...
export async function trackAnalytics(views = 1): Promise<void> {
  ...
  if (existing) { existing.views += views; existing.uniqueVisitors = Math.round(existing.views * 0.75); }
  else data.analytics.push({ id: `an-${Date.now()}`, date: today, views, uniqueVisitors: Math.round(views * 0.75) });
  await saveDb(data);
}
