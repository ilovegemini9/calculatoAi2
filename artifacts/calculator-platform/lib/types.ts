...
export interface Article {
  ...
  // AI SEO Optimization (Phase 9)
  aiSeoOptimized?: boolean;
  aiOverviewTarget?: string;       // 40-60 word direct answer for AI Overviews / Featured Snippets
  semanticKeywords?: string[];     // LSI / semantic topic-cluster vocabulary
  entities?: ArticleEntity[];
  eeatSignals?: EeatSignals;
  seoAudit?: SeoAudit;
}

// ─── Phase 9 — AI SEO Optimization types ─────────────────────────────────────

export interface ArticleEntity {
  name: string;
  type: 'Person' | 'Organization' | 'Concept' | 'Place' | 'Product' | 'MedicalCondition' | 'FinancialProduct' | 'Other';
  description?: string;
}

export interface EeatSignals {
  expertiseLevel: 'beginner' | 'intermediate' | 'expert';
  authoritySignals: string[];   // e.g. ["Cites CDC guidelines", "References IRS data"]
  trustSignals: string[];       // e.g. ["External sources linked", "Schema markup present"]
  experienceIndicators: string[]; // e.g. ["Includes worked examples", "Real numbers used"]
}
...
// ─── Phase 10: Quality Validation types ───────────────────────────────────────
