...
export function calculateBMI(input: BMIInput): BMIResult {
  ...
  if (bmi < 18.5) {
    ...
    recommendation =
      'You are in the underweight category. It is recommended to consult a nutritionist or doctor to see if you need to gain weight healthy.';
  } else if (bmi < 25) {
    category = 'Normal weight';
    recommendation =
      'Congratulations! You are maintaining a healthy body weight. Keep up the balanced diet and regular physical activity.';
  } else if (bmi < 30) {
    category = 'Overweight';
    recommendation =
      'You are in the overweight range. Incorporating more physical activities and managing portions can help you reach a healthier range.';
  } else {
    category = 'Obese';
    recommendation =
      'You are categorized in the obese range. We recommend speaking with a healthcare professional to establish safe fitness and diet plans.';
  }
  ...
  return { bmi, category, healthyRangeText, recommendation };
}
