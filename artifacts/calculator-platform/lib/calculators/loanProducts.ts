...
export function calculateStudentLoan(input: StudentLoanInput): StudentLoanResult {
  const principal = finiteNonNegative(input.principal); const fee = principal * finiteNonNegative(input.originationFeeRate) / 100; const base = calculatePayment({ principal, annualRate: input.annualRate, termYears: input.termYears, fees: fee }); const extra = finiteNonNegative(input.extraMonthlyPayment); const rate = finiteNonNegative(input.annualRate) / 1200; const factor = Math.pow(1 + rate, base.termMonths); const scheduledExact = base.financedAmount === 0 ? 0 : rate === 0 ? base.financedAmount / base.termMonths : base.financedAmount * (rate * factor) / (factor - 1); let balance = base.financedAmount; let totalInterest = 0; let totalPayments = 0; let month = 0; const rows: InstallmentRow[] = [];
  while (balance > 0.005 && month < 1200) { month += 1; const interest = balance * rate; const payment = Math.min(scheduledExact + extra, balance + interest); const principalPaid = Math.min(balance, Math.max(0, payment - interest)); balance = Math.max(0, balance - principalPaid); totalInterest += interest; totalPayments += payment; if (month <= 120 || month % 12 === 0 || balance === 0) rows.push({ period: month, payment: cents(payment), principal: cents(principalPaid), interest: cents(interest), balance: cents(balance) }); }
  return { ...base, originationFee: cents(fee), extraMonthlyPayment: cents(extra), scheduledPayment: cents(scheduledExact), payoffMonths: month, monthlyPayment: cents(scheduledExact + extra), totalInterest: cents(totalInterest), totalCost: cents(totalPayments), amortization: rows, interestSaved: cents(Math.max(0, base.totalInterest - totalInterest)) };
}
