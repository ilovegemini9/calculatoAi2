...
export function calculateDebtConsolidation(balance: number, currentApr: number, currentPayment: number, newApr: number, termYears: number, fees: number): ConsolidationResult {
  ...
  const newStatus = b === 0 ? 'no-balance' : newPayment <= p * r && r > 0 ? 'payment-too-low' : 'payable'; return { currentPayment: money(old.monthlyPayment), newPayment: money(newPayment), monthlySavings: money(old.monthlyPayment - newPayment), currentInterest: money(old.totalInterest), newInterest: money(newInterest), totalNewCost: money(p + newInterest), status: newStatus };
}
...
export function calculateDti(income: number, debt: number, housing: number, proposed: number): DtiResult {
  const gross = nn(income); const monthlyDebt = nn(debt) + nn(housing); const proposedDebt = monthlyDebt + nn(proposed); const dti = gross === 0 ? Infinity : monthlyDebt / gross * 100; const proposedDti = gross === 0 ? Infinity : proposedDebt / gross * 100;
  return { grossMonthlyIncome: money(gross), monthlyDebt: money(monthlyDebt), dti: Number.isFinite(dti) ? Math.round(dti * 10) / 10 : 0, proposedDti: Number.isFinite(proposedDti) ? Math.round(proposedDti * 10) / 10 : 0, headroom: Math.round(Math.max(0, 36 - proposedDti) * 10) / 10, status: gross === 0 || proposedDti > 36 ? 'high' : 'within-range' };
}
