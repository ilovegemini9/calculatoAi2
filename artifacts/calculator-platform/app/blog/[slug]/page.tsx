...
export default async function BlogArticlePage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Speakable — voice search & AI assistant citations */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            speakable: {
              ...
              cssSelector: ['.quick-answer', '.article-body p:first-of-type', 'h1'],
            },
          }),
        }}
      />
      {/* Entity schema — named entities for semantic / entity SEO */}
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              '@context': 'https://schema.org',
              '@type': 'ItemList',
              name: `Topics covered in: ${article.title}`,
              itemListElement: article.entities.map((entity, idx) => ({
                '@type': 'ListItem',
                position: idx + 1,
                item: {
                  '@type': entity.type === 'Person' ? 'Person'
      ...
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="lg:grid lg:grid-cols-[1fr_260px] lg:gap-10">
          ...
          <article className="min-w-0">
            ...
            {/* Quick Answer — AI Overview target (Phase 9) */}
            {article.aiOverviewTarget && (
              <div
                className="quick-answer mb-8 rounded-xl border-l-4 border-purple-500 bg-purple-500/5 px-5 py-4"
                style={{ borderColor: 'var(--border)' }}
              >
                <p className="text-xs font-bold uppercase tracking-wider text-purple-400 mb-2 flex items-center gap-1.5">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/>
                  </svg>
                  Quick Answer
                </p>
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">{article.aiOverviewTarget}</p>
              </div>
            )}
