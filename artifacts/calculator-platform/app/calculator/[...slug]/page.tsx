...
async function getCalculatorData(slug: string) {
  ...
  if (staticCalc) {
    return { calc: staticCalc, isDynamic: false, content: CALCULATOR_CONTENT[slug] };
  }
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/calculator/${mainSlug}` },
    openGraph: {
...
export default async function CalculatorCatchAllPage({ params }: Props) {
  ...
  return (
    <>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {content.formula.variables.map((v) => (
                    ...
                    >
                      <code
                        className="text-xs font-black text-blue-500 shrink-0 w-10 text-center py-0.5 rounded"
            ...
            {content.examples.map((ex, i) => (
              <div key={i} className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div
                    className="rounded-lg px-4 py-3 border-l-4 border-blue-500 text-sm font-semibold"
      ...
      {/* ── Use cases + Pitfalls (side by side on md+) ───────────────────── */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {content?.useCases && content.useCases.length > 0 && (
              <div
                ...
                style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}
              >
                <h2 className="text-xs font-black uppercase tracking-widest mb-4 text-green-600 dark:text-green-400">
                  Practical Use Cases
                </h2>
            ...
            {content?.commonPitfalls && content.commonPitfalls.length > 0 && (
              <div
                ...
                style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}
              >
                <h2 className="text-xs font-black uppercase tracking-widest mb-4 text-amber-600 dark:text-amber-400">
      ...
      {content?.author && (
        <div className="max-w-5xl mx-auto px-4 pb-8">
          ...
          >
            <div
              className="w-10 h-10 rounded-full bg-blue-600 text-white font-black text-sm flex items-center justify-center shrink-0"
              aria-hidden="true"
            >
      ...
      {/* ── Related calculators ──────────────────────────────────────────── */}
      {related.length > 0 && (
        <section className="border-t py-10 px-4" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
