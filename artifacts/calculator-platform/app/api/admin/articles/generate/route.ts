...
function injectCalculatorLinks(html: string, primaryKeyword: string): string {
  ...
  if (relevant.length === 0) {
    ...
    const match = CALCULATORS.find(
      (c) =>
        kwLower.includes(c.shortName.toLowerCase()) ||
        c.keywords.some((k) => kwLower.includes(k.toLowerCase())),
    );
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const isDuplicate = existingKeywords.some(
      (k) => k.toLowerCase() === keyword.toLowerCase() || k.toLowerCase() === primaryKeyword.toLowerCase(),
    );
    if (isDuplicate) {
      return NextResponse.json(
        { error: `Duplicate keyword detected: "${keyword}" already exists in another article.` },
        { status: 409 },
      );
    }

    const allKeywords = [...new Set([primaryKeyword, keyword, ...secondaryKeywords, ...lockedKeywords])];
...
LSI + SECONDARY KEYWORDS (integrate naturally): ${allKeywords.join(', ')}
SEARCH INTENT: ${intentAnalysis || 'informational'}

CONTENT OUTLINE:
${outlineText || `## Introduction\n## Main Content\n## Examples\n## FAQ\n## Conclusion`}
...
2. FEATURED SNIPPET PARAGRAPH: The very first paragraph after the Key Takeaways box must be a 40–60 word direct answer to the main query implied by the focus keyword. It must be self-contained — someone reading only this paragraph gets a complete, factual answer. This is what Google AI Overview, ChatGPT, Gemini, and Perplexity cite as featured snippets.

3. At least ONE comparison or data table using <table>, <thead>, <tbody>, <th>, <td> with real-world data.

4. At least ONE How-To section as an ordered list: <h2>How To…</h2><ol><li>Step 1</li>…</ol>

5. FAQ section: <h2>Frequently Asked Questions</h2> then <h3> for each question (4-6 pairs). Each answer must be a complete standalone sentence — no pronouns referencing prior context. Voice assistants read these aloud.

6. End with a References section:
...
PHASE 9 — SEARCH INTELLIGENCE (apply throughout, naturally):

E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness):
- Include at least one real statistic from an authoritative source (government, academic, major institution)
- Use experiential language where natural: "In practice…", "What most people overlook…", "The real figure is…"
- Never fabricate statistics — if uncertain, omit the claim

ENTITY SEO: Mention 2–3 relevant named entities (regulatory bodies, standards organizations, specific formulas, named concepts) in their natural context. Do not list them — weave them in.

SEMANTIC SEO: Use the full vocabulary of the topic — related terms, synonyms, adjacent concepts. Depth over keyword repetition. Think: what words appear throughout a comprehensive Wikipedia article on this subject?

VOICE SEARCH: Every FAQ answer must function as a standalone spoken response. Start with the topic: "The [topic] is…" or "To [action], you…"

AI OVERVIEW OPTIMIZATION: Begin each H2 section with one crisp declarative sentence that summarizes what that section covers — AI systems use these as section anchors when generating citations.

HELPFUL CONTENT: Solve the reader's problem completely. If they'd still need to Google something basic after reading this, the article fails. Include at least one non-obvious insight per major section.
...
- Be specific: include real numbers, percentages, or examples where relevant.`;
    ...
    // Append schema scripts to content
    const schemaScripts = [
