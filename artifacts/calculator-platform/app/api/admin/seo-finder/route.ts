...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 1. Fetch real Google Autocomplete suggestions
    ...
    // 2. Ask AI to identify top keyword opportunities + generate full SEO briefs
    const systemPrompt = `You are a Senior SEO Strategist specializing in calculator and finance/health/utility content websites. Your job is to analyze real Google search queries and produce actionable keyword opportunities.

CRITICAL: Output ONLY a valid JSON object. No markdown, no explanation, no code fences.`;

    const userPrompt = `Seed Niche: "${niche}"

Real Google Autocomplete Queries (these are actual searches people type):
...
4. Exactly 5 compelling, AdSense-safe titles that include the keyword. Use formats like:
...
5. Primary keyword (exact target)
6. 5 secondary/LSI keywords
7. Search intent analysis (1-2 sentences)
8. H2/H3 outline (4-6 headings with 2-3 subpoints each)
9. Meta title (under 60 characters, includes primary keyword)
10. Meta description (under 155 characters, includes CTA)
...
Return this exact JSON structure:
{
  "opportunities": [
    ...
    let rawText = '';
    // Use ARTICLE_GEN_MODELS primary (google/gemma-4-31b) for content/SEO tasks
    const contentModels = [
