...
export default async function CalculatorPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <div className="border-b" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        <div className="max-w-5xl mx-auto px-4 pt-5 pb-6">
          ...
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 shadow-sm border" style={{ backgroundColor: 'var(--bg-input)', borderColor: 'var(--border)' }} aria-hidden="true">
              {calc.icon}
            </div>
            <div>
      ...
      {safeFormula && (
        <SectionCard title="Formula & Mathematical Basis">
          <div className="space-y-5">
            ...
            {safeFormula.variables.length > 0 && (
              <div>
                <p className="text-[11px] font-black uppercase tracking-widest mb-3" style={{ color: 'var(--text-muted)' }}>Variable Key</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {safeFormula.variables.map((v) => (
                    <div key={v.symbol} className="flex items-start gap-3 rounded-lg p-3 border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-input)' }}>
                      <code className="text-xs font-black text-blue-500 shrink-0 w-10 text-center py-0.5 rounded" style={{ backgroundColor: 'var(--bg-card)' }}>{v.symbol}</code>
              ...
              <div key={i} className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
                ...
                <div className="p-5 space-y-4">
                  <p className="text-sm italic" style={{ color: 'var(--text-muted)' }}>
                    <strong style={{ color: 'var(--text-secondary)', fontStyle: 'normal' }}>Scenario: </strong>{ex.scenario}
                  </p>
      ...
      {(safeUseCases.length > 0 || safeCommonPitfalls.length > 0) && (
        <div className="max-w-5xl mx-auto px-4 pb-8">
              ...
              <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                ...
                <ul className="space-y-2.5 list-disc list-inside">
                  {safeUseCases.map((u, i) => (
                    <li key={i} className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{u}</li>
                  ))}
                </ul>
              </div>
            )}
            {safeCommonPitfalls.length > 0 && (
              <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                <h2 className="text-xs font-black uppercase tracking-widest mb-4 text-amber-600 dark:text-amber-400">Common Pitfalls</h2>
                <ul className="space-y-2.5 list-disc list-inside">
                  {safeCommonPitfalls.map((p, i) => (
                    <li key={i} className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{p}</li>
                  ))}
                </ul>
              </div>
            )}
      ...
      {safeFaqs.length > 0 && (
        <SectionCard title="Frequently Asked Questions" accent>
          <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
            {safeFaqs.map((faq, i) => (
              <details key={i} className="group py-4">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-bold" style={{ color: 'var(--text-primary)' }}>
                  {faq.question}
                  <span className="text-blue-500 transition-transform group-open:rotate-45 text-xl font-light">+</span>
                </summary>
                <p className="pt-3 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{faq.answer}</p>
              </details>
            ))}
          </div>
        </SectionCard>
