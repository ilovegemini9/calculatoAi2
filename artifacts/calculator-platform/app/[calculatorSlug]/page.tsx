...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: pageTitle, description: calc.description, url: canonicalUrl, siteName: siteConfig.name, type: 'website', images: [{ url: siteConfig.ogImage, width: 1200, height: 630, alt: calc.name }] },
    twitter: { card: 'summary_large_image', title: pageTitle, description: calc.description },
  };
}
...
export default async function CalculatorPage({ params }: Props) {
  ...
  return <>
    ...
    {content?.howToSteps && content.howToSteps.length > 0 && <SectionCard title={`How to Use the ${calc.name}`} accent><ol className="space-y-3.5">{content.howToSteps.map((step, i) => <li key={i} id={`step-${i + 1}`} className="flex items-start gap-3"><span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white text-xs font-black flex items-center justify-center mt-0.5 shadow-sm shadow-blue-600/30">{i + 1}</span><p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{step}</p></li>)}</ol></SectionCard>}
    {content?.formula && <SectionCard title="Formula & Mathematical Basis"><div className="space-y-5"><div className="rounded-xl p-4 font-mono text-sm leading-relaxed whitespace-pre-wrap overflow-x-auto" style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)', borderLeft: '3px solid var(--border-focus)' }} aria-label="Mathematical formula">{content.formula.expression}</div>{content.formula.variables.length > 0 && <div><p className="text-[11px] font-black uppercase tracking-widest mb-3" style={{ color: 'var(--text-muted)' }}>Variable Key</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{content.formula.variables.map((v) => <div key={v.symbol} className="flex items-start gap-3 rounded-lg p-3 border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-input)' }}><code className="text-xs font-black text-blue-500 shrink-0 w-10 text-center py-0.5 rounded" style={{ backgroundColor: 'var(--bg-card)' }}>{v.symbol}</code><p className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{v.definition}</p></div>)}</div></div>}{content.formula.notes && <p className="text-xs leading-relaxed italic border-t pt-4" style={{ color: 'var(--text-muted)', borderColor: 'var(--border)' }}>📝 {content.formula.notes}</p>}</div></SectionCard>}
    {content?.examples && content.examples.length > 0 && <SectionCard title="Step-by-Step Examples"><div className="space-y-6">{content.examples.map((ex, i) => <div key={i} className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}><div className="px-5 py-3 border-b flex items-center gap-2" style={{ backgroundColor: 'var(--bg-input)', borderColor: 'var(--border)' }}><span className="w-5 h-5 rounded-full bg-blue-600 text-white text-[10px] font-black flex items-center justify-center shrink-0">{i + 1}</span><h3 className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>{ex.title}</h3></div><div className="p-5 space-y-4"><p className="text-sm italic" style={{ color: 'var(--text-muted)' }}><strong style={{ color: 'var(--text-secondary)', fontStyle: 'normal' }}>Scenario: </strong>{ex.scenario}</p><ol className="space-y-2">{ex.steps.map((step, j) => <li key={j} className="flex items-start gap-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}><span className="text-[10px] font-black text-blue-500 shrink-0 mt-0.5">{j + 1}.</span><span className="leading-relaxed">{step}</span></li>)}</ol><div className="rounded-lg px-4 py-3 border-l-4 border-blue-500 text-sm font-semibold" style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)' }}>✅ {ex.result}</div></div></div>)}</div></SectionCard>}
    {(content?.useCases?.length || content?.commonPitfalls?.length) ? <div className="max-w-5xl mx-auto px-4 pb-8"><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{content?.useCases && content.useCases.length > 0 && <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}><h2 className="text-xs font-black uppercase tracking-widest mb-4 text-green-600 dark:text-green-400">Practical Use Cases</h2><ul className="space-y-2.5 list-disc list-inside">{content.useCases.map((u, i) => <li key={i} className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{u}</li>)}</ul></div>}{content?.commonPitfalls && content.commonPitfalls.length > 0 && <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}><h2 className="text-xs font-black uppercase tracking-widest mb-4 text-amber-600 dark:text-amber-400">Common Pitfalls</h2><ul className="space-y-2.5 list-disc list-inside">{content.commonPitfalls.map((p, i) => <li key={i} className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{p}</li>)}</ul></div>}</div></div> : null}
    {content?.faqs && content.faqs.length > 0 && <SectionCard title="Frequently Asked Questions" accent><div className="divide-y" style={{ borderColor: 'var(--border)' }}>{content.faqs.map((faq, i) => <details key={i} className="group py-4"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-bold" style={{ color: 'var(--text-primary)' }}>{faq.question}<span className="text-blue-500 transition-transform group-open:rotate-45 text-xl font-light">+</span></summary><p className="pt-3 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{faq.answer}</p></details>)}</div></SectionCard>}
    <RelatedCalculators current={calc} calculators={related} />
  </>;
}
