...
function buildSpecContent(slug: string, name: string): CalcContent {
  try {
    const spec = getCalculatorSpec(slug);
    const variables = spec.inputs.map((input) => ({
      symbol: input,
    ...
    return {
      howToSteps: [
        ...
        'Check the units and values before calculating. Invalid values are rejected instead of producing a misleading result.',
        `Review the ${spec.outputs.join(', ')} result${spec.outputs.length === 1 ? '' : 's'} shown in the calculator.`,
        'Use the formula section below to understand exactly how the result is derived.',
      ],
      faqs: [
        { question: `What does the ${name} calculate?`, answer: `${name} uses the verified calculator-engine formula to transform the required inputs into the displayed result${spec.outputs.length > 1 ? 's' : ''}.` },
        { question: 'Are the calculations performed in the browser?', answer: 'Yes. The calculator logic runs locally in the browser, so the entered values do not need to be sent to a server for the calculation.' },
        { question: 'What happens if an input is invalid?', answer: 'The calculator shows a validation message instead of presenting a confident-looking but unreliable result.' },
      ],
      formula: {
      ...
      examples: [{
        ...
        result: `The result is produced directly by the ${name} calculation logic, not by a generic placeholder formula.`,
      }],
      useCases: [
        `Use ${name} for quick estimates and repeatable calculations when the listed assumptions match your situation.`,
        'Change one input at a time to understand which variables have the biggest effect on the result.',
      ],
      commonPitfalls: [
        'Mixing units or entering a value in a different unit than the input label expects.',
        'Using zero, negative, or otherwise invalid values where the formula requires a positive quantity.',
        'Treating an estimate as professional financial, medical, legal, tax, or engineering advice when the result affects a real-world decision.',
      ],
    };
  } catch {
    return {
      howToSteps: [`Enter the inputs shown in the calculator for ${name}.`, 'Review the result and the assumptions used by the calculator.'],
      faqs: [{ question: `What is ${name}?`, answer: `${name} provides an interactive calculation based on the verified implementation for this site.` }],
    };
  }
}
...
async function getCalculatorData(calculatorSlug: string) {
  ...
  if (staticCalc) return { calc: staticCalc, isDynamic: false, content: CALCULATOR_CONTENT[baseSlug] ?? buildSpecContent(baseSlug, staticCalc.name) };
  try {
  ...
  if (omni) return { calc: toCalculatorMeta(omni), isDynamic: true, dynamicSpec: { slug: omni.slug, name: omni.name, category: omni.category, metadata: { inputs: omni.inputs, outputs: omni.outputs, formula: omni.formula, howToUse: omni.howToSteps, faqItems: omni.faqs, examples: omni.examples } }, content: toCalcContent(omni) };
  return null;
}

export async function generateStaticParams() { return CALCULATORS.map((c) => ({ calculatorSlug: `${c.slug}-calculator` })); }
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const safeFormula = content?.formula && Array.isArray(content.formula.variables) ? content.formula : null;
    ...
    <>
      ...
      <div className="border-b" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        <div className="max-w-5xl mx-auto px-4 pt-5 pb-6">
          <nav className="flex items-center gap-1.5 text-xs mb-5" style={{ color: 'var(--text-muted)' }} aria-label="Breadcrumb"><Link href="/" className="hover:text-blue-500 transition-colors">Home</Link><span aria-hidden="true">/</span><span>{CATEGORY_LABELS[calc.category] || calc.category}</span><span aria-hidden="true">/</span><span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{calc.name}</span></nav>
          <div className="flex items-start gap-4"><div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 shadow-sm border" style={{ backgroundColor: 'var(--bg-input)', borderColor: 'var(--border)' }} aria-hidden="true">{calc.icon}</div><div><div className="flex items-center gap-2 mb-1.5"><span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full ${colors.bg} ${colors.text}`}>{CATEGORY_LABELS[calc.category] || calc.category}</span><span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-green-500/10 text-green-600 dark:text-green-400">Verified logic</span></div><h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight" style={{ color: 'var(--text-primary)' }}>{calc.name}</h1><p className="text-sm leading-relaxed mt-1 max-w-xl" style={{ color: 'var(--text-secondary)' }}>{calc.description}</p></div></div>
        </div>
      </div>

      <div className="mx-auto grid max-w-6xl items-start gap-8 px-4 py-8 lg:grid-cols-[minmax(0,1fr)_280px]"><div className="min-w-0">{isDynamic && dynamicSpec ? <DynamicCalculator inputs={dynamicSpec.metadata.inputs || []} outputs={dynamicSpec.metadata.outputs || []} calculatorId={baseSlug} /> : <CalculatorRenderer slug={baseSlug} />}</div><aside className="min-w-0 lg:sticky lg:top-20"><AdSlot placement="sidebar" ads={ads} /></aside></div>
      <div className="max-w-5xl mx-auto px-4 pb-8"><AdSlot placement="inContent" ads={ads} /></div>

      {safeHowToSteps.length > 0 && <SectionCard title={`How to Use the ${calc.name}`} accent><ol className="space-y-3.5">{safeHowToSteps.map((step, i) => <li key={i} id={`step-${i + 1}`} className="flex items-start gap-3"><span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white text-xs font-black flex items-center justify-center mt-0.5 shadow-sm shadow-blue-600/30">{i + 1}</span><p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{step}</p></li>)}</ol></SectionCard>}

      {safeFormula && <SectionCard title="Formula & Mathematical Basis"><div className="space-y-5"><div className="rounded-xl p-4 font-mono text-sm leading-relaxed whitespace-pre-wrap overflow-x-auto" style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)', borderLeft: '3px solid var(--border-focus)' }} aria-label="Mathematical formula">{safeFormula.expression}</div>{safeFormula.variables.length > 0 && <div><p className="text-[11px] font-black uppercase tracking-widest mb-3" style={{ color: 'var(--text-muted)' }}>Variable Key</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{safeFormula.variables.map((v) => <div key={v.symbol} className="flex items-start gap-3 rounded-lg p-3 border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-input)' }}><code className="text-xs font-black text-blue-500 shrink-0 w-10 text-center py-0.5 rounded" style={{ backgroundColor: 'var(--bg-card)' }}>{v.symbol}</code><p className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{v.definition}</p></div>)}</div></div>}{safeFormula.notes && <p className="text-xs leading-relaxed italic border-t pt-4" style={{ color: 'var(--text-muted)', borderColor: 'var(--border)' }}>📝 {safeFormula.notes}</p>}</div></SectionCard>}

      {safeExamples.length > 0 && <SectionCard title="Step-by-Step Examples"><div className="space-y-6">{safeExamples.map((ex, i) => <div key={i} className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}><div className="px-5 py-3 border-b flex items-center gap-2" style={{ backgroundColor: 'var(--bg-input)', borderColor: 'var(--border)' }}><span className="w-5 h-5 rounded-full bg-blue-600 text-white text-[10px] font-black flex items-center justify-center shrink-0">{i + 1}</span><h3 className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>{ex.title}</h3></div><div className="p-5 space-y-4"><p className="text-sm italic" style={{ color: 'var(--text-muted)' }}><strong style={{ color: 'var(--text-secondary)', fontStyle: 'normal' }}>Scenario: </strong>{ex.scenario}</p><ol className="space-y-2">{(Array.isArray(ex.steps) ? ex.steps : []).map((step, j) => <li key={j} className="flex items-start gap-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}><span className="text-[10px] font-black text-blue-500 shrink-0 mt-0.5">{j + 1}.</span><span className="leading-relaxed">{step}</span></li>)}</ol><div className="rounded-lg px-4 py-3 border-l-4 border-blue-500 text-sm font-semibold" style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)' }}>✅ {ex.result}</div></div></div>)}</div></SectionCard>}

      {(safeUseCases.length > 0 || safeCommonPitfalls.length > 0) && <div className="max-w-5xl mx-auto px-4 pb-8"><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{safeUseCases.length > 0 && <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}><h2 className="text-xs font-black uppercase tracking-widest mb-4 text-green-600 dark:text-green-400">Practical Use Cases</h2><ul className="space-y-2.5 list-disc list-inside">{safeUseCases.map((u, i) => <li key={i} className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{u}</li>)}</ul></div>}{safeCommonPitfalls.length > 0 && <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}><h2 className="text-xs font-black uppercase tracking-widest mb-4 text-amber-600 dark:text-amber-400">Common Pitfalls</h2><ul className="space-y-2.5 list-disc list-inside">{safeCommonPitfalls.map((p, i) => <li key={i} className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{p}</li>)}</ul></div>}</div></div>}

      {safeFaqs.length > 0 && <SectionCard title="Frequently Asked Questions" accent><div className="space-y-5">{safeFaqs.map((faq, i) => <div key={i} className="border-b pb-5 last:border-b-0 last:pb-0" style={{ borderColor: 'var(--border)' }}><h3 className="font-bold text-sm" style={{ color: 'var(--text-primary)' }}>{faq.question}</h3><p className="text-sm leading-relaxed mt-2" style={{ color: 'var(--text-secondary)' }}>{faq.answer}</p></div>)}</div></SectionCard>}
