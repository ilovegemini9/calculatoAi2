    ...
    try {
      ...
      if (!controller.signal.aborted) {
        if (!res.ok || !Array.isArray(data.opportunities) || data.opportunities.length === 0) {
          setDiscoveryError(data.error ?? 'No live opportunities available.');
        } else {
      ...
      if (!controller.signal.aborted) setDiscoveryError('Discovery request failed. Check your network connection.');
    } finally {
      ...
      <section className="rounded-2xl border" style={{ borderColor: phase === 'idle' || phase === 'researching' ? 'var(--blue-500, #3b82f6)' : 'var(--border)', backgroundColor: 'var(--bg-card)' }}>
        ...
        <div className="p-5 space-y-4">
          ...
          {loadingDiscovery && phase === 'idle' && (
            <LoadingPulse message="Fetching Google Trends (trending now), Reddit hot, Quora — analysing live signals with AI…" />
          )}

          {/* Discovery error — shown when discovery fails */}
          {!loadingDiscovery && discoveryError && phase === 'idle' && (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/8 px-4 py-3">
              <div className="flex items-start gap-3">
                <Info className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" />
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-amber-700 dark:text-amber-400">{discoveryError}</p>
                  {(discoveryError.toLowerCase().includes('api key') || discoveryError.toLowerCase().includes('openrouter')) && (
                    <Link
                      href="/admin/settings/ai"
                      className="mt-1.5 inline-flex items-center gap-1 text-xs font-semibold text-amber-600 hover:text-amber-500 dark:text-amber-400"
                    >
                      Go to AI Settings →
                    </Link>
                  )}
                ...
                <button
                  ...
                  className="shrink-0 rounded-lg border border-amber-500/30 px-2.5 py-1.5 text-xs font-semibold text-amber-700 transition hover:border-amber-500 dark:text-amber-400"
                >
                  <RefreshCw className="inline h-3 w-3 mr-1" />Retry
                </button>
              </div>
            </div>
          )}
