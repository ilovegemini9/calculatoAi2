...
export default function AdsPage() {
  ...
  if (!ads) return <div className="flex min-h-[420px] items-center justify-center flex-col gap-3"><XCircle className="h-8 w-8 text-red-500" /><p className="text-sm font-medium text-red-500">{message?.text ?? 'Unable to load ad settings.'}</p><button type="button" onClick={() => void load()} className="text-xs text-blue-500 underline">Try again</button></div>;
